...
class LibraryReader(object):
    ...
    def _read_libraries(self):
        ...
        with scope.contrast_scope():
            ...
            logger.debug(
                ...
                [lib.get("file_path", "") for lib in packages],
            )

            libraries = [convert_dict_to_library(lib) for lib in packages]

            self._send_analysis_results_appstart(libraries)
            self._send_analysis_results_files_loaded(results, ignore_dist_list)

    def _send_analysis_results_appstart(self, libraries):
        """
        Send library discovery in ApplicationUpdate message.
        :param libraries: list of dtm.Library instances
        """
        if not libraries or not self.send_message_func:
            return
    ...
    def _send_analysis_results_files_loaded(self, dists_discovered, ignore_dist_list):
        if not dists_discovered or not self.send_message_func:
            return
        ...
        for loaded_files, dist_dict in dists_discovered:
            if id(dist_dict) in ignore_dist_list:
                continue

            if len(loaded_files) > 0:
                send_msg = True
                dist_hash = dist_dict["hash_code"]

                total_loaded_files_cnt += append_files_loaded_to_activity(
                    activity_dtm, loaded_files, dist_hash
                )
...
def search_dist(dist):
    ...
    result = (
        total_used_files,
        create_package(version, manifest, file_count, name, url, library_hash,),
    )
