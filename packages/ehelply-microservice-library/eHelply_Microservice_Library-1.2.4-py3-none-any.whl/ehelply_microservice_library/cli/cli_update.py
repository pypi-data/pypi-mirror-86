...
@cli.command()
def install(root: Optional[Path] = typer.Option(None)):
...
@cli.command()
def preview(root: Optional[Path] = typer.Option(None)):
...
@cli.command()
def review(start_version: str = typer.Option(..., prompt=True), end_version: str = typer.Option(..., prompt=True), root: Optional[Path] = typer.Option(None)):
    with typer.progressbar(length=4, label="Processing") as progress:
        root_path: Path = root
        if not root_path:
            root_path = Path(os.getcwd())
