...
class DataFrameDirectory(AnyDirectory):
    ...
    def remove_arrow_bug_columns_from_data_frame_and_schema(cls, df: pd.DataFrame, schema: DataFrameSchema):
        invalid_columns = [index for index in _get_arrow_index_names(df) if index in df.columns]
        for name in invalid_columns:
            col_index = schema._get_column_index_by_key(name)
            schema._remove_from_label_score_feature(col_index=col_index)
            schema.column_attributes.remove(col_index)
