...
def main(config, data_name):
    ...
    if config.mode == 'train':
        ...
        trainer.train()
    elif config.mode == 'examples':
        params = Params('config/params.json', data_name)

        test_inputs, test_labels = padding_sentence(params.max_sequence_length, 'examples')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=str, default='train', choices=['train', 'examples', 'vectorize'])
    args = parser.parse_args()
    args_test = copy.deepcopy(args)
    args_test.mode = 'examples'
