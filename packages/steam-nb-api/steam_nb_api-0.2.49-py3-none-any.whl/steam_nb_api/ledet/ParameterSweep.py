...
class ParameterSweep(object):
    ...
    def adjust_vQ(self, ROXIE_File):
        ...
        if max(self.ParametersLEDET.Inputs.nStrands_inGroup) > 1:
            strandCount = 0
            GroupCount = 0
            Bcopy = np.zeros(int(sum(self.ParametersLEDET.Inputs.nT)))
            for i in range(int(sum(self.ParametersLEDET.Inputs.nT))):
                Bcopy[i] = sum(B[int(strandCount):int(strandCount+self.ParametersLEDET.Inputs.nStrands_inGroup[GroupCount])])/self.ParametersLEDET.Inputs.nStrands_inGroup[int(GroupCount)]
                TurnSum = sum(self.ParametersLEDET.Inputs.nT[0:GroupCount+1])
                if i>TurnSum: GroupCount = GroupCount + 1
                strandCount = strandCount + self.ParametersLEDET.Inputs.nStrands_inGroup[GroupCount]
            B = Bcopy
        ...
        print(cable.f_SC)
        print(self.ParametersLEDET.Inputs.f_SC_strand_inGroup)
        print(self.ParametersLEDET.Inputs.wBare_inGroup)
        print(self.ParametersLEDET.Inputs.hBare_inGroup)
        print(cable.A_CableInsulated)
