...
def export_fusion_samples(
    ...
    ds_kwargs = dict(region=region, start_time=start_time, end_time=end_time, rescale=True)
    ...
    s1 = s1.apply_func(geeutils.add_indices, indices=['vv_vh_ratio','ndpi','vv_vh_abs_sum'])
    ...
    for i in range(n):
        try:
            ...
            if stratification_img is not None:
                ...
                ).addBands(elv_img).stratifiedSample(
