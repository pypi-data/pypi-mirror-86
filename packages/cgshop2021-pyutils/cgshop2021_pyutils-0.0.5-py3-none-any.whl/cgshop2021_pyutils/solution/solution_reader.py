...
class DirectoryInstanceCache:
    ...
    def _load_all(self):
        for f in self.files.values():
            ...
            except (OSError, json.JSONDecodeError, RuntimeError, InvalidInstanceError) as e:
                pass
...
class SolutionReader:
    ...
    def _expect_type(in_obj, key, t, in_obj_name):
        ...
        if not isinstance(result, t):
            raise SolutionEncodingError(f'{in_obj_name}\'s attribute "{key}" is not a {str(t)}!')
        return result

    def _to_solution_step(self, step_obj):
        if not isinstance(step_obj, dict):
            raise SolutionEncodingError(f'The solution contains an invalid step (not an object)!')
        result = SolutionStep(-1)
        for key, value in step_obj.items():
            try:
                robot_id = int(key)
                if robot_id < 0 or robot_id >= self._n_robots:
                    raise SolutionEncodingError(f'The robot id {robot_id} is out of range!')
                ...
                raise SolutionEncodingError(f'A step in the solution contains an attribute that is not an integer!')
        return result
