...
binarysort(MergeState *ms, sortslice lo, PyObject **hi, PyObject **start)
{
    ...
    for (; start < hi; ++start) {
        ...
        do {
            p = l + ((r - l) >> 1);
            //if (PyLong_AsLong(pivot) < PyLong_AsLong(*p))
            IFLT(pivot, *p)
                r = p;
            else
                l = p+1;
...
static PyObject *
list_sort_impl(PyListObject *self, PyObject *keyfunc, int reverse, Py_ssize_t minrun)
...
void
PyList_Sort(PyObject *self, Py_ssize_t minrun) {
    self = list_sort_impl((PyListObject *)self, NULL, 0, minrun);
}
