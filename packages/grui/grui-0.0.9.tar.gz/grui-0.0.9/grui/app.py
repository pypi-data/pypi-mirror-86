...
class GruiApp:
    ...
    def build_wsgi(self, import_name: str, logger_override=None, **kwargs):
        ...
        if not self.built:
            ...
            self.__config.setdefault('INDEX_FILE_PATH', "./index.html")
            self.__config.setdefault('FAVICON_FILE_PATH', "./favicon.ico")
            self.__config.setdefault('STATIC_FOLDER_PATH', "static")
            self.__config.setdefault('URL_PREFIX', "/api")

            self.__flask_app = Flask(import_name, **kwargs)
            ...
            if os.path.isfile(self.config['FAVICON_FILE_PATH']):
                self.__flask_app.route('/favicon')(self.serve_favicon_file)

            if os.path.isdir(self.config['STATIC_FOLDER_PATH']):
                self.__flask_app.route('/static/<path:path>')(self.serve_static_file)
            ...
            for function in self.registered_function.values():
                self.log(LogLevel.INFO, "register", "%-7s" % function.method, function.path)
    ...
    def serve_favicon_file(self):
        return send_file(self.config['FAVICON_FILE_PATH'])

    def serve_static_file(self, path: str):
        return send_from_directory(self.config["STATIC_FOLDER_PATH"], path)
