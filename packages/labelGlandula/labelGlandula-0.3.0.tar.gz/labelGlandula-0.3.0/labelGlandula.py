...
from predict import detectGlandula
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None, defaultSaveDir=None):
        ...
        detect = action('Detect glandula and save', self.detectGlandula,
        ...
        createMode = action('Add Glandula', self.setCreateMode,
        ...
        self.actions = struct(save=save, save_format=save_format, saveAs=saveAs, open=open, close=close,
                              resetAll=resetAll,
                              lineColor=color1, create=create, createScale=createScale,delete=delete, edit=edit, #createRegion=createRegion,
        ...
        self.menus = struct(
            ...
            # edit=self.menu('&Edit'),
            #view=self.menu('&View'),
            glandula = self.menu('&Edit Glandula'),
            help=self.menu('&Help'),
        ...
        addActions(self.menus.glandula, (create,  detect, createScale, excel, close)) #createRegion,trainModel,close))
        ...
        self.actions.beginner = (
            open, opendir, openNextImg, openPrevImg,None, save,  detect,excel,None,create, createScale, delete) #,trainModel, createRegion,
    ...
    # Callback functions:
    # Importante.
    def newShapeGlandula(self, text='glandula'):
    ...
    def detectGlandula(self, _value=False):
        ...
        if ui.genera:
            if ui.default_1:#if it is by default
                # Detectamos estomas automáticamente, para ello vamos a usar las funciones del archivo detectGlandula.py. Por eso
                ...
                if len(self.mImgList) <= 1:
                    ...
                    box.show()
                    detectGlandula.mainDataset(path)
                    box.close()
                    self.defaultSaveDir = self.targetDirPath
                    self.importDirImages(self.targetDirPath)
