...
class Universe(object):
    ...
    def __init__(self,verbose=False):
        try:
            ...
            self._polity   = PolityClient(api_key=static.API_KEY,verbose=self.verbose)
            self._pytrends = GoogleTrendClient(hl='en-US', tz=360)

            self.ciks    = list(set(self._polity.cik_to_name.keys()))
            self.names   = list(set(self._polity.cik_to_name.values()))
            self.tickers = list(set(self._polity.cik_to_ticker.values()))
            
            messages = self._polity._fetch_equities_messages()
            static.initialized(self.verbose,messages,self._polity.graphql,len(self.ciks))
