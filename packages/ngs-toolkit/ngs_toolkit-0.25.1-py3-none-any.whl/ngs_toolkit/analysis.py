...
class Analysis(object):
    ...
    def generate_report(
        ...
        images = {k: [x for x in v if x.endswith(".svg")] for k, v in list(outputs.items())}
        # Generate dict = {"section": [(caption, file), ...]}
        images = {
            k.capitalize().replace("_", " "): [(fix_name(x, self.name), x,) for x in v]
            for k, v in list(images.items())
        }
        csvs = {k: [x for x in v if x.endswith(".csv")] for k, v in list(outputs.items())}
        csvs = {
            k.capitalize().replace("_", " "): [(fix_name(x, self.name), x,) for x in v]
        ...
        output = template.render(
            analysis=self,
            project_repr={k: v for k, v in list(self.__dict__.items()) if isinstance(v, str)},
    ...
    def unsupervised_analysis(
        ...
        **kwargs: optional
        ...
        cd_kwargs = {
            k: v for k, v in list(kwargs.items()) if k in ["pallete", "uniform_cmap", "diverging_cmap"]
        }
