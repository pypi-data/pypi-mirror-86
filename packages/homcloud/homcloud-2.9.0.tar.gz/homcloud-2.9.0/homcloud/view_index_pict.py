...
from homcloud.diagram import Histogram
...
def main(args=None):
    ...
    if pd.filtration_type != MapType.bitmap:
        sys.stderr.write("Incorrect index information\n")
        exit(1)
    ...
    pairs = [pair for pair in pd.pairs_positions() if all_true(predicates, pair)]
...
def argument_parser():
    ...
    p.add_argument("--birth-color", type=utils.parse_color, default=(255, 0, 0),
                   help="birth pixel color")
    p.add_argument("--death-color", type=utils.parse_color, default=(0, 0, 255),
                   help="death pixel color")
    p.add_argument("--line-color", type=utils.parse_color, default=(0, 255, 0),
...
def pairs_positions(pd):
    warnings.warn("view_index_pict.pairs_positions is obsolete. Please use PD.pairs_positions")
    return pd.pairs_positions()
