...
class Finder(object):
    def __init__(self, diagram):
        ...
        self.bitmap = diagram.index_map.indexed_bitmap()
        assert self.index_map.type() == MapType.bitmap

    def query_pair(self, birth, death):
        path = search(self.bitmap, birth, tuple(self.index_map.positions[birth]))
        return Result(self.index_map.resolve_level(birth),
                      self.index_map.resolve_level(death),
                      path)
