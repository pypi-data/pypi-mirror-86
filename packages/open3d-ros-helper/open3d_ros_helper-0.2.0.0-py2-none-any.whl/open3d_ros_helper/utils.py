...
def convert_nparray_to_rostf(transform_nparray, source_frame, target_frame):
    """
    convert geometry_msgs/PoseStamped to 4x4 numpy array
    """
    tf_stamped = transformStamped
    tf_stamped.header.stamp = rospy.Time.now()
    tf_stamped.header.frame = source_frame
    tf_stamped.child_frame_id = target_frame

    translation = transform_nparray[:3, 3] 
    rotation = tf_trans.quaternion_from_matrix(transform_nparray)

    tf_stamped.transform.position.x = translation[0]
    tf_stamped.transform.position.y = translation[1]
    tf_stamped.transform.position.z = translation[2]
    tf_stamped.transform.rotation.x = rotation[0]
    tf_stamped.transform.rotation.x = rotation[1]
    tf_stamped.transform.rotation.x = rotation[2]
    tf_stamped.transform.rotation.x = rotation[3]

    return tf_stamped



def do_transform_o3d_cloud(cloud_o3d, tf_stamped):
    ...
    H[:3, 3] = [tf_stamped.transform.translation.x, 
