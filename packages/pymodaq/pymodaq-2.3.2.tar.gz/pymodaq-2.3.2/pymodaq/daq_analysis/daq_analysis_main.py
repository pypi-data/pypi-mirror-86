            ...
            if attrs.CLASS == 'ARRAY' or attrs.CLASS == 'CARRAY':  # means there is data to be plotted
                if 'scan_type' not in attrs_names:  # means one of the navigation axis or other type of data
                    ...
                    self.ui.navigator2D.set_scaling_axes(
                        scaling_options=edict(
                            scaled_xaxis=edict(label="x axis", units=None, offset=np.min(self.nav_x_axis),
                                               scaling=self.nav_x_axis[1] - self.nav_x_axis[0]),
                            scaled_yaxis=edict(label="y axis", units=None, offset=np.min(self.nav_y_axis),
                                               scaling=self.nav_y_axis[1] - self.nav_y_axis[0])))
                    if attrs.data_type == '0D':
            ...
            if self.loaded_data is not None:
                ...
                if len(data.shape) == 0:  # means 0D data, plot on 1D viewer
                    ...
                    self.ui.viewer2D.set_scaling_axes(
                        scaling_options=edict(scaled_xaxis=edict(label="x axis", units=None, offset=np.min(self.x_axis),
                                                                 scaling=self.x_axis[1] - self.x_axis[0]),
                                              scaled_yaxis=edict(label="y axis", units=None, offset=np.min(self.y_axis),
                                                                 scaling=self.y_axis[1] - self.y_axis[0])))
