...
class CalibrationCamera(QtWidgets.QWidget,QObject):
    ...
    def save_data(self):
        params = [{'title': 'Axis options:', 'name': 'axes', 'type': 'group', 'visible': False, 'expanded': False, 'children': [
                    {'title': 'X axis:', 'name': 'xaxis', 'type': 'group', 'children': [
                        ...
                        {'title': 'Scaling', 'name': 'xscaling', 'type': 'float', 'default': 1.,
                                            'value': self.settings.child('xaxis', 'dx_units').value()/self.settings.child('xaxis', 'dx_fit').value()},
                    ]},
                    {'title': 'Y axis:', 'name': 'yaxis', 'type': 'group', 'children': [
                        ...
                        {'title': 'Scaling', 'name': 'yscaling', 'type': 'float', 'default': 1.,
                                            'value': self.settings.child('yaxis', 'dy_units').value()/self.settings.child('yaxis', 'dy_fit').value()},
                    ]},
                ]},]
        param_obj = Parameter.create(name='Axes_Settings', type='group', children=params)
        pymodaq.daq_utils.parameter.ioxml.parameter_to_xml_file(param_obj, os.path.join(calib_path, os.path.split(self.fname)[1]))
