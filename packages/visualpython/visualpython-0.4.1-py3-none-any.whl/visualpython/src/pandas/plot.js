define([
        ...
        $(this.wrapSelector('.vp-plot-item')).click(function() {
            ...
            if (type == 'scatter' || type == 'hexbin') {
                ...
                if (idx > -1) {
                    ...
                    // 라벨 필수 입력 표시
                    $(that.wrapSelector('#x')).closest('tr').find('label').text("* " + that.package.variable[idx].label);
                }
                var idy = that.package.variable.findIndex(function(item) {return item.name === 'y'});
                if (idy > -1) {
                    that.package.variable[idy].required = true;
                    // 라벨 필수 입력 표시
                    $(that.wrapSelector('#y')).closest('tr').find('label').text("* " + that.package.variable[idy].label);
                }
            } else {
    ...
    PandasPackage.prototype.bindOptionDivider = function(div1Count) {
        var option2 = $(this.wrapSelector('#vp_optionBox table tr')).filter(':gt(' + div1Count + ')');
        $(this.wrapSelector('#vp_optionBox2 table')).append(option2);
    }
     ...
     * 사용자 옵션 추가 페이지 이벤트
     */
    PandasPackage.prototype.bindUserOption = function() {
        var that = this;
    ...
    PandasPackage.prototype.generateCode = function(addCell, exec) {
        ...
        try {
            ...
            // 코드 생성
            var result = pdGen.vp_codeGenerator(this.uuid, this.package, userOption.toString());
