define([
    ...
    OptionPackage.prototype.loadUdfList = function() {
        ...
        vpSetting.loadUserDefinedCodeList(function(udfList) {
            console.log('udfList', udfList);

            // TEST: test list
            // udfList= [ {name: 'test1', code: 'codecode long code here hahahahahahahahaha example test one two three'}];
    
            udfList.forEach(obj => {
                if (obj.code != null && obj.code != undefined) {
                    var trow = $(`<tr></tr>`);
                    var tdTitle = $(`<td class="vp-udf-key">${obj.name}</td>`);
                    ...
                    tdTitle.click(function() {
                        console.log('click title', $(this).text());
                        
                        var key = $(this).text();
                        if (key == undefined || key === "") return;

                        var code = $(this).closest('tr').find('.vp-udf-code pre').text();
                    ...
                    // click x to remove udf from list
                    tdDelete.find('button').click(function() {
                        // FIXME: vpCommon.renderYesOrNoModal 수정 필요 (appendTo('#header'))
                        // check again before removal
                        // vpCommon.renderYesOrNoModal(function() {
                        //     // change message
                        //     $('.yesOrNoModal-title').html(`<span>삭제하시겠습니까?</span>`); // FIXME: vp-multilang
                        //     // event for yes / no
                        //     $('.yesOrNoModal-inner .yesOrNoModal-yes').click(function() {
                        //         console.log('yes');
                        //     });
        
                        //     $('.yesOrNoModal-inner .yesOrNoModal-no').click(function() {
                        //         console.log('no');
                        ...
                        if (key != undefined
                            && vpSetting.getUserDefinedCode(key) != undefined) {
                            // remove key
