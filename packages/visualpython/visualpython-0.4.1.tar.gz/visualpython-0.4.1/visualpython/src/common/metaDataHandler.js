define([
    ...
    var MdHandler = function (funcID) {
        ...
        this.prefixSelector = '#vp_prefixBox textarea';
        this.postfixSelector = '#vp_postfixBox textarea';
    };
    ...
    MdHandler.prototype.generateMetadata = function(pageThis, inputIdList) {
        ...
        // get prefix value
        metadata.prefix.value = $(pageThis.wrapSelector(this.prefixSelector)).val();
        // get postfix value
        metadata.postfix.value = $(pageThis.wrapSelector(this.postfixSelector)).val();
    ...
    MdHandler.prototype.loadMdAsTag = function(pageThis, cellIdx = -1) {
        ...
        var prefixTag = $(pageThis.wrapSelector(this.prefixSelector));
        if (prefix.id != undefined && prefix.id != '') {
            prefixTag = $(pageThis.wrapSelector('#' + prefix.id));
        }
        prefixTag.val(prefix.value);

        // set postfix
        var postfix = metadata.postfix;
        var postfixTag = $(pageThis.wrapSelector(this.postfixSelector));
        if (postfix.id != undefined && postfix.id != '') {
            postfixTag = $(pageThis.wrapSelector('#' + postfix.id));
        }
        postfixTag.val(postfix.value);
    }
