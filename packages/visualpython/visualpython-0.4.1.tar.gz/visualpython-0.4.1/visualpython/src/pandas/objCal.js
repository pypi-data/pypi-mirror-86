define([
    ...
    PandasPackage.prototype.bindOptions = function() {
        ...
        if (this.package.variable == undefined || this.package.variable.length <= 0) {
            $(this.wrapSelector('#vp_optionBox')).closest('div.vp-pandas-block').remove();
        }
    };
     ...
     * 객체 연산자 선택지 구성
     */
    PandasPackage.prototype.bindCalculatorTypes = function() {
        // 객체 연산자 선택지 구성
        var tagTr = document.createElement('tr');
        var tagTd1 = document.createElement('td');
        var tagTd2 = document.createElement('td');
        var tagLbl = document.createElement('label');
        var id = 'vp-calculatorType';

        $(tagLbl).attr({
            'for': id
        });
        tagLbl.innerText = '객체 연산 유형';
        tagTd1.appendChild(tagLbl);
        ...
        // 연산 옵션 추가 add / sub / div / mul 
        var calList = Object.keys(this.calculator);
        calList.forEach(calType => {
            ...
            tagOption.append(document.createTextNode(calType));
            tagSelect.appendChild(tagOption);
        });
        tagTd2.appendChild(tagSelect);
        tagTr.appendChild(tagTd1);
        tagTr.appendChild(tagTd2);

        // 화면에 추가
        $(this.wrapSelector('#vp_inputOutputBox table')).append(tagTr);

        var packageObject = this;
        // 선택 이벤트 : 선택한 연산 유형에 따라 package 변경
        $(tagSelect).change(function() {
            packageObject.package = libPandas._PANDAS_FUNCTION[packageObject.calculator[this.value]];
            packageObject.showFunctionTitle();
        })
    }
