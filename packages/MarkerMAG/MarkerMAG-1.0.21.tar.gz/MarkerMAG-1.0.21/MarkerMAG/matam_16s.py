...
MarkerMAG matam_16s -p Test -r1 R1.fasta -r2 R2.fasta -t 12 -force -ref path/to/SILVA_128_SSURef_NR95
...
def matam_16s(args):
    ...
    uclust_output_table                     = '%s/%s_assembled_16S.uc'                 % (matam16s_wd, output_prefix)
    ...
    uclust_cmd = '%s -cluster_fast %s -id %s -centroids %s -uc %s -sort length -quiet' % (usearch_exe, combined_all_depth_matam_assemblies, uclust_iden_cutoff, uclust_output_fasta, uclust_output_table)
