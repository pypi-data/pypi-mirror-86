...
def test_string_quote_and_tokenize():
    ...
    for quote_token in ["'", '"', "'''", '"""']:
        ...
        def check(string):
            input_string = freephil.tokenizer.quote_python_str(
                quote_token=quote_token, string=string
            )
            words = freephil.tokenize_value_literal(
    ...
    words = tvl(
        input_string=freephil.tokenizer.quote_python_str(quote_token='"', string=value)
    )
    assert len(words) == 1
    ...
    assert len(words) == 1
    assert words[0].value == value
    word = freephil.tokenizer.word(value=value, quote_token='"')
...
class recycle:
    def __init__(
        ...
        self.parameters = freephil.parse(input_string=input_string)
        self.out = StringIO()
        ...
        if prefix == "":
            out_parameters = freephil.parse(input_string=self.out)
            out_out = StringIO()
            out_parameters.show(
                out=out_out,
                prefix=prefix,
...
""",
    ...
    for expert_level in [-1, 2, 3, None]:
        assert not show_diff(
            params.as_str(expert_level=expert_level),
            """\
x = 1
y = 2
z = 3
...
""",
    ...
    recycle(
        input_string="s{a=1;b=2  b ;c=3}t{x=4;y=5;}",
        expected_out="""\
s {
  a = 1
  b = 2 b
...
""",
    ...
    recycle(
        input_string="a=None;.type=int;s .multiple=True;.optional=False{c=None}"
...
def test_syntax_errors():
    ...
    _test_exception("x=;", "Missing value for x (input line 1)")
    _test_exception("s{z=}", "Missing value for z (input line 1)")
    _test_exception("s{y={", "Missing value for y (input line 1)")
    _test_exception("s{y=1{", 'Syntax error: unexpected "{" (input line 1)')
    _test_exception("s{y=#", "Missing value for y (input line 1)")
    _test_exception("a b", 'Syntax error: expected "=", found "b" (input line 1)')
    _test_exception("{}", 'Syntax error: unexpected "{" (input line 1)')
    _test_exception("a {", 'Syntax error: no matching "}" for "{" at input line 1')
    _test_exception(";{}", 'Syntax error: unexpected ";" (input line 1)')
    _test_exception("s{;}", 'Syntax error: unexpected ";" (input line 1)')
    _test_exception(
        "a=b\n.foo none", "Unexpected definition attribute: .foo (input line 2)"
    )
    _test_exception('a=b\nc "abc', "Syntax error: missing closing quote (input line 2)")
    _test_exception("1 {", 'Syntax error: improper scope name "1" (input line 1)')
    _test_exception("scope\n.foo", "Unexpected scope attribute: .foo (input line 2)")
    _test_exception(
        "a. 2", 'Syntax error: improper definition name "a." (input line 1)'
    )
    _test_exception("a.include=None", 'Reserved identifier: "include" (input line 1)')
    _test_exception("include {}", 'Reserved identifier: "include" (input line 1)')
    _test_exception("a.include.b.c {}", 'Reserved identifier: "include" (input line 1)')
    _test_exception("__foo__=None", 'Reserved identifier: "__foo__" (input line 1)')
    _test_exception("__foo__ {}", 'Reserved identifier: "__foo__" (input line 1)')
    _test_exception(
        "a=None\n.type=foo", 'Unexpected definition type: "foo" (input line 2)'
    )
    _test_exception(
        "s .multiple=True .optional=False {}",
...
def test_phil_on_off_end():
    ...
    assert (
        freephil.parse(input_string="#phil __OFF__\n#phil __ON__ \na=1").as_str()
        == "a = 1\n"
    )
    assert (
        freephil.parse(input_string="#phil __END__\n#phil __ON__ \na=1").as_str() == ""
    )
    params = freephil.parse(
        input_string="""\
...
def check_get(parameters, path, expected_out=None, with_substitution=False):
    out = StringIO()
    parameters.get(path=path, with_substitution=with_substitution).show(out=out)
    out = out.getvalue()
...
""",
    ...
    assert [
        item.path for item in parameters.all_definitions(suppress_multiple=True)
...
"""
    ...
    try:
        check_resolve_variables(parameters, "l")
    except RuntimeError as e:
...
"""
    ...
    assert (
        parameters.get(path="h").objects[0].words[0].where_str()
        == ' (environment: "_X_Y_Z_")'
    )
    check_get_sub(parameters, path="o", expected_out='o = "xyz/abc.ext"\n')
    check_get_sub(parameters, path="p", expected_out='p = "xyz/abc.ext"\n')
    check_get_sub(
        parameters, path="q", expected_out='q = "xyz/abc.extxyz/abc.extxyz/abc.ext"\n'
    )
    check_get_sub(
        parameters, path="r", expected_out='r = "xyz/abc.extxyz abc.extxyz/abc.ext"\n'
    )
...
"""
    ...
    check_get_sub(
        parameters, path="h", expected_out='h = "abcyes nobc" "12yes nox56"\n'
    )
    try:
        parameters.get(path="i")
    except RuntimeError as e:
        assert (
            str(e)
            == "Syntax error: $ must be followed by an identifier:"
            + ' "$" (input line 10)'
        )
    else:
        raise Exception_expected
    try:
        parameters.get(path="j")
    except RuntimeError as e:
        assert str(e) == 'Syntax error: missing ")": "$(abc" (input line 11)'
    else:
        raise Exception_expected
    try:
        parameters.get(path="k")
    except RuntimeError as e:
        assert (
            str(e)
            == 'Syntax error: improper variable name "$(1bc)"' + " (input line 12)"
        )
    else:
        raise Exception_expected
    try:
        parameters.get(path="l")
    except RuntimeError as e:
        assert str(e) == 'Syntax error: improper variable name "$()" (input line 13)'
    else:
        raise Exception_expected
    try:
        parameters.get(path="m")
    except RuntimeError as e:
...
""",
    ...
    assert not show_diff(
        params.as_str(attributes_level=1),
        """\
x = 1
  .help = u
s
...
""",
            ...
            assert (
                str(e) == '"include scope" must be followed one or two arguments,'
...
""",
        ...
        try:
            freephil.parse(
                input_string="""\
include scope freephil.t_s_t.include_scope_target_1
""",
                process_includes=True,
            )
        except ImportError as e:
            assert (
                str(e) == "include scope: no module freephil.t_s_t (input line 1)"
                " or possibly import errors in module freephil.t_s_t"
...
"""
    ...
    source = freephil.parse(input_string="")
...
"""
    ...
    source = freephil.parse("")
    assert not show_diff(
        master.fetch(source=source).as_str(),
        """\
s {
  t {
    v = 1
...
"""
    ...
    master_optional = freephil.parse(
        input_string="""\
s
  .optional=yes
  .expert_level=1
...
"""
    ...
    master_multiple = freephil.parse(
        input_string="""\
s
  .multiple=1
  .expert_level=1
...
"""
    ...
    master_optional_multiple = freephil.parse(
        input_string="""\
s
  .multiple=1
  .optional=yes
...
""",
    ...
    source = freephil.parse(input_string="a { }", source_info="source")
    try:
        master.fetch(source=source)
    except RuntimeError as e:
        assert (
            str(e) == 'Incompatible parameter objects: definition "a" '
            '(master, line 1) vs. scope "a" (source, line 1)'
        )
    else:
        raise Exception_expected
    master.fetch(source=source, skip_incompatible_objects=True)
    source = freephil.parse(input_string="b=None")
    try:
        master.fetch(source=source)
    except RuntimeError as e:
        assert (
            str(e) == 'Incompatible parameter objects: scope "b" '
            '(master, line 2) vs. definition "b" (input line 1)'
        )
    else:
        raise Exception_expected
    master.fetch(source=source, skip_incompatible_objects=True)
    source = freephil.parse(input_string="c { a { } }")
    try:
        master.fetch(source=source)
    except RuntimeError as e:
        assert (
        ...
        raise Exception_expected
    master.fetch(source=source, skip_incompatible_objects=True)
    source = freephil.parse(input_string="d { a=None\n}")
    try:
        master.fetch(source=source)
    except RuntimeError as e:
        assert (
            str(e) == 'Incompatible parameter objects: scope "a" '
...
"""
    ...
    for choice in ["a", "b", "c"]:
        for stat in ["", "*"]:
            source = freephil.parse(input_string="c=" + choice)
            out = StringIO()
...
"""
    ...
    try:
        parameters.fetch(source=parameters)
    except RuntimeError as e:
...
"""
    ...
    source = freephil.parse(input_string="s.a=x")
    out = StringIO()
    master.fetch(source=source).show(out=out)
    assert not show_diff(
        out.getvalue(),
...
"""
    ...
    source = freephil.parse(input_string="s.t.a=x")
    out = StringIO()
    master.fetch(source=source).show(out=out)
    assert not show_diff(
        out.getvalue(),
...
"""
    ...
    source = freephil.parse(input_string="s.t.a.b=x")
    out = StringIO()
    master.fetch(source=source).show(out=out)
    assert not show_diff(
        out.getvalue(),
...
""",
    ...
    for input_string in [
        ...
        source = freephil.parse(input_string=input_string)
        out = StringIO()
        master.fetch(source=source).show(out=out)
        assert not show_diff(
            out.getvalue(),
...
"""
    ...
    source_af = freephil.parse(input_string="s.a.f=1")
    source_bf = freephil.parse(input_string="s.b.f=2")
    source_bg = freephil.parse(input_string="s.b.g=3")
    assert not show_diff(
        master.fetch(sources=[source_af, source_bf]).as_str(),
        """\
s {
  a {
    f = 1
...
""",
    ...
    assert not show_diff(
        master.fetch(sources=[source_bg, source_af]).as_str(),
        """\
s {
  a {
    f = 1
...
""",
    ...
    assert (
        master.fetch(sources=[source_bg, source_af, source_bf]).as_str()
        == """\
s {
  a {
    f = 1
...
"""
    ...
    assert not show_diff(
        master.fetch(sources=[custom, custom]).as_str(),
        """\
s {
  a = 2
  a = 3
...
""",
    ...
    assert not show_diff(
        master.fetch(sources=[custom, master]).as_str(),
        """\
a = z
a = d
a = x
...
"""
    ...
    custom = freephil.parse("")
    assert not show_diff(
        master.fetch(sources=[custom] * 3).as_str(),
        """\
a {
  x = 1
  y = None
...
""",
    ...
    assert not show_diff(
        master.fetch(sources=[custom, master, master, custom]).as_str(),
        """\
a {
  x = 1
  y = None
...
""",
    ...
    assert not show_diff(
        master.fetch(sources=[custom, master, master, custom, master]).as_str(),
        """\
a {
  x = 3
  y = 4
...
"""
    ...
    prev = freephil.parse(master.as_str())
    custom = freephil.parse(
        """\
s {
  a=x
...
""",
    ...
    source = freephil.parse(input_string="s.b = z")
    d = master.fetch_diff(source=source)
    assert not show_diff(
        d.as_str(),
        """\
...
""",
    ...
    source = freephil.parse(input_string="s.a = x; s.b = z")
    d = master.fetch_diff(source=source)
    assert not show_diff(
        d.as_str(),
        """\
...
""",
    ...
    source = freephil.parse(input_string="s.t.b = z")
    d = master.fetch_diff(source=source)
    assert not show_diff(
        d.as_str(),
        """\
...
""",
    ...
    source = freephil.parse(input_string="s.a = x; s.t.b = z")
    d = master.fetch_diff(source=source)
    assert not show_diff(
        d.as_str(),
        """\
...
""",
    ...
    source = freephil.parse(input_string="s.t.u.b = z")
    d = master.fetch_diff(source=source)
    assert not show_diff(
        d.as_str(),
        """\
...
""",
    ...
    source = freephil.parse(input_string="s.a = x; s.t.u.b = z")
    d = master.fetch_diff(source=source)
    assert not show_diff(
        d.as_str(),
        """\
...
"""
    ...
    source = freephil.parse(input_string="a = x")
    d = master.fetch_diff(source=source)
    assert not show_diff(
        d.as_str(),
        """\
...
""",
    ...
    source = freephil.parse(input_string="a = x; a = None; a = y")
    d = master.fetch_diff(source=source)
    assert not show_diff(
        d.as_str(),
        """\
...
""",
    ...
    source = freephil.parse(input_string="s.a = x; s { a = None; b = z } s.a = p")
    d = master.fetch_diff(source=source)
    assert not show_diff(
        d.as_str(),
        """\
...
"""
    ...
    source = freephil.parse(input_string="s.t.a = None")
    d = master.fetch_diff(source=source)
    assert not show_diff(
        d.as_str(),
        """\
...
""",
    ...
    for input_string in ["", "s.t.a = x", "s.b = None", "s { t.a = x; b = None }"]:
        source = freephil.parse(input_string=input_string)
        d = master.fetch_diff(source=source)
        assert d.is_empty()
    #
...
"""
    ...
    os.environ["_X_Y_Z_"] = "xyz"
    source = freephil.parse(input_string="a = $_X_Y_Z_")
...
"""
    ...
    d = master.fetch_diff()
    assert d.is_empty()
    source = freephil.parse(input_string="s.t.u.b=c2")
    d = master.fetch_diff(source=source)
    assert d.is_empty()
    source = freephil.parse(input_string="s.t.u { b=c3; b=c4; b=c3 }")
    d = master.fetch_diff(source=source)
    assert not show_diff(
        d.as_str(),
        """\
...
"""
    ...
    assert (
        parameters.get(path="group.a", with_substitution=False).objects[0].extract()
        is True
    )
    assert parameters.get(path="group.a", with_substitution=False).objects[
        1
    ].extract() == ["yes", "n o"]
    assert (
        parameters.get(path="group.b", with_substitution=False).objects[0].extract()
        == 13
    )
    assert (
        parameters.get(path="group.c", with_substitution=False).objects[0].extract()
        == 1.3
    )
    assert (
        parameters.get(path="group.d", with_substitution=False).objects[0].extract()
        == "abc def ghi"
    )
    assert (
        parameters.get(path="group.e", with_substitution=False).objects[0].extract()
        == "b"
    )
    try:
        parameters.get(path="group.e", with_substitution=False).objects[1].extract()
    except RuntimeError as e:
        assert (
            str(e) == "Multiple choices for group.e;"
            " only one choice can be selected (input line 13)"
        )
    else:
        raise Exception_expected
    try:
        parameters.get(path="group.e", with_substitution=False).objects[2].extract()
    except RuntimeError as e:
        assert (
            str(e) == "Unspecified choice for group.e:"
            " exactly one choice must be selected (input line 15)"
        )
    else:
        raise Exception_expected
    try:
        parameters.get(path="group.f", with_substitution=False).objects[3].extract()
    except RuntimeError as e:
        assert (
            str(e) == "Unspecified choice for group.f:"
            " at least one choice must be selected (input line 24)"
        )
    else:
        raise Exception_expected
    assert (
        parameters.get(path="group.e").objects[0].type
    ...
    assert parameters.get(path="group.f", with_substitution=False).objects[
        0
    ].extract() == ["b"]
    assert parameters.get(path="group.f", with_substitution=False).objects[
        1
    ...
    assert (
        parameters.get(path="group.f", with_substitution=False).objects[2].extract()
        == []
    )
    assert (
        parameters.get(path="group.f").objects[0].type
    ...
    assert (
        parameters.get(path="group.g", with_substitution=False).objects[0].extract()
        == "/var/tmp/foo"
    )
    assert (
        parameters.get(path="group.h", with_substitution=False).objects[0].extract()
        == "var.tmp.foo"
    )
    assert (
        parameters.get(path="group.i", with_substitution=False).objects[0].extract()
        == 2
    )
    try:
        parameters.get(path="group.i", with_substitution=False).objects[1].extract()
    except RuntimeError as e:
        assert (
            str(e) == 'Error interpreting group.i="1/2" as'
            " an integer expression (input line 33)"
        )
    else:
        raise Exception_expected
    assert (
        parameters.get(path="group.j", with_substitution=False).objects[0].extract()
        == 0.5
    )
    try:
        parameters.get(path="group.j", with_substitution=False).objects[1].extract()
    except RuntimeError as e:
        assert (
            str(e) == """Error interpreting group.j="'a'" as"""
            " a floating-point expression (input line 37)"
        )
    else:
        raise Exception_expected
    try:
        parameters.get(path="group.j", with_substitution=False).objects[2].extract()
    except RuntimeError as e:
        assert (
            str(e)
            == 'Error interpreting group.j="a" as a numeric expression:'
            + " NameError: name 'a' is not defined (input line 39)"
        )
    else:
        raise Exception_expected
    assert (
        parameters.get(path="group.n", with_substitution=False).objects[0].extract()
        is None
    )
    assert parameters.get(path="group.m", with_substitution=False).objects[
        0
    ].extract() == ["Plain", "%^ &*"]
    assert (
        parameters.get(path="group.s0", with_substitution=False).objects[0].extract()
        is None
    )
    assert parameters.get(path="group.s1", with_substitution=False).objects[
        0
    ].extract() == [" a ", "b"]
    assert (
        parameters.get(path="group.v", with_substitution=False).objects[0].extract()
        is None
    )
    assert [
        word.value
        for word in parameters.get(path="group.w", with_substitution=False)
        .objects[0]
        .extract()
    ] == ["plain", "% ^&*"]
    ...
    try:
        ...
        raise Exception_expected
    try:
        parameters.get(path="group.int_false", with_substitution=False).objects[
            0
        ].extract()
    except RuntimeError as e:
        assert not show_diff(
        ...
        raise Exception_expected
    try:
        parameters.get(path="group.float_true", with_substitution=False).objects[
            0
        ].extract()
    except RuntimeError as e:
        assert not show_diff(
        ...
        raise Exception_expected
    try:
        parameters.get(path="group.float_false", with_substitution=False).objects[
            0
        ].extract()
    except RuntimeError as e:
        assert not show_diff(
...
"""
    ...
    group = parameters.get(path="group", with_substitution=False).objects[0].extract()
...
"""
    ...
    assert extracted.t.c.d.__phil_path_and_value__(object_name="e") == ("t.c.d.e", "f")
    #
    try:
        extracted.z = 12
    except AttributeError as e:
        assert not show_diff(
            str(e),
            """Assignment to non-existing attribute "z"
  ...
  a new attribute use: obj.__inject__(name, value)""",
    ...
    try:
        ...
        assert not show_diff(
            str(e),
            """Assignment to non-existing attribute "t.c.z"
  Please correct the attribute name, or to create
  a new attribute use: obj.__inject__(name, value)""",
        )
    else:
...
""",
        ...
        master = freephil.parse(input_string=input_string)
        source = freephil.parse(input_string="")
        ...
        if opt is None or opt:
            ...
            assert len(e.a) == 1
            e.a[0] is None
        source = freephil.parse(input_string="a=None")
        w = master.fetch(source=source)
        e = w.extract()
        if opt is None or opt:
            assert len(e.a) == 0
...
""",
        ...
        source = freephil.parse(input_string="s {}")
        w = master.fetch(source=source)
        e = w.extract()
        if opt is None or opt:
            assert len(e.s) == 0
...
foo_converter_registry = freephil.extended_converter_registry(
...
"""
    ...
    try:
        ...
        assert str(e) == "Improper master choice definition: x = *a a (input line 1)"
    else:
        raise Exception_expected
    #
    master = freephil.parse(
...
"""
    ...
    py_params.x = []
    assert not show_diff(
        master.format(py_params).as_str(),
        """\
x = a b
""",
    )
...
"""
    ...
    diff_phil = freephil.parse(input_string=s.getvalue())
...
""",
    ...
    try:
        params.get(path="a").objects[0].extract()
    except RuntimeError as e:
        assert (
            str(e) == ".type=foo2 does not have a from_words method (input line 1):"
            " AttributeError: 'foo2_converters' object has no attribute 'from_words'"
        )
    else:
        raise Exception_expected
    try:
        params.get(path="a").objects[0].format(python_object=0)
    except RuntimeError as e:
        assert (
            str(e) == ".type=foo2 does not have an as_words method (input line 1):"
...
""",
        ...
        assert (
            str(e) == 'Error constructing definition type "foo1": RuntimeError:'
...
""",
        ...
        assert (
            str(e) == 'Error constructing definition type "foo2(foo=1)": TypeError:'
...
"""
        ...
        try:
            ...
            assert not show_diff(
                str(e), "a element cannot be %s (input line 1)" % value
            )
        else:
            raise Exception_expected
        work_params = master_phil.extract()
...
"""
    ...
    assert not show_diff(
        work_phil.extract_format().as_str(),
        """\
a = 12.34 45.67
b = 3 4
""",
...
"""
    master_phil = freephil.parse(input_string=master_string)
...
Error interpreting command line argument as parameter definition:
  ...
  RuntimeError: Missing value for xyz (command line argument, line 1)""",
    ...
    assert pcl.parse is freephil.parse
...
"""
    ...
    def custom_processor(arg):
        try:
            ...
            return freephil.parse("sites=%d" % val)

    pcl = master_phil.command_line_argument_interpreter()
    args = ["2"]
    user_phil = pcl.process_args(args=["2"], custom_processor=custom_processor)
...
"""
    ...
    class _showwarning:
        ...
        def __call__(self, message, category, *args, **kwds):
            if category is freephil.PhilDeprecationWarning:
                self.n += 1
    ...
    user3 = freephil.parse("""strategy=xyz+tls""")
...
"""
    ...
    scope2 = freephil.find_scope(current_phil=phil_scope, scope_name="scope2")
    s = StringIO()
    scope2.show(out=s)
    assert not show_diff(
        s.getvalue(),
...
"""
    ...
    for phil_str, result in [
        (buggy_phil_str, "a {\n  enabled = True\n}\n"),
        (correct_phil_str, ""),
    ]:
        parsed_phil = freephil.parse(phil_str)
...
"""
    master_phil_extract = freephil.parse(master_phil_str).extract()
    assert master_phil_extract.scope.name == "original"

    # change to a new valid name
    new_default_str = 'scope.name = "new"'
    new_phil_str = freephil.change_default_phil_values(master_phil_str, new_default_str)
    new_phil_extract = freephil.parse(new_phil_str).extract()
