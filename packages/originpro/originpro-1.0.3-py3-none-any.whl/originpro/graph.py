...
class GLayer(BaseLayer):
    ...
    def add_plot(self, obj, coly=-1, colx=-1, colz=-1, type = 's'):
        """
        Add plot to graph layer
        Parameters:
            wks(WSheet): worksheet to plot from
            ...
            type(str): 'l'(Line Plot) 's'(Scatter Plot) 'y' (Line Symbols) 'c' (Column)
        Returns:
        ...
        if isinstance(obj, (origin_class('DataRange'), str)):
            return self._add_plot(obj, type)
        wks = obj
