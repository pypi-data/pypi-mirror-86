...
class MUSESelector():
    ...
    def select(self, df : pd.DataFrame, target : str) -> list:
        ...
        features_data = dict()
        self.X_columns = [col for col in df.columns if col != target]
        ...
        for col in list(df.columns):
            ...
            if df[col].dtype in numerics:
                ...
                df[col] = pd.qcut(df[col], self.__n_bins, duplicates='drop', labels=False)
            ...
            for q in df[col].unique():
                labels['impurity'][q] = min(prob / len(df) for prob in dict(df[df[col]==q][target].value_counts()).values())
                labels['H'][q] = sum(-p / len(df) * math.log2(p / len(df)) for p in dict(df[df[col]==q][target].value_counts()).values())
        ...
        for i in range(self.__num_features):
            ...
            for feature in features_data.keys():
                ...
                J[feature] = sum(len(df[df[feature] == q])/len(df)*H[q] for q in selected_intervals)
            ...
            for feature in features_data:
                for i in range(len(features_data[feature]['impurity'])):
                    if features_data[feature]['impurity'][i] < self.__T:
                        features_data[feature]['discarded'].append(i)
