...
本模块主要是为了解决unittest运行测试用例的生成HTML报告的测试问题，同时扩展了邮件发送等功能
该模块预留了两个入口，
...
class TestRunner():
    ...
    def __get_dingtalk_content(self):
        """获取钉钉通知的内容"""
        template_path = os.path.join(os.path.dirname(__file__), '../templates')
        env = Environment(loader=FileSystemLoader(template_path))
    ...
    def get_except_info(self):
        ...
        for i in self.result:
            for texts in i.failures:
                t, content = texts
                except_info.append("用例【{}】执行失败，失败信息如下：".format(t._testMethodDoc))
                except_info.append(content)
            for texts in i.errors:
                t, content = texts
                except_info.append("用例【{}】执行错误，错误信息如下：".format(t._testMethodDoc))
                except_info.append(content)
        except_str = "\n".join(except_info)
        return except_str
    ...
    def dingtalk_notice(self, url, key=None, secret=None, atMobiles=None, isatall=False, except_info=False):
        ...
        """
        # 获取用例执行的结果,转换为md格式
        res_text = self.__get_dingtalk_content()
        # 判断是否需要发送错误用例的信息
        if except_info:
            res_text += '### 未通过用例详情：\n'
