from engpy.misc.gen import getter, start_alpha_index
    ...
    def get(self):
        for coeff in self.expr.expr:
            ...
            if not coeff:
                continue
            if coeff < 0:
                disp_ = f' - {abs(num(coeff))}' if coeff != -1 else ' - 1 ' if self.expr.expr[coeff][0] == {
                    '': 0} else ' - '
            else:
                disp_ = f'{coeff}' if coeff != 1 else ' 1 ' if coeff == 1 and self.expr.expr[coeff][0] == {
                    '': 0} else ''
            for count, expr_ in enumerate(self.expr.expr[coeff]):
                _var__ = ''
                emb = 0
                if count:
                    disp_ = f' - ' if coeff == -1 and self.expr.expr[coeff][count] != {
                        '': 0} else f' - {abs(num(coeff))}' if coeff < 0 else f'{coeff}' if coeff > 0 and coeff != 1 else ' 1 ' if \
                        self.expr.expr[coeff][count] == {'': 0} else ''
                for count_, var in enumerate(expr_):
                    _var__ = ''
                    if var and not expr_[var]:
                        continue
                    _var__ = f'({var})' if len(var) > 1 or '/' in format(
                        var) else f'{var}' if not coeff - 1 and not isinstance(var,
                                                                               str) else f'({var})' if not isinstance(
                        var, str) and expr_[var] != 1 else f'{var}'
                ...
                yield (
                    (f'{coeff}' if not str(coeff).replace(' ', '')[0].isalnum() else f' + {coeff}') if emb else disp_)
