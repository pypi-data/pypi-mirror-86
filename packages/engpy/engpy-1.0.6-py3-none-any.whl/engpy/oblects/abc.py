...
from engpy.misc.assist import deepcopy, List
...
class MatrixObjectClass(BaseClass):
    ...
    def cofactors(self):
        ...
        for integer in range(self.rows):
            cofactor.add_row(
                [cofactor.append(self.cofactor(integer + 1, integers + 1)) for integers in range(self.cols)])
        return cofactor
    ...
    def __eq__(self, other):
        ...
        if getter(other, 'name') not in ('Matrix', 'Matrix_'):
            raise InvalidOperation(f'A Matrix can only be compared to Another Matrix Objects, not {type(other)}')
        if self.isdisabled:
            raise OperationNotAllowed('This Matrix is Disabled')
        return self.Mat == other.Mat
    ...
    def random(self, order, boundary=(-50, 50)):
        ...
        return S if getter(self, 'name') == 'Matrix' else S.matrix_
        ...
        if not aug_mat:
            ...
            if not self.isempty and self.rows != aug_mat.rows:
                raise DimensionError(
                    f"Can't concatenate Matrices with different number of rows: {self.rows} and {aug_mat.rows}")
        aug_mat = aug_mat.trn()
        _mat_ = self.trn()
        for rows in aug_mat:
    ...
    class matrix:
        ...
        def isunit(self):
            ...
            if not self.issq or self[1, 1] != 1:
                return False
            return self == self.unit(self.order)
