...
class Log(BaseClass):
    ...
    def __init__(self, arg=0, base=10, coeff=1, hkeys='', s=False):
        ...
        self.arg = lexer(f"{arg}", hkeys=hkeys)
    ...
    def diff(self, var):
        return ((self.coeff * self.arg.lin_diff(var)) / self.arg * (1 if isinstance(self.base,
                                                                                    float) and round(self.base,
                                                                                                     10) == round(
            alnum(e),
            10) else lexer(f'log{self.base}(.e)')))
    ...
    def cal(self, values='', desolve=True):
        ...
        try:
            return coeff * lexer(str(mth.log10(arg) / mth.log10(base)))
        except ValueError:
            raise InvalidOperation(
                f"log to base 10 is only valid for positive numbers: log10({arg if arg <= 0 else base})")
...
class log(ExpressionObjectClass, UtilityClass):
    ...
    def __str__(self, stray=False):
        ...
        for coeff in self.expr:
            ...
            if coeff < 0:
                disp += (f" - {abs(num(coeff))}" if numable(coeff_) else f"{coeff_}") if coeff_ != -1 else ' - 1' if \
                    self.expr[coeff_] == [{'': 0}] else ' - '
            else:
                if disp:
                    disp = ' + '
                disp += f"{coeff_}" if coeff_ != 1 else ' 1 ' if (coeff_ == 1) and (
                        self.expr[coeff_] == [{'': 0}]) else ''
            for count, expr_ in enumerate(self.expr[coeff]):
                if count:
                    disp += ' - 1 ' if ((
                                            f"{coeff_}") if coeff < 0 else ' + {coeff_}' if coeff > 0 else coeff == -1) and (
                                               self.expr[coeff_] == [{'': 0}] or self.expr[coeff_][count] == {
                                           '': 0}) else ' + 1' if (coeff == 1) and (
                            self.expr[coeff] == [{'': 0}] or self.expr[coeff][count] == {
                        '': 0}) else ' - ' if coeff == -1 else f" - {abs(num(coeff))}" if numable(
                        coeff_) else f" - {coeff}" if coeff < 0 else f" + {coeff}" if coeff > 0 else ' + '
                for var in expr_:
    ...
    def get(self):
        ...
        for coeff in self.expr:
            ...
            if coeff < 0:
                ...
                if disp:
                    disp = ' + '
                disp = f'{coeff_}' if coeff < 0 else ' + {coeff_}' if coeff > 0 else ' - 1 ' if coeff == -1 and (
                        self.expr[coeff_] == [{'': 0}] or self.expr[coeff_][count] == {
                    '': 0}) else ' + 1' if coeff == 1 and (
                        self.expr[coeff] == [{'': 0}] or self.expr[coeff][count] == {
                    '': 0}) else f' - ' if coeff == -1 else (f' - {abs(num(coeff))}' if numable(
                    coeff_) else f'{coeff}') if coeff < 0 else ' + ' if coeff == 1 else f' + {coeff}' if coeff > 0 else ' + '
            for count, expr_ in enumerate(self.expr[coeff]):
                if count:
                    disp = ' - 1 ' if ((
                                           f"{coeff_}") if coeff < 0 else ' + {coeff_}' if coeff > 0 else coeff == -1) and (
                                          ...
                                          '': 0}) else ' + 1' if (coeff == 1) and (
                            self.expr[coeff] == [{'': 0}] or self.expr[coeff][count] == {
                        '': 0}) else ' + ' if coeff == 1 else ' - ' if coeff == -1 else (
                        f" - {abs(num(coeff))}" if numable(coeff_) else (
                            f" - {abs(num(coeff))}") if coeff < 0 else f" + {coeff}" if coeff > 0 else ' + ')
                for var in expr_:
                    disp += f"{var}" if var else str(abs(coeff)) if (not disp) else ''
                    if expr_[var] not in (1, 0):
                        disp += f"^{expr_[var]}"
    ...
    def lin_diff(self, var=''):
        ...
        for terms in self.struct:
            if isinstance(terms, log):
                coeff = terms.coeff
                log_expr = list(terms.expr[coeff][0])[0]
