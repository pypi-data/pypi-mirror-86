...
from engpy.misc.miscs import simp_var, lexer
...
class cos:
    def __init__(self, arg=90, coeff='1', arc='', h=False, hkeys=''):
        ...
        if isinstance(arg, dict):
            self.expr = arg
            return
        self.expr = {alnum(coeff): [lexer(str(arg), hkeys=hkeys)]}

    def __str__(self):
        disp = ''
        sin_ = ''
        for coeff, trigs in self.expr.items():
            ...
            for count, _trig in enumerate(trigs):
                if count:
                    disp += sin_
                disp += f'{str(coeff)}{self.arc}cos{self.h}##' \
                        f'({_trig})' if coeff != -1 and coeff != 1 else f'{str(coeff).replace("1", "")}{self.arc}' \
                                                                        f'cos{self.h}##({_trig})'
        return disp
    ...
    def cal(self, values='', desolve=True):
        ...
        try:
            val = lexer(
                str(eval(f'list(self.expr)[0] * math.{arc}cos{self.h}(self.args._cal(values, desolved = desolve))')),
                skip=desolve)
    ...
    def laplace(self, in_var, out_var):
        return lexer(f'{out_var}/({out_var}^2 {"-" if self.h else "+"} ({self.args.coeff(in_var)})^2)')

    def ztransform(self, in_var, out_var):
        return lexer(f'{out_var}^2 - {out_var}cos{self.h}({self.args.coeff(in_var)})') / lexer(
            f'{out_var}^2 - 2{out_var}cos{self.h}({self.args.coeff(in_var)}) + 1')

    def in_exp(self):

        return lexer(f'e^(j({self.args})) + e^(-j({self.args}))') / 2
...
class sin:
    ...
    def cal(self, values='', desolve=True):
        ...
        try:
            val = lexer(
                str(eval(f'list(self.expr)[0] * math.{arc}sin{self.h}(self.args._cal(values, desolved = desolve))')),
                skip=desolve)
            return val if desolve else val.inroots
        except Exception:
    ...
    def laplace(self, in_var, out_var):
        return lexer(
            f'({self.args.coeff(in_var)})/({out_var}^2 {"-" if self.h else "+"} ({self.args.coeff(in_var)})^2)')

    def ztransform(self, in_var, out_var):
        return lexer(f'{out_var}sin{self.h}({self.args.coeff(in_var)})') / lexer(
            f'{out_var}^2 - 2{out_var}cos{self.h}({self.args.coeff(in_var)}) + 1')
...
class tan:
    ...
    def cal(self, values='', desolve=True):
        ...
        try:
            val = lexer(
                str(eval(f'list(self.expr)[0] * math.{arc}tan{self.h}(self.args._cal(values, desolved = desolve))')),
                skip=desolve)
            return val if desolve else val.inroots
        except Exception:
...
class trig(ExpressionObjectClass, UtilityClass):
    ...
    def __mul__(self, other):
        if isinstance(other, (int, float, set)) or numable(
                str(other)):  # Ensuring that we are dealing with Expr or trig object
            skip = False
            if isinstance(other, set):
                other = list(other)[0]
                skip = True
            if not getter(other, 'name') == 'Expr':
                other = lexer(format(other), skip=skip)
            if numable(self):
                return other * self
            ...
            for coeffs, vars_ in exprs_.expr.items():
                ...
                for vars__ in vars_[0]:
                    if not isinstance(vars__, str):
                        _vars = vars__.cal(values, desolve=desolve) ** (
                            {(vars_[0][vars__] if numable(vars_[0][vars__]) else vars_[0][vars__].cal(values,
                                                                                                      desolve=desolve))} if desolve else (
                                vars_[0][vars__] if numable(vars_[0][vars__]) else vars_[0][vars__].cal(values,
                                                                                                        desolve=desolve)))
                    else:
                        _vars = 1
    ...
    def lin_diff(self, var=''):
        ...
        for trigs in self.struct:
            ...
            if not len(trigs_) - 1:
                ...
                coef = _trigs_.coeff
                _var = list(_trigs_.expr[coef][0])[0]
                pow_ = _trigs_.expr[coef][0][_var]
                prime = _var.diff(var) if pow_ > 0 else _var.diff_(var)
                coef_ = prime.coeff
                var_ = prime.expr[coef_][0]
                ...
                _trig += __trig_ * coeff + trig({1: [trigs_]}) * coeff.lin_diff(var) if not numable(
                    coeff) else coeff * __trig_
    ...
    def __pow__(self, other):
        ...
        if isinstance(other, float) or not numable(other):
            if numable(self) and numable(other):
                ...
                coeff = self.coeff
                arg = self.expr[coeff][0]
                arg[list(arg)[0]] *= other
                return trig({coeff: [arg]})
        for interger in range(abs(other) - 1):
            self_ *= self
        return self_ if other > 0 else lexer('1') if not other else 1 / self_
