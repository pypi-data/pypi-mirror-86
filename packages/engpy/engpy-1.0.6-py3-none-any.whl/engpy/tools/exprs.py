...
from engpy.misc import helpers as help_
import random
...
class Expr(ExpressionObjectClass, Utilities.expr, BasicOperatorsClassABC, UtilityClass, Article, AI.Implementation):
    ...
    def simp(self, psort=False, desolve=True):
        ...
        alpha = sorted(alpha.union({char for char in ('s', 'c', 't', 'l') if char in ff}))
        res = {}
        sim = {}
        arr = {}
        arr_ = {}
    ...
    def integrate(self, var_='x', repeat=1, *args, **kwargs):
        ...
        for expr_ in self:
            ...
            for coeff, var in tmp_.expr.items():
                var__ = tmp_.expr[coeff]
                if var_ in var__[0]:
                    var__[0][var_] += 1
                else:
                    var__[0][var_] = 1
                pw = var__[0][var_]
            if 1 / pw * coeff in tmp:
                tmp[1 / pw * coeff].append(var__)
            else:
                tmp[1 / pw * coeff] = var__

        return (
            Expr(tmp).simp() if not args and not kwargs else Expr(tmp).cal(*args, **kwargs)) if repeat == 1 else Expr(
            tmp).integrate(var_, repeat - 1, *args, **kwargs)
    ...
    def __psort__(self, init='', direct=True):
        ...
        for coeff, var in vars_.expr.items():
            if coeff:
                __vars__[coeff] = var
        return self if format(Expr(__vars__).replace(' ', '')) == format(self).replace(' ', '') else Expr(__vars__).simp(True)
            ...
            for count, expr_ in enumerate(self.expr[coeff]):
                ...
                if count:
                    disp_ = ' - 1 ' if numm_ == -1 and (self.expr[coeff] == [{'': 0}] or self.expr[coeff][count] == {
                        ...
                        '': 0}) else f' - ' if numm_ == -1 else f' - {abs(numm_)}' if numm_ < 0 else f' + {numm_}' if numm_ > 0 and numm_ != 1 else ' + '
                for count_, var in enumerate(expr_):
