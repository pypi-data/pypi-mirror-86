...
from engpy.misc.gen import imap, star
from engpy.misc.assist import getter, List
...
class Matrix(MatrixObjectClass, Utilities.matrix, StaticStates):
    ...
    def __init__(self, X='', *Y):
        ...
        if Y:
            ...
            try:
                ...
                raise UnacceptableToken(
                    f'Unaccepetable entry: {items}'
                    f', Note that Matrix class only accepts Numbers, you can use Matrix_ instead')
        if isinstance(X, set):
            try:
                self.Mat = [fnum(items) if c > 1 else items for c, items in enumerate(X)]
            except Fizzle:
        ...
        if isinstance(X, ndarray) or isinstance(X, (list, tuple)):
            ...
            try:
                ...
                col = int(order.split('x')[1])
                row = int(order.split('x')[0])
                Mat.append(row)
                Mat.append(col)
                count = 1
                while count <= row:
                        ...
                        if len(element) == col:
                            for elements in element:
                                try:
                                    ...
                                    raise UnacceptableToken(
                                        f'Unaccepetable entry: {elements}'
                                        f', Note that Matrix class only accepts Numbers, you can use Matrix_ instead')
                            break
                        _num = "element" if len(element) == 1 else 'elements'
                        print(f"Error, You Entered {len(element)} {_num} instead of {col}")
    ...
    def unit(self, order):
        ...
        for integer in range(order[0]):
            for integers in range(order[1]):
                _mat_.append(1 if integer == integers else 0)
        __mat.Mat = _mat_
        return mat
    ...
    def __setitem__(self, key, value):
        """ Set elements in a MatObj
            MatObj[i] = value
                if i is an integer:
                    if i < 0, then backward counting; -1 will select the last row, -2 will select the second to the
                        last row and so on
        ...
        if isinstance(key, int):
            ...
            if key[0] == '*':
                ...
                raise OutOfRange(
                    f"Entry {key} doesn't exist. This Matrix only has {self.rows} rows and {self.cols} columns")
            else:
    ...
    def __abs__(self):
        ...
        for integers in range(self.cols):
            ...
            result += (-1) ** integers * mult[integers] * abs(self)
    ...
    def add_row(self, *row):
        ...
        if self.isimut:
            raise InvalidOperation('This Matrix is Immutable')
        row_ = []
        empty = True if len(self.Mat) == 2 else False
        nel = 0
        for rows in row:
    ...
    def __invert__(self):
        ...
        for integer in range(emulated.cols):  # row
            ...
            for integers in range(emulated.rows):  # col
                count_ = 0
                if self[integers + 1, integer + 1] != emulated[integers + 1, integer + 1]:
                    if emulated[integers + 1, integer + 1] == 1:
                        try:
                        ...
                        while True:
                            try:
                                if integers > integer:
                                    self.perform(rows=[
                                        (self[integers + 1, integer + 1] / self[integer + 1, integer + 1]) ** (-1),
                                        integers + 1])
                                    self.perform('-', [integers + 1, integer + 1])
                                elif integers < integer:
                                    self.perform(
                                        rows=[(self[integers + 1, integer + 1] / self[integer + 1, integer + 1]),
                                              integer + 1])
                                    self.perform('-', [integers + 1, integer + 1])
                                break
                            except ZeroDivisionError:
    ...
    def ev(self, repeat=False):
        ...
        if self.isdisabled:
            raise OperationNotAllowed('This Matrix is Disabled')
        return self.matrix_.ev(repeat=repeat)
        ...
        try:
            ...
            try:
                for i in range(2, 6):
                    if self_.unit:
                        print(
                            f'This Matrix raised to the power of {i - 1} gives its inverse\n '
                            f'Hence This Matrix raised to the power of {i} gives a Unit Matrix')
                        break
                    self_ *= self
            except Exception:
                pass
        ...
        if isinstance(other, int) or isinstance(other, float):
            ...
            if not self.cols == other.rows:
                raise DimensionError(
                    f'Incompatible Matrices: {" x ".join(self.order)} and {" x ".join(other.self.order)}')
...
class Matrix_(MatrixObjectClass, Utilities.matrix, StaticStates):
    ...
    def null(self, order):
        ...
        _mat_ = self.empty
        if isinstance(order, int):
            _mat_.Mat = [order, order]
            order = order * order

        else:
            _mat_.Mat = [order[0], order[1]]
            order = order[0] * order[1]
        _mat_.Mat += [Expr({0: [{'': 0}]}) for i in range(order)]
        return _mat_
    ...
    def __abs__(self):
        ...
        if not self.issq:  # For Square Matrix certainty
            raise InvalidOperation('Determinants are only supported by Square Matrices')
        mat_init = tuple(self.Mat)
        res = 0
        if self.order == [2, 2]:
    ...
    def __lshift__(self, other):
        ...
        if self.order != other.order:
            raise DimensionError(f'Only Matrices of the same order can be compared: {" x ".join(self.order)} and'
                                 f' {" x ".join(other.order)}')
    ...
    def __rshift__(self, other):
        ...
        if not isinstance(other, (Matrix_, Matrix)):
            raise InvalidOperation(f'Only Matrices can be compared: Matrix and {type(other)}')
        if self.order != other.order:
            raise DimensionError(f'Only Matrices of the same order can be compared: {" x ".join(self.order)} '
                                 f'and {" x ".join(other.order)}')
    ...
    def perform(self, operation='', rows=[], A=1, B=1, cols=[]):
        ...
        if not operation:
            ...
            row2 = self.get_row(rows[1])
            for integer in range(self.cols):
                self[rows[0], integer + 1] = (A * row1[integer] + B * row2[integer]).simp() if operation == '+' else (
                        A * row1[integer] - B * row2[integer]).simp()
    ...
    def quad(self, vec=''):
        if vec:
            ...
            if len(vec) != self.cols:
                raise UnacceptableToken(
                    f'This Matrix has {self.cols} columns; so, a {self.cols} column vector is expected, not {len(vec)}')
    ...
    def __invert__(self):
        ...
        for integer in range(emulated.cols):  # row
            ...
            for integers in range(emulated.rows):  # col
                count_ = 0
                if self[integers + 1, integer + 1] != emulated[
                    integers + 1, integer + 1]:  # To check if the elements corresponds as in Unit Matrix
                    if emulated[integers + 1, integer + 1] == 1:  # To try convert the element to 1
                        ...
                        while True:
                            try:
                                if integers > integer:
                                    self.perform(rows=[self[integer + 1, integer + 1] / self[integers + 1, integer + 1],
                                                       integers + 1])
                                    self.perform('-', [integers + 1, integer + 1])
                                elif integers < integer:
                                    self.perform(
    ...
    def nullspace(self):
        ...
        self_ = copy(self)
        self_.add_col(zeros)
        solns = eqn.solve(matrix=self_, vmat=var, q='')
        vlist = []
    ...
    def property(self):
        ...
        try:
            ...
            print(self.type)
            try:
                print(f'Determinant is {abs(self)}')
            except InvalidOperation:
                print('Determinant Not Supported')
            print(f'Rank is {self.rank}')
            try:
