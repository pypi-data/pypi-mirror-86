...
def lexpr(*expr):
    try:
        ...
        from engpy.tools.exprs import Expr
    return Expr(expr[0]) if len(expr) == 1 else [Expr(exprs) for exprs in expr]


def lexer(expr,keys = '',hkeys = '',skip  = '',  **kwargs):
    try:
        Expr
    except NameError:
        from engpy.tools.exprs import Expr
    return Expr(expr,keys=keys, hkeys=hkeys, skip=skip,  **kwargs)
