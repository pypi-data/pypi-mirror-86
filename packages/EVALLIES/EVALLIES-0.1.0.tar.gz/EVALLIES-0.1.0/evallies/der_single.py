...
def der_single(show, ref_file, uem_file, hyp_file):
    ...
    return der, time
    #print(f"{show}" +"\tDER: %6.2f%%  Time: %.2fs" % (der, time))

    #mapf = open(sys.argv[4], 'w')
    #for s in map:
    #    print("%s %s" % (s, map[s]), file=mapf)

def der_single_averaged(hyp_dir, lifelong_data, mdtm_dir, uem_dir,json_name="der_dic.json"):
    ...
    for idx, (show, file_info, uem, ref, filename) in enumerate(lifelong_data):
        ...
        der, time = der_single(show, ref_file, uem_file, hyp_file)
        print(f"{show}" +"\tDER:\t%6.2f%%\tTime:\t%.2fs" % (der, time))
        ref = load_mdtm(ref_file)
        hyp = load_mdtm(hyp_file)
#         import pdb
#         pdb.set_trace()
        der_dic[show]=[der, time,len(set(ref.speaker)),len(set(hyp.speaker))]
        
        total_time += time
        total_error += der * time / 100.

#     import json
#     with open(json_name+"_"+str(lifelong_data.number_of_files)+".json", 'w') as fp:
#         json.dump(der_dic, fp)
    
    return 100 * total_error / total_time
