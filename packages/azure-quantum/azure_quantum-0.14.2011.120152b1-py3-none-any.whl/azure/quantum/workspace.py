...
class Workspace:
    ...
    def submit_job(self, job: Job) -> Job:
        ...
        details = client.put(job.details.id, job.details, custom_headers=self._custom_headers())
        return Job(self, details)

    def cancel_job(self, job: Job) -> Job:
        # TODO: AB#1283
        client = self._create_jobs_client()
        details = client.delete(job.details.id, custom_headers=self._custom_headers())
        return Job(self, details)
