from .shared import validate_list_duplicated, validate_list_min_max, validate_region, validate_country
...
def validate_impact_assessment(impact_assessment: dict):
    return [
        ...
        validate_list_duplicated(impact_assessment, 'impacts', [
            'term.@id'
        ])
    ] if 'impacts' in impact_assessment else []) + ([
        validate_list_min_max(impact_assessment, 'emissionsResourceUse'),
        validate_list_duplicated(impact_assessment, 'emissionsResourceUse', [
            'term.@id'
        ])
