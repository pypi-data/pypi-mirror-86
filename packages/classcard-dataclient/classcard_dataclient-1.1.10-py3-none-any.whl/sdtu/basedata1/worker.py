...
def transact_student_data():
    ...
    for cut_year in range(front_years):
        ...
        page_index = 1
        while True:
            ecard_res = sdtu_requester.get_bk_student_list(page=page_index, pagesize=SDTU_PAGE_SIZE,
    ...
    for cut_year in range(front_years):
        ...
        page_index = 1
        while True:
            ecard_res = sdtu_requester.get_yj_student_list(page=page_index, pagesize=SDTU_PAGE_SIZE,
                                                           params={"NJ": str(current_year)})
            total_count = ecard_res["total"]
            current_rows = ecard_res["data"]["Rows"]
            for d in current_rows:
