...
class OpenScaleClient(OpenScaleReset):
    ...
    def _reliable_bind_mlinstance(self, credentials, ml_engine=None, is_mrm_preprod=False):
        ...
        if self._args.ml_engine_type is not MLEngineType.WML or not self._args.is_icp: # WML spaces not yet supported in OpenScale public cloud
            ...
            use_ml_credentials_in_binding = False
            api_env = ApiEnvironment()
            if 'token' in credentials:
                use_ml_credentials_in_binding = True

            if use_ml_credentials_in_binding:

                logger.log_info("Using ML Instance information in the add service provider call ...")
                if "apikey" in credentials:
                    wml_service_provider_creds = WMLCredentialsCloud(
                        name=credentials["instance_name"] if "instance_name" in credentials else None,
                        apikey=credentials["apikey"],
                        url=credentials["url"],
                        instance_id=credentials["instance_id"] if "instance_id" in credentials else None,
                        instance_crn=credentials["instance_crn"] if "instance_crn" in credentials else None
                    )
                else:
                    wml_service_provider_creds = WMLCredentialsCloud(
                        name=credentials["instance_name"] if "instance_name" in credentials else None,
                ...
                if is_mrm_preprod:
                    self._binding_id_mrm_preprod = service_provider_result.metadata.id
                else:
                    self._binding_id = service_provider_result.metadata.id
            else:
            ...
            wml_service_provider_creds = WMLCredentialsCP4D.from_dict(credentials)
            if is_mrm_preprod:
                operational_space_id = OPERATIONAL_PRE_PRODUCTION_SPACE_ID
            else:
                operational_space_id = OPERATIONAL_PRODUCTION_SPACE_ID
    ...
    def _post_drift_metrics(self, records, day):
        ...
        for i in range(len(drift_measurement)):
            ...
            # Building the patch JSON
            patch_documents = []
            for annotation_payload in drift_annotations:
                patch_document = PatchDocument(
