...
def stock_VaR_normal_standard(ticker,shares,today, \
    ...
    if printout == True:
        print("*** Value at risk based on Standard Normality Model")
        print("Holding stock :",ticker)
        print("Holding shares:",format(shares,','))
        print("Standing date :",today)
        print("Holding days  :",future_days)
        print("Confidence    : ",alpha*100,'%',sep='')
        print("Value at risk :",format(round(VaR,2),','))
        print("VaR ratio     : ",round(VaR_ratio*100,2),'%',sep='')
...
def series_VaR_normal_standard(ticker,shares,datelist, \
    ...
    result3['VaR amount']=round(result3['VaR'],2)
    result3['VaR ratio %']=round(result3['ratio']*100,2)
    result3.drop(columns=['ticker','VaR','ratio'],inplace=True)
    
    text1=ticker+": VaR Ratios for Holding "+str(future_days)+" Day(s)"
    print(text1)    
    print(result3)
    ...
    #VaR金额绘图    
    plt.plot(result3['VaR amount'],c='r',lw=4)  
    title1=ticker+": Changes of VaR Amount for Holding "+str(future_days)+" Day(s)"
    plt.title(title1)
    plt.ylabel('VaR amount')  
    plt.xticks(rotation=45)
    plt.show()    
    
    plt.plot(result3['VaR ratio %'],c='r',lw=4)  
    title2=ticker+": Changes of VaR Ratios for Holding "+str(future_days)+" Day(s)"
    plt.title(title2)
...
def compare_VaR_normal_standard(tickerlist,shares,datelist, \
    ...
    title1="Comparing VaR Amount for Holding "+str(future_days)+" Day(s)"
    plt.title(title1)
    plt.ylabel('VaR amount')  
    plt.xticks(rotation=45)
    plt.legend(loc='best')
    ...
    title2="Comparing VaR Ratio for Holding "+str(future_days)+" Day(s)"
    plt.title(title2)
    plt.ylabel('VaR ratio %')  
    plt.xticks(rotation=45)
    plt.legend(loc='best')
...
def stock_ES_normal_standard(ticker,shares,today, \
    ...
    if printout == True:
        print("*** Expected Shortfall based on Standard Normality Model")
        print("Holding stock :",ticker)
        print("Holding shares:",format(shares,','))
        print("Standing date :",today)
        print("Holding days  :",future_days)
        print("Confidence    : ",alpha*100,'%',sep='')
        print("ES            :",format(round(ES,2),','))
        print("ES ratio      : ",round(ratio*100,2),'%',sep='')    
...
def series_ES_normal_standard(ticker,shares,datelist, \
    ...
    #打印
    result3=result2.copy()
    result3['ES amount']=round(result3['ES'],2)
    result3['ES ratio %']=round(result3['ratio']*100,2)
    result3.drop(columns=['ticker','ES','ratio'],inplace=True)
    
    text1=ticker+": ES Ratios for Holding "+str(future_days)+" Day(s)"
    print(text1)    
    print(result3)
    ...
    import matplotlib.pyplot as plt
    #VaR金额绘图    
    plt.plot(result3['ES amount'],c='r',lw=4)  
    title1=ticker+": Changes of ES Amount for Holding "+str(future_days)+" Day(s)"
    plt.title(title1)
    plt.ylabel('ES amount')  
    plt.xticks(rotation=45)
    plt.show()    
    
    plt.plot(result3['ES ratio %'],c='r',lw=4)  
    title2=ticker+": Changes of ES Ratios for Holding "+str(future_days)+" Day(s)"
    plt.title(title2)
    plt.ylabel('ES ratio %')  
    plt.xticks(rotation=45)
    plt.show()
...
def compare_ES_normal_standard(tickerlist,shares,datelist, \
    ...
    title1="Comparing ES Amount for Holding "+str(future_days)+" Day(s)"
    plt.title(title1)
    plt.ylabel('ES amount')  
    plt.xticks(rotation=45)
    plt.legend(loc='best')
    ...
    title2="Comparing ES Ratio for Holding "+str(future_days)+" Day(s)"
    plt.title(title2)
    plt.ylabel('ES ratio %')  
    plt.xticks(rotation=45)
    plt.legend(loc='best')
...
def plot_rets_histogram(ticker,start,end,num_bins=20):
    ...
    #绘制正态分布曲线
    plt.plot(bins,y,'r--',label='Normal Distribution',lw=3)
    plt.ylabel('Frequency')
    plt.xlabel('Stock return')
    titletxt="Normality Check for Stock Returns: "+ticker+", "+start+" to "+end
    plt.title(titletxt)
    plt.legend(loc='best')
    plt.show()    
...
def stock_VaR_normal_modified(ticker,shares,today, \
    ...
    if printout == True:
        print("*** Value at risk based on Modified Normality Model")
        print("Holding stock :",ticker)
        print("Holding shares:",format(shares,','))
        print("Standing date :",today)
        print("Holding days  :",future_days)
...
def stock_VaR_historical_1d(ticker,shares,today,alpha=0.99, \
    ...
    if printout == True:
        print("*** Value at risk based on Historical Simulation Model")
        print("Holding stock :",ticker)
        print("Holding shares:",format(shares,','))
        print("Standing date :",today)
        future_days=1
        print("Holding days  :",future_days)
        print("Confidence    : ",alpha*100,'%',sep='')
        print("Value at risk :",format(round(VaR_1d,2),','))
        print("VaR ratio     : ",round(VaR_ratio*100,2),'%',sep='')    
...
def stock_VaR_historical_grouping(ticker,shares,today, \
    ...
    if printout == True:
        ...
        print("Holding days  :",future_days)
        print("Confidence    : ",alpha*100,'%',sep='')
        print("Value at risk :",format(round(VaR_days,2),','))
        print("VaR ratio     : ",round(VaR_ratio*100,2),'%',sep='')      
...
def stock_VaR_montecarlo(ticker,shares,today,future_days=1,alpha=0.99, \
    ...
    if printout == True:
        print("*** Value at risk based on Monte Carlo Simulation Model")
        print("Standing date :",today)
        print("Stock         :",ticker)
        print("Shares        :",format(shares,','))
        print("Position      :",format(round(position,2),','))
        print("Holding days  :",future_days)
        print("Confidence    : ",alpha*100,'%',sep='')
        print("Value at risk :",format(round(VaR_days,2),','))
        #四舍五不入
        print("VaR ratio     : ",(int(ratio*10000))/100,'%',sep='')      
...
def calc_VaR_tlcp(ticker,today,alpha=0.99, \
    ...
    if stop1==0:
        if printout == True:
            print("*** VaR Total Loss Critical Point")
            print("Stock code   :",ticker)
            print("Standing date:",today)
            print("Model used   :",model)
            print("Confidence   : ",alpha*100,'%',sep='')
            print("TLCP day(s)  : >",format(max1,','))
        return max1        
...
def series_VaR_tlcp(tickerlist,today,alpha=0.99,pastyears=1,model="montecarlo"):
    ...
    plt.xlabel('TLCP (Days)')
    plt.ylabel('Stock code')
    titlel1="*** VaR Total Loss Critical Point (TLCP) ***"
    titlel2="\n"+str(today)+", Model "+model+", Confidence "+str(alpha*100)+"%"
    titletxt=titlel1+titlel2
    plt.title(titletxt)
    plt.show() 
    
    print("*** VaR Total Loss Critical Point (TLCP, Days)")
    print("Standing date:",today)
    print("Model used   :",model)
    print("Confidence   : ",alpha*100,'%',sep='') 
    print(tlcpdf)
...
def plot_VaR_days_changes(ticker,shares,today,dayslist,alpha=0.99, \
    ...
    if printout == True:
        ...
        plt.legend(loc='best')
        titlel1="Changing Trend of VaR Ratio for Holding Stock"
        titlel2="\n"+ticker+", "+today+", Model "+model
        titletxt=titlel1+titlel2
        plt.title(titletxt)
        plt.show()
...
def compare_VaR_days_changes(tickerlist,shares,today,dayslist,alpha=0.99, \
    ...
    plt.xlabel('Holding period(days)')
    plt.legend(loc='best')
    titlel1="Comparing Trend of VaR Ratio for Holding Stock"
    titlel2="\nStock(s): "+str(tickerlist)+", "+today+", Model "+model
    titlel3="\nNote: Horizontal straight line means total loss in VaR"
...
def get_VaR_allmodels(ticker,shares,today, \
    ...
    print("*** Comparing VaR from Different Models")
    print("Standing date:",today)
    print("Stock        :",ticker)
    print("Shares       :",format(shares,','))
    print("Position     :",format(round(position,2),','))   
    ...
    print("\nVaR Amounts from Different Models:")
    print("Normal Standard       :",format(VaR1,','))
    print("Normal Modified       :",format(VaR2,','))
    print("Historical Simulation :",format(VaR3,','))
    print("Monte-Carlo Simulation:",format(VaR4,','))
    
    print("\nVaR Ratios from Different Models:")
    print("Normal Standard       :",round(ratio1*100,2),'%',sep='')
    print("Normal Modified       :",round(ratio2*100,2),'%',sep='')
    print("Historical Simulation :",round(ratio3*100,2),'%',sep='')
    print("Monte Carlo Simulation:",round(ratio4*100,2),'%',sep='')       
