...
def get_price(ticker,start,end):
    ...
    start: 样本开始日期，尽量远的日期，以便取得足够多的原始样本，mm/dd/yyyy
    end: 样本结束日期，既可以是今天日期，也可以是一个历史日期，mm/dd/yyyy
    """
    ...
    #按日期升序排序，近期的价格排在后面
    sortedprice=price.sort_index(axis=0,ascending=True)
...
def regparms(results):
    """
    功能：将现行回归结果生成数据框，包括变量名称、系数数值、t值、p值和显著性星星
    """
...
def reg_ff3_betas(ticker,start,end,scope='US',graph=True):
    ...
    if graph == True:
        ...
        import matplotlib.pyplot as plt
        print("\n",parms)
        title="FF3 Betas for "+ticker
        plt.title(title,fontsize=12,fontweight='bold')
        plt.ylabel("Beta",fontsize=12,fontweight='bold')
