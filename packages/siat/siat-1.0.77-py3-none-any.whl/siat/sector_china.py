...
def sort_pinyin(hanzi_list): 
    ...
    现有bug：对于同音字和同音词组无法区分，结果列表可能产生重复字词
    """
...
def sector_rank_china(comp="涨跌幅",indicator="新浪行业",num=10):
    ...
    print(footnote1)
    print("    板块数:",len(df),"\b, 来源：新浪财经,",today,"\b（信息为上个交易日）") 
...
def sector_detail_china(sector="new_dlhy",comp="涨跌幅",num=10):
    ...
    print("    来源：新浪财经,",today,"\b（信息为上个交易日）") 
...
def sector_position_china(ticker,sector="new_dlhy"):
    ...
    import datetime
    today = datetime.date.today()
    print("*** 板块内成分股个数:",len(df),"\b, 来源：新浪财经,",today,"\b（信息为上个交易日）")
