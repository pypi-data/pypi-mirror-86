...
def economy_trend(start,end,scope='China',factor='GDP',datatag=False,power=3,zeroline=False):
    ...
    footnote='Units: '+list(ds['units'])[0]+', '+list(ds['freq'])[0]+ \
        '\nSource: FRED|IMF|OECD/IBA/UPENN/WB reports, '+str(today)
...
def compare_economy(tickers,measures,fromdate,todate,power=0,twinx=False):
    ...
    if (security_num == 1) and (measure_num >= 2):
        ...
        footnote=measure1+': '+list(df1['units'])[0]+', '+list(df1['freq'])[0]+ \
            ...
            '\nSource: FRED|IMF|OECD reports, '+str(today)
        ...
        if measure1 == 'GNP':
            ylabeltxt='GNP/GDP%'
        titletxt=ticker1+' vs '+ticker2+'\n'+list(df1['name'])[0]
        
        footnote='Units: '+list(df1['units'])[0]+', '+list(df1['freq'])[0]+ \
            '\nSource: FRED|IMF|OECD reports, '+str(today)
...
def econ_fin_depth(fromdate,todate,scope,power=0,graph=True):
    ...
    colname='m2/gdp'
    collabel="Econ/Fin Depth"
    ylabeltxt="M2/GDP coefficient"
    titletxt=scope+': Financial Depth in Economy'
    footnote="Source: FRED/OECD/WB"
...
def compare_efd(fromdate,todate,scopelist,power=0):
    ...
    ylabeltxt="EFD coefficient"
    titletxt=scope1+' vs '+scope2+': \nFinancial Depth in Economy (EFD)'
    footnote="Note: EFD = M2 / GDP. "+ "Source: IMF/FRED"
...
def economy_security(scope,fromdate,todate,econ_factor,sec_ticker):
    ...
    ylabeltxt=''
    titletxt=scope+": Economy vs Security Market"
    footnote="Source: FRED|Yahoo Finance"
    plot_line2(df1,ticker1,colname1,label1, \
               df2,ticker2,colname2,label2, \
               ylabeltxt,titletxt,footnote, \
               power=0,twinx=True)
    ...
    titletxt=scope+": Economic Growth vs Security Market"
    plot_line2(df1,ticker1,colname1,label1, \
               df2,ticker2,colname2,label2, \
               ylabeltxt,titletxt,footnote, \
               power=0,twinx=True)
...
def plot_line(df,colname,collabel,ylabeltxt,titletxt,footnote,datatag=False, \
    ...
    if power > 0:
        try:
            ...
            plt.plot(df.index, f(df.id),"r--", label="Trend")
        except: pass
