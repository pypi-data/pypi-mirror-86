...
def stock_ret(ticker,fromdate,todate,type="Daily Ret%",datatag=False,power=4,graph=True):
    ...
    if pricedf is None:
        print("*** 错误#1(ret)，抓取证券价格信息失败：",ticker,fromdate,todate)
        return
    ...
    if not (type in colnames):
        print("*** 错误#2(ret)，支持的收益率类型：",colnames)
        return        
...
def stock_price_volatility(ticker,fromdate,todate,type="Weekly Price Volatility",datatag=False,power=4,graph=True):
    ...
    pricedf=get_price(ticker,fromdate1,todate)
    if pricedf is None:
        print("*** 错误#1(price_volatility)，抓取证券价格信息失败：",ticker,fromdate,todate)
        return
    pricedfcols=list(pricedf)    
    ...
    #检查type是否在支持的收益率列名中
    if not (type in colnames):
        print("*** 错误#2(price_volatility)，支持的价格波动风险类型：",colnames)
        return        
...
def stock_ret_volatility(ticker,fromdate,todate,type="Weekly Ret Volatility%",datatag=False,power=4,graph=True):
    ...
    if not (type in colnames):
        print("*** 错误#2(ret_volatility)，支持的收益率波动风险类型：",colnames)
        return        
...
def ret_volatility2(retdf,ticker,fromdate,todate,type="Weekly Ret Volatility%",datatag=False,power=4,graph=True):
    ...
    #检查type是否在支持的收益率列名中
    if not (type in colnames):
        print("*** 错误#2(ret_volatility2)，支持的收益率波动风险类型：",colnames)
        return        
...
def ret_lpsd(ticker,fromdate,todate,type="Weekly Ret Volatility%",datatag=False,power=4,graph=True):
    ...
    #检查type是否在支持的收益率波动指标列名中
    if not (type in colnames):
        print("*** 错误#2(ret_lpsd)，支持的收益率波动风险类型：",colnames)
        return        
...
def ret_lpsd2(retdf,ticker,fromdate,todate,type="Weekly Ret Volatility%",datatag=False,power=4,graph=True):
    ...
    #检查type是否在支持的收益率列名中
    if not (type in colnames):
        print("*** 错误#2(ret_lpsd2)，支持的收益率波动风险类型：",colnames)
        return        
...
def compare_stock(tickers,measures,fromdate,todate,twinx=False):
    ...
    if isinstance(tickers,list): 
        ...
        if security_num == 0:
            print("*** 错误1(compare_stock)，未提供证券代码！")
            return
    ...
    if isinstance(measures,list): 
        ...
        if measure_num == 0:
            print("*** 错误2(compare_stock)，未提供测度指标！")
            return
    ...
    if (security_num == 1) and (measure_num >= 2):
        ...
        print("*** 错误3(compare_stock)，未能理解比较内容！")
        return
...
def candlestick(stkcd,fromdate,todate,volume=False,style='default',mav=0):
    ...
    if not (style in stylelist):
        print("*** 错误#1(candlestick)，仅支持如下类型的图形风格：",stylelist)
        return
    ...
    if daily is None:
        print("*** 错误#2(candlestick)，抓取价格信息失败：",stkcd,fromdate,todate)
        return
    ...
    try:
        ...
        print("*** 错误#3(candlestick)：需要先安装插件mplfinance，然后重新运行！")
        return None
...
def candlestick_pro(stkcd,fromdate,todate,colorup='#00ff00',colordown='#ff00ff'):
    ...
    if daily is None:
        print("*** 错误#1(candlestick)，抓取价格信息失败：",stkcd,fromdate,todate)
        return
    ...
    try:
        ...
        print("*** 错误#2(candlestick)：需要先安装插件mplfinance，然后重新运行！")
        print("安装方法：")
        print("打开Anaconda prompt，执行命令：")
...
def candlestick_demo(stkcd,fromdate,todate,colorup='red',colordown='black',width=0.7):
    ...
    if p is None:
        print("*** 错误#1(candlestick_demo)，抓取价格信息失败：",stkcd,fromdate,todate)
        return    
    ...
    try:
        ...
        print("*** 错误#2(candlestick_demo)：需要先安装插件mplfinance，然后重新运行！")
        print("安装方法：")
        print("打开Anaconda prompt，执行命令：")
        print("pip install mplfinance")
        return None    
...
def stock_dividend(ticker,fromdate,todate):
    ...
    print("Stock:",ticker,'\b,',codetranslate(ticker))
...
def fix_mac_hanzi_plt():
    ...
    os=pltf[0:5]    
    if not (os == "macOS"):
        print("This command is only valid for MacOSX.")    
        return
