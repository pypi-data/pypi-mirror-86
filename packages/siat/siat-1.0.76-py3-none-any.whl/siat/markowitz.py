...
def get_prices_yahoo(ticker,fromdate,todate):
    ...
    **Adjusted close price adjusted for both dividends and splits. 
    ...
    if result is None:
        print("#Error(get_prices_yahoo): failed to get stock prices!")        
        return None         
    ...
    try:
        ...
        return None            
    if len(prices)==0:
        print("#Error(get_prices_yahoo): fetched empty stock data!")
        return None         
...
def portfolio_cumret(portfolio,today,pastyears=1):
    ...
    # 抓取股价
    prices=get_prices_yahoo(tickerlist,start,today)
    # 复权后收盘价
    aclose=prices['Close']    
    #print(aclose.head())
    # 计算每日收益率，并丢弃缺失值
    ...
    plt.rcParams['axes.unicode_minus'] = False  
    # 绘制portfolio随时间变化的图
    StockReturns.Portfolio.plot(label='Portfolio')
    plt.axhline(y=0,ls=":",c="red")
    plt.title("Portfolio: Daily Return Trend")
    plt.ylabel("Daily Return")
    plt.xlabel("Source: Yahoo Finance, "+str(today))
    plt.legend()
    plt.show()
    #..........................................................................
    ...
    # 计算累积的组合收益，并绘图
    name_list=["Portfolio"]
    titletxt="Portfolio: Cumulative Return Trend"
    ylabeltxt="Cumulative Return"
    xlabeltxt="Source: Yahoo Finance, "+str(today)
    ...
    titletxt="Portfolio: Cumulative Return Comparison"
    cumulative_returns_plot(StockReturns,name_list,titletxt,ylabeltxt,xlabeltxt)
...
def portfolio_corr(pf_info):
    ...
    sns.heatmap(correlation_matrix,annot=True,cmap="YlGnBu",linewidths=0.3,
            annot_kws={"size": 8})
    plt.title("Portfolio: Constituent Stock Correlation")
    plt.ylabel("Constituent Stock")
    plt.xlabel("Source: Yahoo Finance, "+str(today))
...
def portfolio_covar(pf_info):
    ...
    sns.heatmap(cov_mat_annual,annot=True,cmap="YlGnBu",linewidths=0.3,
            annot_kws={"size": 8})
    plt.title("Portfolio: Constituent Stock Covariance")
    plt.ylabel("Constituent Stock")
    plt.xlabel("Source: Yahoo Finance, "+str(today))
...
def portfolio_expectation(pf_info):
    ...
    print("\nPortfolio:")
    print("   ",tickerlist)
    print("    Portion:",sharelist)
    print("Standing date:",today)
    print("Expected return    :",round(annual_return,4))
    print("Expected volatility:",round(portfolio_volatility,4))
...
def portfolio_es(pf_info,simulation=1000):
    ...
    """
    plt.title("Portfolio: Markowitz Efficient Set")
...
def portfolio_GMV(es_info):
    ...
    plt.title("Portfolio: GMV Point")
    plt.ylabel("Expected Return")
    plt.xlabel("Expected Volatility\nGMV means Global Minimium Variance. \
               \nSource: Yahoo Finance, "+str(today))
    plt.legend()
    plt.show()
    ...
    #输出GMV投资组合构成比例
    print("\nGMV Portfolio: with Global Minimium Volatility")
    print("   ",tickerlist)
    GMV_weights_new=ratiolist_round(GMV_weights,num=3)
    print("    by",GMV_weights_new)
    print("Standing date:",today)
    print("Expected return    :",round(y,4))
    print("Expected volatility:",round(x,4))
...
def portfolio_MSR_GMV(es_info,RF=0):
    ...
    plt.title("Portfolio: Efficient Sets")
    plt.ylabel("Expected Return")
    plt.xlabel("Expected Volatility \
               \nSource: Yahoo Finance, "+str(today))     
    plt.show()
    ...
    plt.title("Portfolio: MSR and GMV Points")
    plt.ylabel("Expected Return")
    plt.xlabel("Expected Volatility \
               \nSource: Yahoo Finance, "+str(today))    
    plt.legend()
    ...
    #输出MSR投资组合构成比例
    print("\nMSR Portfolio: with Maximium Sharpe Ratio")
    print("   ",tickerlist)
    MSR_weights_new=ratiolist_round(MSR_weights,3)
    print("    by",MSR_weights_new)
    print("Standing date:",today)
    print("Expected return    :",round(MSR_y,4))
    print("Expected volatility:",round(MSR_x,4))
    ...
    print("    by",GMV_weights_new)
    print("Standing date:",today)
    print("Expected return    :",round(GMV_y,4))
    print("Expected volatility:",round(GMV_x,4))    
...
def portfolio_ef_0(stocks,fromdate,todate):
    ...
    prices=get_prices_yahoo(stocks,fromdate,todate)
...
def portfolio_ef(stocks,fromdate,todate):
    ...
    plt.title('Markowitz Efficient Frontier of Portfolio')
    plt.xlabel('Standard Deviation of portfolio (Risk)')
    plt.ylabel('Return of portfolio')
    ...
    plt.figtext(x_left,y_left-0.05,' '+str(stocks))
    plt.figtext(x_left,y_left-0.1,'Time period: '+str(fromdate)+' to '+str(todate))
    plt.plot(out_df_ef.index,out_df_ef['mean'],color='r',ls='--',lw=2,label='Efficient Frontier')
    plt.plot(out_df_ief.index,out_df_ief['mean'],color='k',ls=':',lw=2,label='Inefficient Frontier')
    plt.plot(out_std_min,out_mean_min,'g*-',markersize=16,label='Point of risk minimium')
