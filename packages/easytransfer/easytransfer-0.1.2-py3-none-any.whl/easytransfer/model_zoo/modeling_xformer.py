...
import pdb
XFORMER_PRETRAINED_MODEL_ARCHIVE_MAP = {
    'pai-xformer-base-en': "xformer/pai-xformer-base-en/model.ckpt"
}

XFORMER_PRETRAINED_CONFIG_ARCHIVE_MAP = {
    'pai-xformer-base-en': "xformer/pai-xformer-base-en/config.json"
}


class XFormerConfig(PretrainedConfig):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(XFormerConfig, self).__init__(**kwargs)
...
class XFormerEmbeddings(layers.Layer):
    """Construct the embeddings from word, position and token_type embeddings.
    """
    def __init__(self, config, **kwargs):
        super(XFormerEmbeddings, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        super(XFormerEmbeddings,self).build(input_shape)
...
class SelfAttention(layers.Layer):
    ...
    def call(self, attention_input, attention_mask, kv=None, training=False):
        ...
        if training is False:
            tf.logging.info("*************Xformer Evaluation using full attention****************")
        ...
        """
        random = tf.random.uniform([self.num_blocks, self.num_attention_heads], minval=0, maxval=self.num_blocks * self.num_attention_heads,
                                   dtype=tf.int32)
        """
        ...
        random = (identity + head_offsets) % self.num_blocks
        random = random * self.num_attention_heads + tf.range(start=0, limit=self.num_attention_heads, delta=1)

        k_head_h = tf.gather(k_head_h, random, axis=1)
        v_head_h = tf.gather(v_head_h, random, axis=1)
...
class XFormerEncoder(layers.Layer):
    def __init__(self, config, **kwargs):
        super(XFormerEncoder, self).__init__(**kwargs)
...
class XFormerBackbone(layers.Layer):
    def __init__(self, config, **kwargs):

        self.embeddings = XFormerEmbeddings(config, name="embeddings")

        self.encoder = XFormerEncoder(config, name="encoder")
        ...
        super(XFormerBackbone, self).__init__(config, **kwargs)
...
class XFormerPreTrainedModel(PreTrainedModel):
    config_class = XFormerConfig
    pretrained_model_archive_map = XFORMER_PRETRAINED_MODEL_ARCHIVE_MAP
    pretrained_config_archive_map = XFORMER_PRETRAINED_CONFIG_ARCHIVE_MAP

    def __init__(self, config, **kwargs):
        super(XFormerPreTrainedModel, self).__init__(config, **kwargs)
        self.bert = XFormerBackbone(config, name="xformer")
    ...
    def call(self, inputs,
             ...
             **kwargs):
        tf.logging.info("********** XFormerPreTrainedModel ***********")
