...
LINFORMER_PRETRAINED_MODEL_ARCHIVE_MAP = {
    'pai-linformer-base-en': "linformer/pai-linformer-base-en/model.ckpt"
}

LINFORMER_PRETRAINED_CONFIG_ARCHIVE_MAP = {
    'pai-linformer-base-en': "linformer/pai-linformer-base-en/config.json"
}
...
class LinFormerConfig(PretrainedConfig):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(LinFormerConfig, self).__init__(**kwargs)
        tf.logging.info("********** linformer projection size is {} ***********".format(factorized_size))
...
class SelfAttention(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.E_mapping = self.add_weight(
            shape=(input_shape[1], self.factorized_size),
            initializer=self.initializer,
            dtype=tf.float32,
            name='e_mapping/kernel',
        )

        self.E_mapping_bias = self.add_weight(
            shape=(self.hidden_size,),
            initializer=self.initializer,
            dtype=tf.float32,
            name='e_mapping/bias',
        ...
        self.F_mapping = self.add_weight(
            shape=(input_shape[1], self.factorized_size),
            initializer=self.initializer,
            dtype=tf.float32,
            name='f_mapping/kernel',
        )

        self.F_mapping_bias = self.add_weight(
            shape=(self.hidden_size,),
            initializer=self.initializer,
            dtype=tf.float32,
            name='f_mapping/bias',
    ...
    def _abs_attn_core(self, q_head, k_head, v_head, attn_mask, training,
                       scale):
        # multiplying two n * d matrices, which is O(n^2) in time and space complexity
        attn_score = tf.einsum('bind,bjnd->bnij', q_head, k_head)
        attn_score = tf.multiply(attn_score, scale)
        #attn_mask = tf.broadcast_to(attn_mask, [tf.shape(attn_mask)[0], tf.shape(attn_mask)[1], tf.shape(attn_score)[3]])
        #attn_mask = attn_mask[:,:,:tf.shape(attn_score)[3]]
    ...
    def call(self, attention_input, attention_mask, kv=None, training=False):
        ...
        q_head_h = tf.einsum('bnm,md->bnd', q_input, self.q_head_weight)
        q_head_h = tf.nn.bias_add(q_head_h, self.q_head_bias)

        k_head_h = tf.einsum('bnm,md->bnd', k_input, self.k_head_weight)
        k_head_h = tf.nn.bias_add(k_head_h, self.k_head_bias)

        k_head_h = tf.einsum('bnd,nk->bkd', k_head_h, self.E_mapping)
        k_head_h = tf.nn.bias_add(k_head_h, self.E_mapping_bias)

        v_head_h = tf.einsum('bnm,md->bnd', v_input, self.v_head_weight)
        v_head_h = tf.nn.bias_add(v_head_h, self.v_head_bias)

        v_head_h = tf.einsum('bnd,nk->bkd', v_head_h, self.F_mapping)
        v_head_h = tf.nn.bias_add(v_head_h, self.F_mapping_bias)

        q_head_h = tf.reshape(q_head_h, [batch_size, seq_length, self.num_attention_heads, self.attention_head_size])
        k_head_h = tf.reshape(k_head_h, [batch_size, self.factorized_size, self.num_attention_heads, self.attention_head_size])
        v_head_h = tf.reshape(v_head_h, [batch_size, self.factorized_size, self.num_attention_heads, self.attention_head_size])
...
class LinFormerEncoder(layers.Layer):
    def __init__(self, config, **kwargs):
        super(LinFormerEncoder, self).__init__(**kwargs)
...
class LinFormerBackbone(layers.Layer):
    def __init__(self, config, **kwargs):

        self.embeddings = layers.BertEmbeddings(config, name="embeddings")

        self.encoder = LinFormerEncoder(config, name="encoder")
        ...
        super(LinFormerBackbone, self).__init__(config, **kwargs)
...
class LinFormerPreTrainedModel(PreTrainedModel):
    ...
    def __init__(self, config, **kwargs):
        super(LinFormerPreTrainedModel, self).__init__(config, **kwargs)
        self.bert = LinFormerBackbone(config, name="linformer")
    ...
    def call(self, inputs,
             ...
             **kwargs):
        tf.logging.info("********** LinformerPreTrainedModel ***********")
