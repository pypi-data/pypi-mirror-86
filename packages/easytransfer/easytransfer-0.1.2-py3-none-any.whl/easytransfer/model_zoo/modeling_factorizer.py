...
FACTORIZER_PRETRAINED_MODEL_ARCHIVE_MAP = {
    'pai-factorizer-base-en': "xformer/pai-factorizer-base-en/model.ckpt"
}

FACTORIZER_PRETRAINED_CONFIG_ARCHIVE_MAP = {
    'pai-factorizer-base-en': "xformer/pai-factorizer-base-en/config.json"
}


class FactorizerConfig(PretrainedConfig):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(FactorizerConfig, self).__init__(**kwargs)
        self.vocab_size = vocab_size
        self.hidden_size = hidden_size
        self.factorized_size = factorized_size
        self.embedding_size = factorized_size
...
class FactorizerEmbeddings(layers.Layer):
    """Construct the embeddings from word, position and token_type embeddings.
    """
    def __init__(self, config, **kwargs):
        super(FactorizerEmbeddings, self).__init__(**kwargs)

        self.vocab_size = config.vocab_size
        self.factorized_size = config.factorized_size
    ...
    def build(self, input_shape):
        ...
        self.word_embeddings = self.add_weight(
            "word_embeddings",
            dtype=tf.float32,
            shape=[self.vocab_size, self.factorized_size],
        ...
        self.position_embeddings = self.add_weight(
            "position_embeddings",
            dtype=tf.float32,
            shape=[self.max_position_embeddings, self.factorized_size],
            initializer=self.initializer,
        )

        super(FactorizerEmbeddings,self).build(input_shape)
...
class SelfAttention(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.q_head_weight = self.add_weight(
            shape=(self.factorized_size, self.num_attention_heads, self.attention_head_size),
            initializer=self.initializer,
            dtype=tf.float32,
            name='query/kernel',
        )

        self.q_head_weight_mapping = self.add_weight(
            shape=(self.hidden_size, self.factorized_size),
            initializer=self.initializer,
            dtype=tf.float32,
            name='query/kernel_mapping',
        )
        ...
        self.k_head_weight_mapping = self.add_weight(
            shape=(self.hidden_size, self.factorized_size),
            initializer=self.initializer,
            dtype=tf.float32,
            name='key/kernel_mapping',
        )
        ...
        self.v_head_weight_mapping = self.add_weight(
            shape=(self.hidden_size, self.factorized_size),
            initializer=self.initializer,
            dtype=tf.float32,
            name='value/kernel_mapping',
        )
    ...
    def call(self, attention_input, attention_mask, kv=None, training=False):
        ...
        q_input = tf.einsum("BFH,HD->BFD", q_input, self.q_head_weight_mapping)
        q_head_h = tf.einsum("BFH,HND->BFND", q_input, self.q_head_weight)
        q_head_h += self.q_head_bias

        k_input = tf.einsum("BFH,HD->BFD", k_input, self.k_head_weight_mapping)
        k_head_h = tf.einsum("BFH,HND->BFND", k_input, self.k_head_weight)
        k_head_h += self.k_head_bias

        v_input = tf.einsum("BFH,HD->BFD", v_input, self.v_head_weight_mapping)
...
class dense_dropoutput_layernorm(layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        self.dense = layers.Dense(
            config.factorized_size, kernel_initializer=layers.get_initializer(config.initializer_range), name="dense"
        )
        self.dense_mapping = layers.Dense(
            config.hidden_size, kernel_initializer=layers.get_initializer(config.initializer_range), name="dense_mapping"
        )
    ...
    def call(self, inputs, training=False):
        ...
        hidden_states = self.dense_mapping(hidden_states)
...
class Attention(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.weight = self.add_weight(
            shape=(self.num_attention_heads, self.attention_head_size, self.factorized_size),
            initializer=self.initializer,
            dtype=tf.float32,
            name='query/kernel',
        )

        self.weight_mapping = self.add_weight(
            shape=(self.factorized_size, self.hidden_size),
            initializer=self.initializer,
            dtype=tf.float32,
            name='query_mapping/kernel',
        )
    ...
    def call(self, inputs, training=False):
        ...
        attention_output = tf.einsum("BFH,HD->BFD", attention_output, self.weight_mapping)
...
class EncoderBlock(layers.Layer):
    ...
    def call(self, inputs, training=False):
        ...
        intermediate_output = self.intermediate_mapping(intermediate_output)
        layer_output = self.bert_output([intermediate_output, attention_output], training=training)
        return layer_output, attention_output

class FactorizerEncoder(layers.Layer):
    def __init__(self, config, **kwargs):
        super(FactorizerEncoder, self).__init__(**kwargs)
...
class FactorizerBackbone(layers.Layer):
    def __init__(self, config, **kwargs):

        self.embeddings = FactorizerEmbeddings(config, name="embeddings")
        ...
        self.encoder = FactorizerEncoder(config, name="encoder")
        ...
        super(FactorizerBackbone, self).__init__(config, **kwargs)
...
class FactorizerPreTrainedModel(PreTrainedModel):
    config_class = FactorizerConfig
    pretrained_model_archive_map = FACTORIZER_PRETRAINED_MODEL_ARCHIVE_MAP
    pretrained_config_archive_map = FACTORIZER_PRETRAINED_CONFIG_ARCHIVE_MAP

    def __init__(self, config, **kwargs):
        super(FactorizerPreTrainedModel, self).__init__(config, **kwargs)
        self.bert = FactorizerBackbone(config, name="factorizer")
