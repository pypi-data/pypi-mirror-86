...
class SelfAttention(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.w2 = self.add_weight(
            shape=(self.hidden_size, self.reduce_length),
            initializer=self.initializer,
            dtype=tf.float32,
            name='w2/kernel',
        )
    ...
    def call(self, attention_input, attention_mask, kv=None, training=False):
        ...
        #Attn_score_1 from synthesiszer dense
        ...
        attn_score_1 = tf.nn.bias_add(tmp2, self.b2)

        # Attn_score_2 from bigbird random
        ...
        tf.logging.info("*********Synthesizer reduce length is {} ********".format(self.reduce_length))
        ...
        batch_range = tf.tile(tf.reshape(tf.range(batch_size, dtype=tf.int32),
                                         shape=[batch_size, 1, 1]), [1, self.reduce_length, 1])

        tmp = tf.slice(tf.random_shuffle(tf.range(seq_length, dtype=tf.int32)), [0], [self.reduce_length])
        random = tf.tile(tf.reshape(tmp, shape=[1, self.reduce_length, 1]), [batch_size, 1, 1])
        indices = tf.concat([batch_range, random], axis=2)

        k_head_h = tf.gather_nd(k_head_h, indices)
        v_head_h = tf.gather_nd(v_head_h, indices)

        attn_score_2 = tf.matmul(q_head_h, k_head_h, transpose_b=True)
        scale = 1 / (self.attention_head_size ** 0.5)
        attn_score_1 = tf.multiply(attn_score_1, scale)
        attn_score_2 = tf.multiply(attn_score_2, scale)
        attn_score = (attn_score_1 + attn_score_2)/2
        attn_vec = self._abs_attn_core(attn_score, v_head_h, attention_mask, training, attn_score)
        return attn_vec
