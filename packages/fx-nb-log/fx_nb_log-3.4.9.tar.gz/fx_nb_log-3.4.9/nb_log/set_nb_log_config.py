...
if os.name == "nt":
    ...
    user = os.getlogin()
    user_path = tempdir[:tempdir.find(os.getlogin())] + user
else:
    user_path = '/home/' + os.environ['USER']

user_config_dir = Path(user_path) / Path('.nb_log')

if not user_config_dir.exists():
    os.makedirs(user_config_dir.absolute().as_posix())

sys.path.append(user_config_dir.absolute().as_posix())
...
def nb_print(*args, sep=' ', end='\n', file=None):
    ...
    if file == sys.stderr:
        ...
        if nb_log_config_default.DISPLAY_BACKGROUD_COLOR_IN_CONSOLE:
            sys.stdout.write(
                f'''\033[0;34m{time.strftime("%H:%M:%S")}  "{file_name}:{line}"   \033[0;30;44m{sep.join(
                    args)}\033[0m{end} \033[0m''')  # 36  93 96 94
        else:
            sys.stdout.write(
                f'''\033[0;34m{time.strftime("%H:%M:%S")}  "{file_name}:{line}"   {sep.join(
                    args)} {end} \033[0m''')  # 36  93 96 94
...
# PRINT_TIPS = False # 是否打印导入模块时的提示信息
...
# AUTO_PATCH_HANDLERS = False  # 是否自动打add_handlers和call_handlers的猴子补丁，如果打了猴子补丁同类。
...
def use_config_form_nb_log_config_module():
    ...
    try:
        ...
        if nb_log_config_default.PRINT_TIPS:
            msg = f'''在用户目录下生成了nb_log包的日志配置文件：\n "{user_config_dir / Path('nb_log_config.py')}:1" ，可按需修改一些自定义配置'''
            sys.stdout.write(f'{time.strftime("%H:%M:%S")}  "{file_name}:{line}"   {msg} \n \033[0m')
            # auto_creat_config_file_to_project_root_path()
            auto_creat_config_file_to_user_path()
...
def auto_creat_config_file_to_user_path():
    # print(Path(sys.path[1]).as_posix())
    # print((Path(__file__).parent.parent).absolute().as_posix())
    """
    :return:
    ...
    user_config_path = Path(user_config_dir) / Path('nb_log_config.py')
    with user_config_path.open(mode='w', encoding='utf8') as f:
        f.write(config_file_content)
