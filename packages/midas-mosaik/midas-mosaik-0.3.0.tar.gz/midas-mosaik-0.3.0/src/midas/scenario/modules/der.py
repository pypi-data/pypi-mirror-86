        ...
        for bus, entities in self.sim_params["mapping"].items():
            for name, _ in entities:
                ...
                self.connect_entities(
                    weather_entity, self.scenario[mod_key], info[2]
                )
                ...
                if weather_fc_entity is not None:
                    ...
                    self.connect_entities(
                        weather_fc_entity, self.scenario[mod_key], fc_attrs
                    )
    ...
    def _connect_to_grid(self, model, info):
        ...
        for bus, entities in self.sim_params["mapping"].items():
            for name, _ in entities:
                ...
                try:
                    self.connect_entities(
                        self.scenario[mod_key], grid_entity, info[3]
                    )
                except ScenarioError:
                    LOG.warning("No grid entity for %s.")
