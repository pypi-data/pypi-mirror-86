...
SIM_CONFIG = {
    'Powergrid': {
        'python': 'midas.core:PandapowerSimulator'
    },
    "Households": {
        "python": "midas.core:SmartNordDataSimulator"
    },
    "UnitMods": {
        "python": "pysimmods.mosaik:FlexibilitySimulator"
    },
    "Weather": {
        "python": "midas.core:WeatherDataSimulator"
    },
    'Operator': {
        'python': 'midas.core:GridOperatorSimulator'
    },
    'Collector': {
...
household_params = {
    ...
    'household_mapping': {'Land-0': {'scaling': 0.15, 'eidx': 0, 'bus': 0},
...
unitmods_params = {
    ...
    "num_schedules": 10,
    'unit': 'mw',
    'pv_mapping': {'Photovoltaic-0': {'p_peak_mw': 0.051751, 'q_control': "q_set", 'bus': 1},
                   'Photovoltaic-1': {'p_peak_mw': 0.09195, 'q_control': "q_set", 'bus': 2},
...
mas_params = {
    ...
    'holon_topology': {'agent8': ['BlackstartSwitchAgent-6', 'BlackstartSwitchAgent-7', 'BlackstartSwitchAgent-8'],
    ...
    'der_mapping': {'BlackstartUnitAgent-0': 'Pysimmods-0.Photovoltaic-0',
    ...
    'load_mapping': {'BlackstartUnitAgent-4': 0,
                     'BlackstartUnitAgent-5': 1},
    'switch_mapping': {'BlackstartSwitchAgent-6': {'own_bus': 1,
                                                   'adjacent_switches': {'switch-0': {'other_bus': 2, 'access': True}},
                                                   'assigned_speaker': 'BlackstartUnitAgent-2'},
                       'BlackstartSwitchAgent-7': {'own_bus': 2,
                                                   'adjacent_switches': {'switch-0': {'other_bus': 1, 'access': False},
                                                                         'switch-1': {'other_bus': 3, 'access': True}},
                                                   'assigned_speaker': 'BlackstartUnitAgent-0'},
                       'BlackstartSwitchAgent-8': {'own_bus': 3,
...
weather = weather_sim.WeatherCurrent(**weather_params["weather_mapping"]["Weather"][0])
weather_fc = weather_sim.WeatherForecast(**weather_params["weather_mapping"]["WeatherForecast"][0])
...
for load, load_data in household_params['household_mapping'].items():
    load = household_sim.Land(scaling=load_data['scaling'], eidx=load_data['eidx'])
    load_models.append(load)

pv_models = []
for pv_id, pv_data in unitmods_params['pv_mapping'].items():
    pv_params = get_presets("PV", p_peak_mw=pv_data['p_peak_mw'], q_control=pv_data['q_control'])
    pv = unit_sim.Photovoltaic(params=pv_params[0], inits=pv_params[1])
    pv_models.append(pv)

unit_agents = []
for unit_agent in mas_params['der_mapping'].keys():
    blackstart_unit_agent = mas.BlackstartUnitAgent(check_inbox_interval=mas_params['check_inbox_interval'])
...
for switch_agent in mas_params['switch_mapping'].keys():
    blackstart_switch_agent = mas.BlackstartSwitchAgent(check_inbox_interval=mas_params['check_inbox_interval'])
...
for pv, unit_agent in zip(pv_models, unit_agents):
    ...
    world.connect(unit_agent, pv, 'schedule', time_shifted=True, initial_data={'schedule': None})
...
for switch_agent in switch_agents:
    for switch in mas_params['switch_mapping'][switch_agent.eid]['adjacent_switches'].keys():
        switch_id_components = switch.split('-')
        switch_number = int(switch_id_components[1])
        world.connect(switches[switch_number], switch_agent, ('closed', 'switch_state'))
        world.connect(switch_agent, switches[switch_number], ('switch_state', 'closed'), time_shifted=True,
