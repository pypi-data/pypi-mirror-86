...
def gen_warp_params (w, flip, rotation_range=[-10,10], scale_range=[-0.5, 0.5], tx_range=[-0.05, 0.05], ty_range=[-0.05, 0.05], rnd_state=None  ):
    ...
    mapx[1:-1,1:-1] = mapx[1:-1,1:-1] + random_normal( size=(cell_count-2, cell_count-2) )*(cell_size*0.24)
    mapy[1:-1,1:-1] = mapy[1:-1,1:-1] +random_normal( size=(cell_count-2, cell_count-2) )*(cell_size*0.24)
