...
class AdaBelief(Optimizer):
    ...
    arguments
        ...
        beta_1(0.9)     AdaBelief beta_1 value >0.0 <1.0

        beta_2(0.999)   AdaBelief beta_2 value >0.0 <1.0
...
__kernel void impl(__global float* V, __global const float* M, __global const float* G)
{{
    size_t gid = get_global_id(0);
    float s = G[gid]-M[gid];
    V[gid] = {beta_2}*V[gid] + (1.0 - {beta_2})*s*s;
}}
""")
...
""")
    ...
    def _on_step(self):
        for t in self.get_trainable_tensors():
            ...
            self._update_vs_krn.run (vs_t, ms_t, t.get_grad(), global_shape=(t.shape.size,) )
