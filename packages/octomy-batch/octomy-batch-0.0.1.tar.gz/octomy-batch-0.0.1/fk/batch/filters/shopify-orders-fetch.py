...
def put_batch_job_fetch_product(config, shop_domain, product_id):
    ...
    try:
        ...
        logger.error(f"Could not submit batch job fetch product {product_id}: {e}", exc_info=True)
    return id


def batch_filter_entrypoint(batch_item={}, config={}):
    # logger.info(f"SHOPIFY ORDERS FETCH RUNNING WITH {batch_item} ######################")
    shop_domain = batch_item.get("data", None)
    if not shop_domain:
    ...
    # sm.self_install()
    orders, orders_errors = sm.list_orders()
    # logger.warning(pprint.pformat(orders))
    if orders is None:
        return None, orders_errors
    if not orders:
        return None, "No orders"
    # logger.info("Orders:")
    for order in orders:
        # logger.info(f" + ORDER: {order}")
        order_dict_raw = order.to_dict()
