...
from asmlib.Riscv import TT_COMMENT, TT_INTEGER, TT_NEWLINE

import argparse
import json
import sys
import traceback
from typing import Any, Dict, List, no_type_check, Optional, Tuple
...
def cli_parsing(extensions: list = []) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        ...
        description='Instruction set simulator for RISC-V \'binary\' files.')
    ...
    parser.add_argument('--initial-pc', dest='initial_pc', type=int, default=0,
                        metavar='<addr>',
    ...
    parser.add_argument('--abi', dest='abi', action='store_true', default=False,
                        help='Print ABI names in simulation trace.')
...
def load_hex_file(filename: str) -> Dict[int, int]:
    lexer_rules = [
        ...
        # TokenRule(r';[^\n]*', TT_COMMENT),
        TokenRule(r'\r?\n', TT_NEWLINE),
        TokenRule(r'[0-9a-fA-F]{8}', TT_INTEGER, lambda str: int(str, 16).to_bytes(4, 'little')),
        TokenRule(r'[0-9a-fA-F]{2}', TT_INTEGER, lambda str: int(str, 16).to_bytes(1, 'little')),
    ]
    lexer_ctx = asmlib.Lexing.Context(lexer_rules, filename)
    line_parser = Match(TT_INTEGER, name='value') + \
    ...
    while it:
        ...
        for idx, byte in enumerate(res.annotations['value'].value):
            data[addr+idx] = byte
        addr += len(res.annotations['value'].value)
    return data
...
class RamWithProgramExtension:
    ...
    def trace_memory_cb(write: bool, addr: int, data: List[int]):
        if write:
            print("  RAM write: mem[{:03x}] <- {}".format(
                addr, asmlib.Simulation.format_value_list(data, 8, False)[0]))
        else:
            print("  RAM read:  mem[{:03x}] == {}".format(
                addr, asmlib.Simulation.format_value_list(data, 8, False)[0]))
    ...
    def update_simulator_devices(arbiter: asmlib.Simulation.MemoryArbiter,
        ...
        if args.relaxed:
            init_value = 0
        data = [init_value] * 0x800
        for addr, value in load_hex_file(args.input).items():
            data[addr] = value
        trace_cb = None
        if args.trace_ram:
            trace_cb = RamWithProgramExtension.trace_memory_cb
        ram = asmlib.Simulation.SimpleRAM(sim, 0, data, 8, trace_cb=trace_cb)
        arbiter.devices.insert(0, ram)
        sim.add_peripheral(ram)


def load_stdin_file(filename: str) -> List[int]:
    lexer_rules = [
        TokenRule(r'[ \t]+'),  # skip whitespacs
        TokenRule(r'\r?\n'),  # skip newlines
    ...
    while it:
        ...
        if token.value > 0xFFFFFFFF:
            raise asmlib.Assembler.AssemblerTypeError(
                "Value {0} (0x{0:x}) is not in the valid range [{1},{2}]!".format(
                    token.value, 0, 0xFFFFFFFF), lexer_ctx, token)
        data.append(token.value)
    return data


class StdIOModule(asmlib.Simulation.MemorySubsystem, asmlib.Simulation.PeripheralDevice):
    def __init__(self, simulator: Optional[asmlib.Simulation.Simulator],
        ...
        if input_file_name is not None:
            self._input = load_stdin_file(input_file_name)
    ...
    def bounds(self) -> Tuple[int, int]:
        if self._handshake:
            return (self._address, self._address+4)
    ...
    def read(self, addr: int, size: int = 1) -> List[int]:
        ...
        if len(self._input) < 1:
            raise asmlib.Simulation.SimulationError("Reading uninitialized data from stdin.",
                                                    self.simulator)
        if self._handshake:
            return self._input[0].to_bytes(4, 'little')
        return list(self._input.pop(0).to_bytes(4, 'little'))
    ...
    def save_output(self, output_file_name: str):
        with open(output_file_name, 'w') as fd:
            for value in self._output:
                print(asmlib.Simulation.format_value(value, 32).lower(), file=fd)

    def format_state(self, skip_none: bool = False) -> List[str]:
        lines = []
        if len(self._input) > 0:
            lines.append('Input:')
            lines += asmlib.Simulation.format_value_list(self._input, 32, False)
        if len(self._output) > 0:
            lines.append('Output:')
            lines += asmlib.Simulation.format_value_list(self._output, 32, False)
        return lines
...
class SimpleStdIOExtension:
    ...
    def setup_simulator_arguments(parser: argparse.ArgumentParser):
        ioopts = parser.add_argument_group(
            'IO Options',
            'Customize and configure the IO module at address 0x7FC.')
    ...
    def update_simulator_devices(arbiter: asmlib.Simulation.MemoryArbiter,
                                 sim: asmlib.Simulation.Simulator,
                                 args: argparse.Namespace):
        iomodule = StdIOModule(sim, 0x7FC, args.stdin, args.stdout)
...
def trace_callback(pc: int, encoding: int,
    ...
    line = "{}: {:}  {}\t{}".format(
        ...
        asmlib.Simulation.format_value(encoding, 32),
        inst.desc.format(inst),
        sim.state.format_registers(per_line=sim.state.reg_number, skip_none=True, abi=False))
    print(line.expandtabs(35))

def trace_callback_with_abi(pc: int, encoding: int,
                            inst: asmlib.Simulation.Instruction, sim: asmlib.Simulation.Simulator):
    line = "{}: {:}  {}\t{}".format(
        asmlib.Simulation.format_value(pc, sim.state.max_pc.bit_length()),
        asmlib.Simulation.format_value(encoding, 32),
        inst.desc.format(inst, abi=True),
        sim.state.format_registers(per_line=sim.state.reg_number, skip_none=True, abi=True))
    print(line.expandtabs(35))
...
def lib_main(extensions: list, args: argparse.Namespace) -> int:
    ...
    if args.relaxed:
        initial_value = 0
    init_state = asmlib.Simulation.ZeroR0ExecutionState(32, 32, 0x7FF, args.initial_pc,
                                                        initial_value=initial_value)
    sim = asmlib.Simulation.Simulator(init_state, isa, fetch_size=4)
    ...
    print('===============================================================================')
    print("\n".join(sim.format_state(include_instructions=False, skip_none=True)))
    ...
    if args.trace:
        if args.abi:
            trace_cb = trace_callback_with_abi
        else:
    ...
    print('===============================================================================')
    print("\n".join(sim.format_state(skip_none=True, abi=args.abi)))
