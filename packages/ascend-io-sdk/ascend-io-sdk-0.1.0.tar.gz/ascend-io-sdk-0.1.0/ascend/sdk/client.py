...
sys.path = [
    ...
    if not path.endswith("cli.runfiles") and not path.endswith("ascend/sdk")
]
...
try:
  ...
  path = os.path.normpath(
      os.path.join(os.path.dirname(ascend.sdk.version.__file__), "..", "external"))
...
setattr(ApiClient, "orig_sanitize_for_serialization",
        getattr(ApiClient, "sanitize_for_serialization"))
  ...
  def __init__(self, hostname=None, access_key=None, secret_key=None, verify_ssl=True):
    if not hostname:
      ...
      raise ValueError(f"Invalid hostname {hostname}: "
                       f"must follow format <SOMETHING>{_HOSTNAME_SUFFIX}")
