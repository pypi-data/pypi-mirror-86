...
class DTSCommms:
    ...
    def processTimeoutAndResult(self, socket, timeToWait):
        ...
        process_object = threading.Thread(target=self.getReturnPacket(socket))
                ...
                try:
                    ...
                    if data.count("</Response>") > 1:
                        ...
                        for response in data:
                            response = response[: response.lower().index('</response>') + len("</response>")]
                            xml_tree = cElementTree.fromstring(response)
                            if not self.parse_response(xml_tree):
                                logging.warning("Unknown response")
                    else:
                        data = data[: data.lower().index('</response>') + len("</response>")]
                        xml_tree = cElementTree.fromstring(data)
                        if not self.parse_response(xml_tree):
                            logging.warning("Unknown response")
                    break
    ...
    def create_request_log(self, logTime, messageType, messageText, messageSource, messageData=None, test_result=None, uId=""):
    ...
    def parse_response(self, xml_tree):
        ...
        # OMG THIS GAVE ME AN HOURS WORTH OF TROUBLE. FFCKKK
