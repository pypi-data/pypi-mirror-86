...
def importUSB(version="None"):
    if os.name == 'nt':
        ...
        if (version != "None"):
            ...
            libusb1DLLFolder = os.path.realpath(os.path.abspath(os.path.split(inspect.getfile(inspect.currentframe()))[
                                                                    0]) + "//usb_libs//" + "MS" + version + "//libusb-1.0.dll")
        ...
        # if no version was specified, uses default system version
            ...
            libusb1DLLFolder = os.path.realpath(os.path.abspath(os.path.split(inspect.getfile(inspect.currentframe()))[
                                                                    0]) + "//usb_libs//" + osVersion + "//libusb-1.0.dll")
            if libusb1DLLFolder not in sys.path:
                sys.path.insert(0, libusb1DLLFolder)
    ...
    libusb1Folder = os.path.realpath(
        os.path.abspath(os.path.split(inspect.getfile(inspect.currentframe()))[0]) + "//usb_libs")
...
def USB(SerID=''):
    ...
    for device in context.getDeviceList():
        if device.getVendorID() == QUARCH_VENDOR_ID:
            ...
            if sNo is None:
                continue
            if sNo.strip().lower().replace("qtl",
                                           "") in SerID.strip():  # comparison must look for sNo in SerID as SerID may contain<portNo>
...
class TQuarchUSB_IF:
    ...
    def SendCommand(self, command):
        ...
        try:
            ...
            if sys.version[0] == '3':
                self.lastWriteByteCount = self.deviceHandle.bulkWrite(endpoint=self.QCmdEP,
                                                                      data=(str(command + pad)).encode('utf-8'),
                                                                      timeout=self.timeout)
            else:
                self.lastWriteByteCount = self.deviceHandle.bulkWrite(endpoint=self.QCmdEP, data=command + pad,
                                                                      timeout=self.timeout)
    ...
    def clean_and_flush_stuck_usb_comms_andy(self):
        """
        ...
        while True:
            ...
            # NOTE - Sometimes we find OK but not the >\r\n, so this will loop infitely.
        ...
        if retLineStr != "":
            print("retLineStr (andy func) : " + str(retLineStr))
    ...
    def FetchCmdReplyTOut_andy(self, timeout):
        response = ''
        self.lastError = ''
        retLineStr = ''
        ...
        while True:
            ...
            # Exception can be due to device connection lost, or a temporary glitch that we can recover from
                ...
                logging.error(os.path.basename(__file__) + ".FetchCmdReplyTOut: bulkRead exception (" + str(
                    ex) + ") during response read: " + str(retLineStr))

                logging.error("response before " + response)
                logging.error("retLineStr before " + retLineStr)

                # Run a cleanup process and see if we can recover the response
                flushed_response = self.clean_and_flush_stuck_usb_comms()
                """
                if flushed response == something, let the loop continue
                else append >/r/n and let it fall out of loop to show we had no response.
               """
                if flushed_response != "":
                    # Appending the flushed response to what we (may) have originally recieved
                    response += flushed_response
                else:
                    # Path still fails
                    retLineStr = b">\r\n"

                logging.error("flushed response after " + flushed_response)
                logging.error("response after " + response)
    ...
    def clean_and_flush_stuck_usb_comms(self, original_reply=""):
        """
        USB comms can become stuck if a command is sent out too quickly after the end of the previous one
        If this is the case, the module response buffer contains data which needs flushed. This command
        triggers a buffer return and performs a full read of all data.  If  possible, it returns the
        ...
        print("Response from read that caused exception was : " + original_reply)
        ...
        for x in range(1):
            ...
            try:
                ...
                if retLineStr == "":
                    # Return nothing.
                    logging.error("Found no response from flush.")

            # Append whatever was already sent back from the reply
            if isinstance(retLineStr, str):
                retLineStr = str(original_reply) + retLineStr
            else:
                retLineStr = bytes.decode(retLineStr)
                retLineStr = str(original_reply) + retLineStr

            print("Response from bad command was : " + retLineStr)

            # Condition 1, Found cursor
            if ">" in str(retLineStr):
                # We don't resend, so if we read 2x '>' there's a good chance we got the response from first command too
                max_meep_count += str(retLineStr).count(">")
                response += str(retLineStr)
                meep_found = True
                responses_found += 1

            # condition 2, found fail response from second command
            if "fail" in str(retLineStr).lower():
                response += str(retLineStr)
                fail_found = True
                responses_found += 1

            # condition 3, found OK response from first command
            if "ok" in str(retLineStr).lower():
                response += str(retLineStr)
                responses_found += 1
                ok_found = True

            # Exit condition 1, we found all valid responses.
            if ok_found and meep_found and fail_found:
                pos = str(response).lower().rfind('fail')
                if pos != 0:
                    logging.error("Found every response from flush")
                    print("Full response below: \r\n" + str(response))
                    # Return everything before fail, discard of the rest
                    return response[0:pos]

        # Exit condition 2.
        # if responses_found > 0:
        #     if "ok" in str(response).lower():
        #         logging.error("Found OK in response")
        #     else:
        #         logging.error("Missing OK from response")
        #     if ">" in str(response).lower():
        #         logging.error("Found > in response")
        #     else:
        #         logging.error("Missing > from response")
        #     if "fail" in str(response).lower():
        #         logging.error("Found FAIL in response")
        #     else:
        #          logging.error("Missing FAIL from response")
        # else:
        #     logging.error("Found a response, but does not include what we're searching for(!)")

        if "fail" in str(response).lower():
            pos = str(response).lower().rfind('fail')
            response = response[0:pos]
    ...
    def FetchCmdReplyTOut(self, timeout):
        ...
        while True:
            ...
            # Exception can be due to device connection lost, or a temporary glitch that we can recover from
                ...
                print("Error Encoutered, response currently is : " + response)

                # Run a cleanup process and see if we can recover the response
                flushed_response = self.clean_and_flush_stuck_usb_comms(retLineStr)

                self.lastError = 'Run: Read Error'

            # Already append the original read to flushed response, dont need to add twice.
            if err_caught:
                response += self.strip_reply(flushed_response)
            else:
                response += self.strip_reply(retLineStr)
            ...
            if ">\r\n" in response:
                if response.find(">\r\n") == 0:
                    if loop_counter < 1:
                        ...
                        # Append the ok to beginning if we find it somewhere else in string.
                        response = "OK" + response.replace("OK", "").replace(" ", "").strip()
                # sometimes the response seems to include ONLY >\r\n but it isn't at the beginning
                # check ensures we loop around at least once more if this is the case
        ...
        if err_caught:
            if "ok" in str(response).lower():
                print(repr(response))
                two_am_given_up = response.find("OK")
                response = response[:two_am_given_up + 2] + "\r\n\r\n>\r\n"
                # if str(response).count("\r\n") > 1:
                #     response = response.replace(" ", "").strip() + "\r\n"
            print("Empty response counter was : " + str(empty_counter))
            print("Loop counter was : " + str(loop_counter))
            print("Sending back response: " + str(response).replace(">", ""))

        return str(response).replace(">", "")

    def strip_reply(self, retLineStr):
        if sys.version[0] == '3':
            if isinstance(retLineStr, str):
                retLineStr = retLineStr.strip('\0').strip() + '\r\n'
            else:
    ...
    def RunCommand(self, command, expectedResponse=True):  # , timeout ):
        ...
        #     time.sleep
        # M.H - increased the timer to 0.1
        if (timer() - self.last_command_time < 0.1):
            time.sleep(0.1)
