...
class BaseTest(ABC):
    ...
    def _add_quarch_command(self, command, quarch_device, expected_response="OK", retry=True):
        ...
        if not correct_response and not retry:
            print("Failed once with : " + str(repr(result)))
            if "run" in str(command).lower():
                expected_response = ["FAIL: 0x41 -Failed to change state of action", "OK"]
            return self._add_quarch_command(command, quarch_device, expected_response=expected_response, retry=False)
