...
def powerOnDrive(myQuarchDevice, outputMode=None):
    ...
    counter = 0
    while "on" not in str(power_status).lower() and counter < 50:
        if counter == 0:
            # send command once, not 50 times
            myQuarchDevice.sendCommand("run:pow up")
        if "plugged" in str(power_status).lower():
            return
        time.sleep(0.2)
        counter += 1
        power_status = myQuarchDevice.sendCommand("run:pow?")
...
class QuarchComplianceSuite:
    ...
    def attempt_restart(self, reason=None):
        if reason is None:
            logging.warning("Java connection closed, attempting to recover ")
    ...
    def activate_remote_server(self, port_number=9742):
        ...
        try:
            ...
            self.tcp_ip = '{address}'.format(
                address="127.0.0.1" if getLocalIpAddress() is None else getLocalIpAddress())
    ...
    def commandParser(self, data):
        ...
        try:
            ...
            if isinstance(err, ConnectionRefusedError):
                ...
                logging.warning("Connection Reset, restarting server : " + str(traceback.print_exc()))
            self.attempt_restart()
        except Exception as e:
            traceback.print_exc()
            logging.error("ERROR - Unexpected failure in command parser : " + str(traceback.print_exc()))
            raise
    ...
    def parse_request(self, xml_tree):
        ...
        if xml_tree.find('function'):
            ...
            print(sub_elem.find('function_dir').text)
            ...
            if sub_elem.find('function_call').text == "init_test":
                ...
                # from quarchpy.disk_test.test_test2 import HotPlugTest
    ...
    def setup_mdns_server(self):
        ...
        if zeroConfAvail:
            try:
                ...
                if platform == "win32":
                    # This is what already worked.. but syntax is bad on linux?
                    # mdns_info = ServiceInfo(type_="_http._tcp.local.",
                    #                         name="quarchCS._http._tcp.local.",
                    #                         socket.inet_aton(self.tcp_ip),
                    #                         port=self.tcp_port, 0, 0,
