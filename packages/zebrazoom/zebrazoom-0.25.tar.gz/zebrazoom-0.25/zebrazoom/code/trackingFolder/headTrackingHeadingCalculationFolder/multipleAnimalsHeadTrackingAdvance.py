...
def calculateMinDistFromOtherAnimals(animal_Id, hyperparameters, output, i, firstFrame):
  mindist   = 1000000
  for animal_Id2 in range(0, hyperparameters["nbAnimalsPerWell"]):
    ...
    if x_curFrame_animal_Id != 0 and y_curFrame_animal_Id != 0:
      if animal_Id != animal_Id2:
        x_curFrame_animal_Id2  = output[animal_Id2, i-firstFrame][0][0]
        y_curFrame_animal_Id2  = output[animal_Id2, i-firstFrame][0][1]
        dist = math.sqrt((x_curFrame_animal_Id-x_curFrame_animal_Id2)**2 + (y_curFrame_animal_Id-y_curFrame_animal_Id2)**2)
        if dist < mindist:
          mindist = dist
  return mindist

def multipleAnimalsHeadTrackingAdvance(outputHeading, output, hyperparameters, gray, i, firstFrame, thresh1, thresh3, lengthX):
  ...
  if i > firstFrame:
    for animal_Id in range(0, hyperparameters["nbAnimalsPerWell"]):
      ...
      if ((x_prevFrame_animal_Id != 0) and (y_prevFrame_animal_Id != 0) and (x_curFrame_animal_Id == 0) and (y_curFrame_animal_Id == 0) and (x_prevFrame_animal_Id > marginX) and (x_prevFrame_animal_Id < lengthX - marginX) ):
        ...
        mindistFromOtherAnimals = calculateMinDistFromOtherAnimals(animal_Id, hyperparameters, output, i, firstFrame)
        if index_min != -1 and mindistFromOtherAnimals >= 5:
          ...
          output[animal_Id, i-firstFrame][0][0] = headCoordinatesOptionsAlt[index_min][0]
          output[animal_Id, i-firstFrame][0][1] = headCoordinatesOptionsAlt[index_min][1]
          # THIS BELLOW IS NEW !!!
          if headCoordinatesOptionsAlt[index_min][2] != -1: # this alt head option is coming from a blob split in half
            for animal_Id2 in range(0, hyperparameters["nbAnimalsPerWell"]):
              ...
              if animal_Id2 != animal_Id:
                otherAnimal_x = output[animal_Id2, i-firstFrame][0][0]
                otherAnimal_y = output[animal_Id2, i-firstFrame][0][1]
                if (otherAnimal_x == alt_center_x and otherAnimal_y == alt_center_y) or (otherAnimal_x == alt_assigned_x and otherAnimal_y == alt_assigned_y):
                  ...
                  output[animal_Id2, i-firstFrame][0][0] = alt_opo_x
                  output[animal_Id2, i-firstFrame][0][1] = alt_opo_y
                break
      ...
      # for animal_Id2 in range(0, hyperparameters["nbAnimalsPerWell"]):
        ...
        # if x_curFrame_animal_Id != 0 and y_curFrame_animal_Id != 0:
          # if animal_Id != animal_Id2:
            ...
            # if dist < mindist:
              # mindist = dist
      mindist = calculateMinDistFromOtherAnimals(animal_Id, hyperparameters, output, i, firstFrame)
      if mindist < 4:
        print("removed ", output[animal_Id, i-firstFrame][0][0], output[animal_Id, i-firstFrame][0][1])
        output[animal_Id, i-firstFrame][0][0] = 0
        output[animal_Id, i-firstFrame][0][1] = 0
