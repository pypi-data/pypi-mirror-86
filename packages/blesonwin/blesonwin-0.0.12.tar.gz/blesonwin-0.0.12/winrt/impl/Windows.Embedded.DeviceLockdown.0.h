...
template <> struct abi<Windows::Embedded::DeviceLockdown::IDeviceLockdownProfileStatics>{ struct type : ::IInspectable
{
    ...
    virtual HRESULT __stdcall GetCurrentLockdownProfile(GUID* pProfileID) = 0;
    virtual HRESULT __stdcall ApplyLockdownProfileAsync(GUID profileID, ::IUnknown** ppWaitableOperation) = 0;
    virtual HRESULT __stdcall GetLockdownProfileInformation(GUID profileID, ::IUnknown** ppInformation) = 0;
};};
