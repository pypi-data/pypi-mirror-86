...
struct consume_Windows_Devices_Midi_IMidiChannelPressureMessage
{
    ...
    uint8_t Pressure() const noexcept;
};
...
struct consume_Windows_Devices_Midi_IMidiControlChangeMessage
{
    ...
    uint8_t ControlValue() const noexcept;
};
...
{
    Windows::Foundation::TimeSpan Timestamp() const noexcept;
    Windows::Storage::Streams::IBuffer RawData() const noexcept;
    Windows::Devices::Midi::MidiMessageType Type() const noexcept;
};
...
{
    Windows::Devices::Midi::IMidiMessage Message() const noexcept;
};
...
{
    uint8_t Channel() const noexcept;
    uint8_t Program() const noexcept;
};
...
{
    uint8_t FrameType() const noexcept;
    uint8_t Values() const noexcept;
};
...
template <> struct abi<Windows::Devices::Midi::IMidiMessage>{ struct type : ::IInspectable
{
    ...
    virtual HRESULT __stdcall get_Type(Windows::Devices::Midi::MidiMessageType* value) = 0;
};};
