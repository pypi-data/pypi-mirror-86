...
struct consume_Windows_Devices_Lights_ILamp
{
    ...
    float BrightnessLevel() const noexcept;
    void BrightnessLevel(float value) const noexcept;
    bool IsColorSettable() const noexcept;
...
template <> struct abi<Windows::Devices::Lights::ILamp>{ struct type : ::IInspectable
{
    ...
    virtual HRESULT __stdcall add_AvailabilityChanged(::IUnknown* handler, event_token* token) = 0;
