...
template <typename D> hstring consume_Windows_Globalization_Collation_ICharacterGrouping<D>::First() const noexcept
{
    hstring value{};
    check_terminate(WINRT_SHIM(Windows::Globalization::Collation::ICharacterGrouping)->get_First(put_abi(value)));
    return value;
}

template <typename D> hstring consume_Windows_Globalization_Collation_ICharacterGrouping<D>::Label() const noexcept
{
    hstring value{};
    check_terminate(WINRT_SHIM(Windows::Globalization::Collation::ICharacterGrouping)->get_Label(put_abi(value)));
    return value;
}
...
{
    HRESULT __stdcall Create(HSTRING language, ::IUnknown** result) noexcept final
    {
        try
        {
            typename D::abi_guard guard(this->shim());
