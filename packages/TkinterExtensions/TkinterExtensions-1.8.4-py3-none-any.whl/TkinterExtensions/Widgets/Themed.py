...
class TreeViewThemed(ttk.Treeview, BaseTkinterWidget, CommandMixin):
    ...
    def _json_tree(self, tree: ttk.Treeview, d: Union[ListItem, List[ListItem]], parent: str = ''):
        if not d: return
        if not isinstance(parent, str): return

        if isinstance(d, ListItem):
            tree.insert(parent, index=tk.END, iid=d.ID, text=d.Name)
            self._json_tree(tree, d.Children, parent=d.ID)

        elif isinstance(d, list):
            for item in d:
                if not isinstance(item, ListItem): raise TypeError(type(item), (ListItem,))
                tree.insert(parent, index=tk.END, iid=item.ID, text=item.Name)
                self._json_tree(tree, item.Children, parent=item.ID)
