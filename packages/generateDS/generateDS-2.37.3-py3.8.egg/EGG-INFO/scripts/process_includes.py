...
def make_names_unique(root,
    ...
    el_decls = root.xpath('./xs:element', namespaces=nsmap)
    ct_defs = root.xpath('./xs:complexType', namespaces=nsmap)
    st_defs = root.xpath('./xs:simpleType', namespaces=nsmap)
    for type_def in itertools.chain(el_decls, ct_defs, st_defs):
        ...
        rename_data.global_names.add(new_name)
...
def fix_top_level_simpletype_elements(root, rename_data):
    ...
    pat = './xs:element'
    elements = xpath_find(root, pat)
    # Find all the top-level simpleType definitions.
    pat = './xs:simpleType'
    #simple_types = xpath_find(root, pat)
    ...
    #   a complexType containing a simpleContent.
    for element in elements:
        el_type_name = element.attrib.get('type')
        if el_type_name:
            # Is there a top-level complexType?  If so, we're good.
            pat = './xs:complexType[@name="%s"]' % (el_type_name, )
            complex_types = xpath_find(root, pat)
            if not complex_types:
                simple_type = find_top_level_st_object(root, el_type_name)
                if simple_type is not None:
                    _, new_el_type_name = create_complex_simplecontent_item(
                        root, el_type_name, simple_type, prefix, rename_data)
                    element.attrib['type'] = new_el_type_name
                    # Hold a ref to the lxml element so that the python
                    # representation does not disappear.
                    rename_data.modified_elements.add(element)


def create_complex_simplecontent_item(
        root, type_name, simple_type, prefix, rename_data):
    """Create a complexType containing simpleContent.  Add it at top-level."""
    # fix
    ct_prefix, name1 = split_prefix(type_name)
    ct_name = unique_name(name1, rename_data)
    if ct_prefix:
        ct_qname = '{}:{}'.format(ct_prefix, ct_name)
    else:
        ct_qname = '{}'.format(ct_name)
    ...
    etree.SubElement(
        ...
        # fix
        attrib={'base': type_name},
    )
    return complextype_obj, ct_qname
