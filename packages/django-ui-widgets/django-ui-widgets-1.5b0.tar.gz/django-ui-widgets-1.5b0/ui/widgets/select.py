...
class DynamicMixin:
    ...
    def __init__(self, url='', placeholder=None, attrs=None, **kwargs):
        if placeholder:
            attrs = attrs or {}
...
class DynamicSelect(DynamicMixin, ChoiceWidget):

    class Media:
        # css = {'all': ("iri.css",)}
        js = ("js/dynamic_select.js",)

    template_name = 'forms/widgets/dynamic_select.html'
    default_attrs = ['id', 'required']

    def get_context(self, name, value, attrs):
        default_attrs = {key: attrs.pop(key) for key in self.default_attrs if key in attrs}
