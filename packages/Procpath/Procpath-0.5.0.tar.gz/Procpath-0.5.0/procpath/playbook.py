...
def read_watch(section, overrides):
    return read(section, 'watch', overrides, multivalue_args=['environment', 'query', 'command'])
...
class Playbook:
    ...
    def _is_pattern_section(self, section_name):
        return any(s in section_name for s in ['*', '?', '[', ']'])

    def get_command_sections(self, patterns: list):
        for pattern in patterns:
            for section_name in fnmatch.filter(filter(bool, self._parser), pattern):
                if (
                    self._get_command_name(section_name) in self._command_section_readers
                    and not self._is_pattern_section(section_name)
                ):
                    yield section_name
