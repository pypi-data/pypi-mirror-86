...
To test this library you may use test_rdoclient.py, which can be found in the source code (Github). Note that to run the accompanying tests the _API_KEY_1 field in test_rdoclient.py must be given an authentic value, i.e. a valid API key. The _API_KEY_2 field does not need to be changed. 
