...
class DeviceGalil(DeviceSocket):
    ...
    def _init_attributes(self):
        ...
        for i in range(8):
            self.create_attribute(self.AI_config[i], "Analog input", unit=u"V",
                                  default_type=np.float16, decimals=4, readonly=True)
            self.create_attribute(self.AO_config[i], "Analog output", unit=u"V",
    ...
    def handle_readout(self, readout_buffer):
        ...
        if output and output[0] == "?":
                    ...
                    if "programmable range" in line:
                        ...
                        if values is not None:
                            ...
                            for index in range(int(first_input), int(last_input) + 1):
                                if self.get_attribute(("Analog input", self.AI_config[index], MIN)) in (None, np.inf, -np.inf):
                                    self.set_raw_value(("Analog input", self.AI_config[index]), float(min_value), key=MIN)
                                if self.get_attribute(("Analog input", self.AI_config[index], MAX)) in (None, np.inf, -np.inf):
                                    self.set_raw_value(("Analog input", self.AI_config[index]), float(max_value), key=MAX)
                if line.startswith("analog outputs"):
                    if "programmable range" in line:
                        ...
                        if values is not None:
                            first_input, last_input, min_value, max_value = values.group(1), values.group(2), values.group(3), values.group(4)
                            for index in range(int(first_input), int(last_input) + 1):
                                if self.get_attribute(("Analog output", self.AO_config[index], MIN)) in (None, np.inf, -np.inf):
                                    self.set_raw_value(("Analog output", self.AO_config[index]), float(min_value), key=MIN)
                                if self.get_attribute(("Analog output", self.AO_config[index], MAX)) in (None, np.inf, -np.inf):
                                    self.set_raw_value(("Analog output", self.AO_config[index]), float(max_value), key=MAX)
            ...
            for index, limit_range_option in enumerate(analog_outputs_limits):
                ...
                if self.get_attribute(("Analog output", self.AO_config[index], MIN)) in (None, np.inf, -np.inf):
                    self.set_raw_value(("Analog output", self.AO_config[index]), lower_limit, key=MIN)
                if self.get_attribute(("Analog output", self.AO_config[index], MAX)) in (None, np.inf, -np.inf):
                    self.set_raw_value(("Analog output", self.AO_config[index]), upper_limit, key=MAX)
            ...
            for index, limit_range_option in enumerate(analog_inputs_limits):
                ...
                # Set both limits for analog channel
                if self.get_attribute(("Analog input", self.AI_config[index], MIN)) in (None, np.inf, -np.inf):
                    self.set_raw_value(("Analog input", self.AI_config[index]), lower_limit, key=MIN)
                if self.get_attribute(("Analog input", self.AI_config[index], MAX)) in (None, np.inf, -np.inf):
                    self.set_raw_value(("Analog input", self.AI_config[index]), upper_limit, key=MAX)
            # Set numpy data type, which is used to parse binary UDP readout
