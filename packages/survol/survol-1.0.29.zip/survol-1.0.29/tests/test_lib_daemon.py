...
class CgiScriptIOMemoryStartOnlyTest(unittest.TestCase):
    ...
    def test_events_generator_psutil_system_counters(self):
        ...
        def check_present(property_name):
            ...
            value_triples_list = list(result_snapshot.triples((None, property_node, None)))
            self.assertEqual(len(value_triples_list), 1)
            self.assertEqual(type(value_triples_list[0][2]), rdflib.Literal)
...
class CgiScriptStartThenEventsTest(unittest.TestCase):
    ...
    def test_events_generator_psutil_processes_perf(self):
        ...
        print("result_snapshot=", result_snapshot, len(result_snapshot))
        print("result_events=", result_events, len(result_events))
