...
def load_p48_filters_to_sncosmo(bands="*"):
    """ register the given bands ['g','r' or 'i'] """
    try:
        import sncosmo
    except ImportError:
        raise ImportError("You do not have sncosmo. Please install it (pip install sncosmo)")
    ...
    for bandname in bands_:
        ...
        band = sncosmo.Bandpass(df_["wavelength"].values, df_["eff_trans"].values, name="ztf:"+bandname.lower())
