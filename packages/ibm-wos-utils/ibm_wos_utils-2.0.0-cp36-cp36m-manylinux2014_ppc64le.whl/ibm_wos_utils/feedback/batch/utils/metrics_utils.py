...
class MetricsUtils():
    ...
    def mainpulate_data(self, data, model_type, label_col, prediction_col, probability_col):
        ...
        if model_type != constants.REGRESSION_MODEL_TYPE:
            ...
            if not label_col_type in constants.CLASSIFICATION_MODEL_DATA_TYPES:
                error_msg = "Label {} and Prediction {} column datatypes are not supported for classification model's"\
                            .format(label_col_type, prediction_col_type)
                raise Exception(error_msg)

            if label_col_type in ["boolean", "int", "double"]:
    ...
    def compute_quality_metrics(self, sc, data, model_type, label_col, prediction_col, probability_col):
