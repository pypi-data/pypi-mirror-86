...
_item_keys = ["id", "name", "description", "severity", "enabled", "scope", "ruleNames", "actions", "notificationChannelIds"]
_list_keys = ["id", "name", "severity", "enabled"]
...
@click.option('--rule', nargs=1, multiple=True, help="Rule name. It must be name instead of id, as the rules list view is by name, to account for multiple rules having the same name.")
...
@click.option('--severity', nargs=1, default=0, type=click.INT, help="How severe is this policy when violated. Range from 0 to 7 included.")
...
@click.pass_obj
def add(cnf, name, description, rule, action, scope, severity, notenabled, notification):
    """
...
@click.option('--sampling', nargs=1, type=click.INT, default=60000000, help="sample all policy events using sampling interval.")
@click.option('--aggregate', nargs=1, multiple=True, default=['ruleType'], help="Specifies how to aggregate events (sampling does not need to be specified, because when it's present it automatically means events will be aggregated). This field can either be a scope metric or a policyEvent field but (currently) not a mix of the two. When policy events fields are specified, only these can be used: severity, agentId, containerId, policyId, ruleType.")
...
@click.option('--event-filter', nargs=1, help="this is a SysdigMonitor-like filter (e.g. policyEvent.policyId=3). When provided, events are filtered by some of their properties. Currently the supported set of filters is policyEvent.all(which can be used just with matches, policyEvent.policyId, policyEvent.id, policyEvent.severity, policyEvent.ruleTye, policyEvent.ruleSubtype.")
@click.pass_obj
def events(cnf, event_id, duration, start, end, sampling, aggregate, scope, event_filter):
    """
    ...
    if duration:
        ...
        if event_id:
            ok, res = cnf.sdsecure.get_policy_events_id_duration(event_id, duration_sec, **kwargs)
        else:
            ok, res = cnf.sdsecure.get_policy_events_duration(duration_sec, **kwargs)
        ...
        if event_id:
            ok, res = cnf.sdsecure.get_policy_events_id_range(event_id, start_time, end_time, **kwargs)
        else:
            ok, res = cnf.sdsecure.get_policy_events_range(start_time, end_time, **kwargs)
...
@click.option('--action', nargs=1, multiple=True, help="It can be a stop, pause and/or capture action. It will replace the actions defined in the policy")
@click.option('--scope', nargs=1,
              help="Where the policy is being applied- Container, Host etc.. (example: 'container.image.repository = sysdig/agent')")
@click.option('--severity', nargs=1, type=click.INT, help="How severe is this policy when violated. Range from 0 to 7 included.")
@click.option('--notenabled', is_flag=True, help=" If the policy should not be considered")
@click.option('--notification', nargs=1, multiple=True, help="Notification channel id to subscribe to the policy.")
@click.pass_obj
...
@click.option('--default', is_flag=True, help="Get the default falco rules file. (if not set the custom falco rules will be displayed)")
@click.pass_obj
...
@click.option('--default', is_flag=True, help="Set the default falco rules file. (if not set the custom falco rules will be setted)")
@click.pass_obj
