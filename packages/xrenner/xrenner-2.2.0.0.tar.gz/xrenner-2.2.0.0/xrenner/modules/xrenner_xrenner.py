...
class Xrenner:
	...
	def process_sentence(self, tokoffset, sentence):
		...
		# Check for singleton classifier if desired
		# TODO: remove or fix
		check_singleton = False
		if check_singleton:
			import cPickle as pickle
			classifier = pickle.load(open(".." + os.sep + "models" + os.sep + self.model + os.sep + "singleton_net.p", "rb"))
			for head_id in mark_candidates_by_head:
				mark = mark_candidates_by_head[mark_candidates_by_head]
				if classifier.classify(mark):
					keys_to_pop.append(head_id)
