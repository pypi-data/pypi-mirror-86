...
class FacebookScraper:
    ...
    def get_posts(self, account: str, **kwargs) -> Iterator[Post]:
        if 'cookie_locale' in kwargs:
            print("cookie_locale:%s" % kwargs['cookie_locale'])
    ...
    def _generic_get_posts(
            self, extract_post_fn, iter_pages_fn, page_limit=DEFAULT_PAGE_LIMIT, options=None, cookie_locale=None):
        ...
        for i, page in zip(counter, iter_pages_fn()):
            ...
            for post_element in page:
                if cookie_locale is not None:
                    ...
                    self.default_headers['cookie'] = 'locale=%s;' % cookie_locale
                    self.session.headers.update(self.default_headers)
