...
class FacebookScraper:
    ...
    def get_posts(self, account: str, **kwargs) -> Iterator[Post]:
        iter_pages_fn = partial(iter_pages, account=account, request_fn=self.get, cookie_locale=kwargs['cookie_locale'])
    ...
    def get(self, url, **kwargs):
        try:
            kwargs['verify'] = False
            if os.getenv('SOCIAL_METRIC_HTTP_PROXY') is not None:
                kwargs['proxies'] = {
                    'http': os.getenv('SOCIAL_METRIC_HTTP_PROXY'),
                    'https': os.getenv('SOCIAL_METRIC_HTTPS_PROXY')
                }
            if 'cookie_locale' in kwargs:
                self.default_headers['cookie'] = 'locale=%s;' % kwargs['cookie_locale']
    ...
    , cookie_locale=None):
        print("cookie_locale:%s" % cookie_locale)
        ...
        for i, page in zip(counter, iter_pages_fn()):
            ...
            for post_element in page:
                yield extract_post_fn(post_element, options=options, request_fn=self.get, cookie_locale=cookie_locale)
