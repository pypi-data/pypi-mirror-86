...
def main_impl(argv, env_accessor=RealEnvAccessor(), printer=RealPrinter(), file_accessor=RealFileAccessor()):
  ...
  systemTestArgParser.add_argument('--reuse-cluster', dest='reuseCluster', type=str, required=False, default=None,
                             help="Optionally reuse an existing kind-cluster for system-tests. The cluster will be created if it does not yet exist.")
  ...
  def systemTest(argv):
    ...
    kubedev = Kubedev()
    return kubedev.system_test(args.config, args.app, args.reuseCluster)
