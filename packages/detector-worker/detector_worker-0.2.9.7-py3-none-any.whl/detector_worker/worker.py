...
class TaskResult:
    def __init__(self, result, error_message, stack_trace, progress, state):
        self.result = result
    ...
    def create_from_request_entity(entity):
        result, error, trace, progress, state = None, None, None, None, None
        if 'Result' in entity:
        ...
        return TaskResult(result, error, trace, progress, state)
    ...
    def to_json(self):
        ...
        if self.error_message:
            ...
            json_response['Completed'] = True
        if self.progress:
            json_response['Progress'] = self.progress
...
class Worker:
    ...
    def get_result(self, request_id):
        ...
        if "ChildTasksPartition" in entity and entity['ChildTasksPartition']:
            child_tasks_partition = entity['ChildTasksPartition']
            completed_count = 0
            total_count = 0
            for task_entity in self.table_client.query_entities(self.metadata_table_name, filter="PartitionKey eq '%s'" % child_tasks_partition):
                ...
                if 'State' in task_entity and task_entity['State'] == 'Completed':
                    completed_count += 1
            entity["Progress"] = completed_count * self.segment_limit
    ...
    def _process_blob(self, request_id, request_entity, func):
        blob_name, params, child_tasks_partition, retry_count = self._read_metadata(request_entity)
        if child_tasks_partition:
            self._poll_child_tasks(request_entity, request_id, child_tasks_partition)
            return
    ...
    def _split_blob_into_chunks(self, segment_iterator, blob_name):
        ...
        batch = []
        batch_number = 1
        for source_segment, target_segment in segment_iterator:
            batch.append([source_segment, target_segment])
            if len(batch) == self.segment_limit - 1:
                partial_blob_name = "%s/%s_%d" % (self.worker_name, blob_name, batch_number)
                out_blob = self._get_input_blob(partial_blob_name)
                serialized_result = "\n".join([json.dumps(pair) for pair in batch])
                out_blob.upload_blob(serialized_result, timeout=1200)
    ...
    def _merge_blobs(self, blob_names, blob_name):
        out_blob = self._get_output_blob("%s/%s" % (self.worker_name, blob_name))
        blob_names.append("%s/%s_0" % (self.worker_name, blob_name))
        content = []
        for partial_blob_name in sorted(blob_names, key=lambda b: int(b.split('_')[-1])):
            in_blob = self._get_output_blob(partial_blob_name)
    ...
    def _process_blob_by_segments(self, blob_name, func, params):
        ...
        for source_segment, target_segment in blobs_iterator:
    ...
    def _read_blob_by_segments(input_blob):
        ...
        for chunk in blob_data.chunks():
            ...
            for l in lines[:-1]:
                ...
                if len(l) == 0:
                    continue
                source_segment, target_language = json.loads(l)
                yield source_segment.strip(), target_language.strip()
