...
class Worker:
    def __init__(self, queue_name, metadata_table_name, worker_name, should_wake_up=False, segment_limit=200):
    ...
    def trigger_blobs(self, blob_names: List[str], data=None):
        guid = self._generate_id()
        request_ids = []
        for blob_name in blob_names:
            request_id = self._insert_blob_metadata(blob_name, data, guid)
            self._enqueue(request_id)
    ...
    def get_results(self, guid):
        entities = list(self.table_client.query_entities(self.metadata_table_name, filter="PartitionKey eq '%s'" % guid))
    ...
    def _process_blob(self, request_id, request_entity, func):
        ...
        try:
            if next_chunk:
                result, next_chunk = self._process_next_chunk(next_chunk, blob_name, func, offset, params)
            else:
                result, next_chunk = self._process_blob_by_segments(blob_name, func, params)
        except Exception as e:
            if retry_count == 3:
                self._record_failure(request_id, e)
            else:
    ...
    def _process_next_chunk(self, next_chunk, blob_name, func, offset, params):
        ...
        result = func(next_chunk, params) # Process current chunk
    ...
    def _insert_blob_metadata(self, blob_name, data=None, guid=None):
