...
class TcpPoolClient(TcpClient):
    ...
    def __init__(self, connectionString: str, appname: str, username: str, password: str, use_ssl: Optional[bool] = True, file_cache_byte_limit: Optional[int] = 33554432, pool_size: Optional[int] = 10):
        servers = []
        for serverString in connectionString.split("#"):
            serverParts = serverString.split(":")
            if len(serverParts) != 3:
                raise Exception(
                    f"Connection String invalid, server '{serverString}' needs hostname:port:weight. For example localhost:4000:1")
            servers.append({
