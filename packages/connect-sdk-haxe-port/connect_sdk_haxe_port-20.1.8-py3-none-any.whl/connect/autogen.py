...
class connect_Env(connect_Base):
    ...
    _hx_fields = []
    _hx_methods = []
    _hx_statics = ["config", "logger", "defaultQuery", "apiClient", "fulfillmentApi", "usageApi", "tierApi", "generalApi", "marketplaceApi", "subscriptionsApi", "initConfig", "loadConfig", "isConfigInitialized", "initLogger", "isLoggerInitialized", "initDefaultQuery", "isDefaultQueryInitialized", "getConfig", "getLogger", "getApiClient", "getFulfillmentApi", "getUsageApi", "getTierApi", "getGeneralApi", "getSubscriptionsApi", "getMarketplaceApi", "_reset", "_getDefaultQuery"]
    _hx_interfaces = []
    _hx_super = connect_Base
...
class connect_Flow(connect_Base):
    ...
    __slots__ = ("filterFunc", "steps", "model", "originalModelStr", "data", "abortRequested", "abortMessage", "stepAttempt", "storeRequestOnFailure", "skipRequestOnPendingMigration")
    _hx_fields = ["filterFunc", "steps", "model", "originalModelStr", "data", "abortRequested", "abortMessage", "stepAttempt", "storeRequestOnFailure", "skipRequestOnPendingMigration"]
    _hx_methods = ["setStoreRequestOnFailure", "storesRequestOnFailure", "setSkipRequestOnPendingMigration", "skipsRequestOnPendingMigration", "setup", "step", "getAssetRequest", "getListing", "getTierConfigRequest", "getUsageFile", "setData", "getData", "getDataKeys", "approveByTemplate", "approveByTile", "fail", "inquire", "pend", "_run", "getCurrentAttempt", "process", "processSetup", "loadStepDataIfStored", "getStepParam", "processStep", "prepareRequestAndOpenLogSection", "processAbortAndCloseLogSection", "abort", "logStepData", "getClassName"]
    _hx_statics = ["SKIP_MSG", "STEP_PARAM_ID", "STEP_PARAM_ID_TIER", "STEP_PARAM_ID_TIER2", "getExceptionMessage", "getFormattedRequest", "getFormattedData", "getDataTable"]
    _hx_interfaces = []
    _hx_super = connect_Base
    ...
    def process(self,model):
        ...
        if self.prepareRequestAndOpenLogSection(model):
            if self.processSetup():
                ...
                self.stepAttempt = (stepData.attempt if ((stepData.attempt is not None)) else 0)
                _g = []
    ...
    def getStepParam(self):
        ...
        if (assetRequest is not None):
            ...
            return tierConfigRequest.getParamById(connect_Flow.STEP_PARAM_ID_TIER2)
        else:
            return None

    def processStep(self,step,index,lastRequestStr,lastDataStr):
        requestStr = connect_util_Util.beautifyObject(self.model.toObject(),connect_Env.getLogger().isCompact(),(connect_Env.getLogger().getLevel() != connect_logger_Logger.LEVEL_DEBUG),connect_Env.getLogger().isBeautified())
        ...
        try:
            ...
            try:
                ...
                connect_Env.getLogger().error("Error updating conversation")
            try:
                if (self.getAssetRequest() is not None):
                    self.getAssetRequest().update(None)
                elif (self.getTierConfigRequest() is not None):
                    self.getTierConfigRequest().update(None)
            except BaseException as _g:
                connect_Env.getLogger().error("Error updating request")
            self.abort()
    ...
    def prepareRequestAndOpenLogSection(self,model):
        ...
        assetRequest = self.getAssetRequest()
        connect_Env.getLogger().setFilenameFromRequest(self.model)
        ...
        if ((((assetRequest is not None) and assetRequest.needsMigration()) and self.skipsRequestOnPendingMigration()) and ((assetRequest.type == "purchase"))):
    ...
    def processAbortAndCloseLogSection(self,index,requestStr,dataStr):
        if self.abortRequested:
            if (self.abortMessage is None):
                ...
                saveResult = (connect_storage_StepStorage.save(self.model,connect_storage_StepData(index,self.data,connect_storage_StorageType.ConnectStorage,(self.stepAttempt + 1)),self.getStepParam(),Reflect.field(self.model,"update")) if (self.storesRequestOnFailure()) else connect_storage_StorageType.OmittedStorage)
                ...
                if (tmp == 0):
                    ...
                    connect_Env.getLogger().write(connect_logger_Logger.LEVEL_INFO,"Step data will not be saved because feature is disabled.")
                else:
                    pass
    ...
    def getFormattedRequest(request,lastRequest,fmt):
        if (request != lastRequest):
            if (connect_Env.getLogger().getLevel() == connect_logger_Logger.LEVEL_DEBUG):
                ...
                requestStr = (connect_util_Util.beautifyObject(diff,connect_Env.getLogger().isCompact(),False,connect_Env.getLogger().isBeautified()) if ((diff is not None)) else request)
...
class connect_api_ConnectHelper:
    ...
    @staticmethod
    def put(resource,id,suffix,body):
        return connect_api_ConnectHelper.checkResponse(connect_api_ConnectHelper.connectSyncRequest("PUT",connect_api_ConnectHelper.parsePath(resource,id,suffix),connect_api_ConnectHelper.getHeaders(),None,None,body))
...
class connect_api_GeneralApi(connect_Base):
    ...
    def listConversations(self,filters):
        return connect_api_ConnectHelper.get(connect_api_GeneralApi.CONVERSATIONS_PATH,None,None,filters)

    def createConversation(self,data):
        return connect_api_ConnectHelper.post(connect_api_GeneralApi.CONVERSATIONS_PATH,None,None,data)

    def getConversation(self,id):
        return connect_api_ConnectHelper.get(connect_api_GeneralApi.CONVERSATIONS_PATH,id)

    def createConversationMessage(self,id,data):
        return connect_api_ConnectHelper.post(connect_api_GeneralApi.CONVERSATIONS_PATH,id,"messages",data)
...
class connect_api_SubscriptionsApi(connect_Base):
    ...
    def createBillingRequest(self,data):
...
class connect_api_UsageApi(connect_Base):
    ...
    def listUsageFiles(self,filters):
        return connect_api_ConnectHelper.get(connect_api_UsageApi.USAGE_FILES_PATH,None,None,filters)
...
class connect_logger_Logger(connect_Base):
    ...
    __slots__ = ("path", "level", "handlers", "sections", "maskedFields", "maskedParams", "regexMaskingList", "compact", "beautify", "defaultFilename")
    _hx_fields = ["path", "level", "handlers", "sections", "maskedFields", "maskedParams", "regexMaskingList", "compact", "beautify", "defaultFilename"]
    _hx_methods = ["getPath", "getLevel", "isBeautified", "isCompact", "setFilename", "setFilenameFromRequest", "getFilename", "getHandlers", "openSection", "closeSection", "writeBlock", "writeCodeBlock", "writeList", "writeTable", "write", "getMaskedFields", "getMaskedParams", "getRegExMaskingList", "log", "debug", "info", "notice", "warning", "error", "critical", "alert", "emergency", "_writeToHandler", "writeSections"]
    ...
    def setFilenameFromRequest(self,request):
...
class connect_logger_PlainLoggerFormatter(connect_Base):
    ...
    __slots__ = ("currentRequest", "currentRequestLevel")
    _hx_fields = ["currentRequest", "currentRequestLevel"]
    ...
    def formatSection(self,level,sectionLevel,text):
        ...
        self.parseCurrentRequest(sectionLevel,textStr)
    ...
    def parseCurrentRequest(self,sectionLevel,text):
        textStr = Std.string(text)
        if textStr.startswith(connect_logger_PlainLoggerFormatter.REQUEST_PREFIX):
            ...
            self.currentRequestLevel = sectionLevel
        elif (sectionLevel <= self.currentRequestLevel):
            self.currentRequest = connect_logger_PlainLoggerFormatter.NO_REQUEST
            self.currentRequestLevel = 0
    ...
    def _hx_empty_init(_hx_o):
        ...
        _hx_o.currentRequestLevel = None
...
class connect_models_SubscriptionRequest(connect_models_IdModel):
    ...
    def register(self):
        try:
            request = connect_Env.getSubscriptionsApi().createBillingRequest(self.toString())
...
class connect_models_TierConfigRequest(connect_models_IdModel):
        ...
        if ((hasTcrParams or hasTcParams) or hasConfigParams):
            def _hx_local_0(p):
                if (not python_Boot.hasField(p,"value")):
                    ...
                    setattr(p,(("_hx_" + "value") if (("value" in python_Boot.keywords)) else (("_hx_" + "value") if (((((len("value") > 2) and ((ord("value"[0]) == 95))) and ((ord("value"[1]) == 95))) and ((ord("value"[(len("value") - 1)]) != 95)))) else "value")),value)
            Lambda.iter(Reflect.field(diff,"params"),_hx_local_0)
    ...
    def approveByTile(self,text):
        try:
            tcr = connect_Env.getTierApi().approveTierConfigRequest(self.id,haxe_format_JsonPrinter.print(_hx_AnonObject({'activation_tile': text}),None,None))
...
class connect_storage_StorageType(Enum):
    ...
    _hx_constructs = ["ConnectStorage", "LocalStorage", "FailedStorage", "OmittedStorage"]
...
connect_storage_StorageType.OmittedStorage = connect_storage_StorageType("OmittedStorage", 3, ())
...
connect_Flow.STEP_PARAM_ID_TIER2 = "__sdk_processor_step_tier2"
