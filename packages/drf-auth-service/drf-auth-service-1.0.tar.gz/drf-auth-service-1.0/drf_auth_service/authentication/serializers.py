...
class ResetPasswordConfirmSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        """
        validate_token_by_type(data['token'], ActivationCode.CodeTypes.RESET_PASSWORD)
...
class EBSTokenRefreshSerializer(TokenRefreshSerializer):
    def validate(self, attrs):
        ...
        user = User.objects.filter(user=refresh.payload.get("user_id")).first()
        if not user:
        ...
        if self.context['request'].get('service'):
            UserLogs.objects.create(service=self.context['request'].service, user=user,
                                    username=user, type_log=LogsType.LOGIN_BY_CREDENTIALS)

        return super(EBSTokenRefreshSerializer, self).validate(attrs)
