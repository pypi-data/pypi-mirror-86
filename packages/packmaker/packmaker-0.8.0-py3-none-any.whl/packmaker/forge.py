...
class ForgeModLoader (ModLoader):
    ...
    def install(self, cache_location, install_location, side):
        ...
        self._install_libraries(cache_location, lib_location, self.get_libraries())
        self._install_libraries(cache_location, lib_location, self._get_installer_libraries())
    ...
    def _get_installer_jar(self):
        if self.installer_jar_filename is None:
            url = '{}net/minecraftforge/forge/{}/forge-{}-installer.jar'.format(ForgeModLoader.FORGE_REPO_URL,
                                                                                self.full_version,
                                                                                self.full_version)
            self.installer_jar_filename = self.downloader.download(url)
