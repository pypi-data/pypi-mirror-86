...
def download_objects_inner(client, bucketname, parentdir, array_of_files, is_csv,\
...
def actually_run_transformation(client, is_pandas_df, bucketname,\
        prefix_in, is_csv, transformation_string, **kwargs):
    array_of_files = []
    ...
    objects = download_objects_inner(client, bucketname, '', array_of_files, is_csv,\
                None, None, **kwargs)
...
def read_and_xform_one_object(client, bucketname,\
        prefix_in, transform_string, **kwargs):
    if (verbose == True):
...
def read_and_xform_dir_by_dir(client, bucketname, prefix_in, transform_string, **kwargs):
    if (enable_infin_ast == True):
    ...
    for parentdir in dict_of_arrays_of_files:
        ...
        namespaced_infin_transform_fnx = namespaced_function(glb[xn], glb, None, True)
        try:
            namespaced_infin_transform_fnx(temp_input_dir, temp_output_dir, parentdir, **kwargs)
        except Exception as e:
            status_str = str(e)
            info("Error executing " + xn + ", status=" + status_str)
        else:
        ...
        try:
            if (parentdir.startswith('mlflow-artifacts/')):
                words = parentdir.split('/')
                # e.g. mlflow-artifacts/jaganeds1/0/0-f95f0f7a8c2e47f6a9867cc04d0d4708/infinstor
                if (len(words) >= 5):
                    words = words[5:]
                    dest_path = 'infinstor/'
                    for word in words:
...
def read_and_xform_all_objects(client, bucketname, prefix_in, transform_string, **kwargs):
    if (enable_infin_ast == True):
        transformAst = infin_ast.extract_transform(TRANSFORM_DIR_BY_DIR, src_str=transform_string)
        transformSrc = infin_ast.get_source(transformAst)
    else:
    ...
    for parentdir in dict_of_arrays_of_files:
        temp_input_dir = os.path.join(temp_input_dir_root, parentdir)
    ...
    try:
        if (prefix_in.startswith('mlflow-artifacts/')):
            words = prefix_in.split('/')
            # e.g. mlflow-artifacts/jaganeds1/0/0-f95f0f7a8c2e47f6a9867cc04d0d4708/infinstor
            if (len(words) >= 5):
                words = words[5:]
                dest_path = 'infinstor/'
...
def run_transform_string_inline(client, bucketname, prefix_in,\
    ...
    if (look_for_transform(transform_string, TRANSFORM_RAW_PD)):
        ...
        return read_and_xform_all_objects(client, bucketname, prefix_in,\
                transform_string, **kwargs)
    else:
...
def test_infin_transform(service_name, input_data_spec, **kwargs):
    ...
    for one_line in code_str_lines:
        if (one_line.lstrip().startswith('%infinstor-dockerfile')):
            if (mode == 0):
                mode = 1
            else:
                mode = 0
    ...
    with start_run() as run:
        run_transform_string_inline(client, bucketname, prefix_in, clean_code, **kwargs)
        return run.info.run_id
