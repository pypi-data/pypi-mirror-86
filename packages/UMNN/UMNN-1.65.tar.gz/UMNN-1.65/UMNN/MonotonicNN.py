...
class MonotonicNN(nn.Module):
    ...
    def inverse(self, y, h, max=20, min=-20, nb_iter=20):
        ...
        x_max = torch.ones_like(y) * max
        x_min = torch.ones_like(y) * min
        y_max = self.forward(x_max, h)
        y_min = self.forward(x_min, h)
        for i in range(nb_iter):
            x_middle = (x_max + x_min) / 2
            y_middle = self.forward(x_middle, h)
            left = (y_middle > y).float()
            right = 1 - left
            x_max = left * x_middle + right * x_max
            x_min = right * x_middle + left * x_min
            z_max = left * y_middle + right * z_max
            z_min = right * y_middle + left * z_min
