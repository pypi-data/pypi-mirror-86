...
from typing import Iterable, List, Hashable, Tuple, Union, Collection
...
class TransientEFMSimulation:
    ...
    def _interpolate_intervals(tss, intervals, skips):
        ...
        for ss, ((ts1, vs1), (ts2, vs2)) in zip(skips, pairwise(zip(tss, intervals))):
            ...
            interpolated_vs.extend(
                np.tile(np.interp(merged_relative_ts, ts1 - ts1[0], vs1), ss) * (1 - linear_ramp) +
                np.tile(np.interp(merged_relative_ts, ts2 - ts2[0], vs2), ss) * linear_ramp
            )
    ...
    def get_interpolated_states(self, component: Component) -> Tuple[np.ndarray, np.ndarray]:
        ...
        """
        with self._modification_lock:
            for state in self.get_interval_states(component):
                return self._interpolate_intervals(
                    self.get_interval_timesteps(),
                    state,
                    self._skips,
    ...
    def plot(self, *entities: Union[Component, Branch], features: Collection[str] = 'vip'):
        """
        Plots the result.
