...
nod_color_dark = {
        ...
        "didn't get!":88,
        "didn't get":88,
        "didn't get, needs review":88,
        "what?!":88,
        "what?! needs review":88,
        "didn't get, but okay":95,
        "didn't get, so?":88,
        "so?":32,
        "okay, so?":32,
    ...
    while ch != ord('q'):
        ...
        while cc < start + 15 and jj < len(articles): 
            ...
            if "nods" in a and a["nods"][0] != "":
                nod = a["nods"][0]
                art_nod_color = find_color(nod_color_dark, a["nods"], 0) 
...
def find_color(colors, nods, fsn):
   ...
   if nod == "next":
      ...
      while nods[ii] == "next" and ii < len(nods):
          ii += 1
      nod = nods[ii] 
   ret = int(theme_menu["hl-text-color"]) 
...
def print_nod(nods, ypos, left, fsn, si, width):
    l_color = find_color(nod_color_light, nods, si)
    win = cur.newwin(8, left -1, ypos + 1, width + left)
    win.bkgd(' ', cur.color_pair(TEXT_COLOR)) # | cur.A_REVERSE)
    if nods[si] != "":
        mprint(' '*2 + nods[si], win, l_color) 
    win.refresh()

def print_sect(title, prog, ypos, left, width):
    l_color = SEL_ITEM_COLOR
    prog = int(prog)
    win = cur.newwin(2, width, ypos, left)
    win.bkgd(' ', cur.color_pair(TEXT_COLOR)) # | cur.A_REVERSE)
    mprint(title, win, l_color, attr = cur.A_BOLD, end ="") 
    p = 0 if prog== 0 else 1 if prog == 100 else 100 - prog
    prog_color = TEXT_COLOR #scale_color(p)
    add_info = " [" + str(prog) + "%] " # + f"({sect_fc+1}/{fnum})" 
    mprint(add_info, win, prog_color, attr = cur.A_BOLD)
    win.refresh()
...
def show_article(art, show_nod=""):
    ...
    while ch != ord('q'):
        ...
        d_color = find_color(nod_color_dark, nods, 0)
                       ...
                       if True:
                           ...
                           for sent in frag_sents:
                               ...
                               if fsn >= bmark and fsn <= si and not passable[fsn]:
                                   ...
                                   cur_sent = sent
                                   hlcolor = HL_COLOR
                                   l_color = find_color(nod_color_light, nods, fsn)
                                   if True: #nods[fsn] == "":
                                   ...
                                   _color = DIM_COLOR
                                   if nods[fsn] != "":
                                       _color = find_color(nod_color_dark, nods, fsn)
        ...
        if not is_section and sect_opened[sc]:
            ...
            if pos[offset] <= start_row: 
                print_sect(art["sections"][sc]["title"], art["sections"][sc]["prog"], 2, left, width)
        if jump_key == 0:
            ch = get_key(std)
        else:
            ch = jump_key
...
def load_preset(new_preset, options, folder=""):
    ...
    if menu == None and folder == "theme":
        neon ={'preset': 'neon',"sep1":"colors", 'text-color': '247', "bold-text":"False", 'back-color': '16', 'item-color': '71', 'cur-item-color': '247', 'sel-item-color': '33', 'title-color': '28', "sep2":"reading mode","dim-color":'241' ,"highlight-color":'235', "hl-text-color":'74',"inverse-highlight":"True", "bold-highlight":"True"}
        ...
        for mm in [neon, dark, light]:
           mm["save as"] = "button"
           mm["reset"] = "button"
           mm["delete"] = "button"
           mm["save and quit"] = "button"
        save_obj(neon, "neon", "theme")
