...
def test_revset(tmpdir, monkeypatch):
    ...
    ctx4 = wrapper.commit_file(
        ...
        utc_timestamp=time.mktime(datetime(2005, 5, 12).utctimetuple()),
        parent=ctx2)
    ...
    #   looks like hg 5.2 and 5.3 had a regression on these
    #   since this is after all only an example and it's run in
    #   lots of combinations in CI, we'll skip them. Of course inclusion
    #   in the core of this testcase would not.
    if hg_version[:2] not in ((5, 2), (5, 3)):
