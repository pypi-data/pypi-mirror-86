    ...
    def add(self,data2):
        ...
        with open(self.filename, "r+") as fi:
            ...
            try:
                ...
                data2['id']=int(str(uuid.uuid1().int)[:8])
                data['data'].append(data2)
                fi.seek(0)
                json.dump(data, fi) 
            ...
            try:
                ...
                for ji in data_add:
                    if list(data["data"][0].keys())==list(ji.keys())+["id"]:
                        ji['id']=int(str(uuid.uuid1().int)[:8])
                        data['data'].append(ji)
                        fi.seek(0)
                        json.dump(data, fi)
                ...
                for ji2 in data_add:
                    if list(type_data)==list(ji2.keys()):
                        ji2['id']=int(str(uuid.uuid1().int)[:8])
