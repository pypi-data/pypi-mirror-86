...
def get_deid_log_payload(walker, filepath, deid_profile):
    """Get the deid log payload for a file"""
    indexed_fields = {
        "AccessionNumber",
        "PatientBirthDate",
        "PatientID",
    ...
    with walker.open(filepath, "rb") as fileobj:
        df = DicomFile(fileobj)

        for field in indexed_fields:
            if df.get(field) is not None:
                payload["indexed_fields"][field] = str(df.get(field))

        for profile in deid_profile.file_profiles:
            if profile.name == "dicom":
                for profile_field in profile.fields:
                    for field in profile_field.list_fieldname(df.raw):
                        fieldname = str(field)
                        if fieldname not in df.raw:
                            tag = field.dicom_tag
                            data_elem = df.raw[tag]
                        else:
                            data_elem = df.raw[fieldname]
