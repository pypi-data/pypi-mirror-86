...
class Endpoint(ABC):
    ...
    def __init__(self, func: Callable, profile_columns: Optional[List] = None):
        self._func = func
    ...
    def get_input_and_output_for_profiling(
        self, full_x=False, reset_index=False
...
class TabularEndpoint(Endpoint):
    explain_input_schema: EndpointInputSchema
    KIND: str = EndpointKinds.TABULAR

    def __init__(self, func, X, y, profile_columns):
        super().__init__(func, profile_columns)
        ...
        if self.profiling_supported:
            input_value = self.input_schema.get_pandas_representation()
            if not profile_columns:
                self.profile_columns = input_value.columns
    ...
    def explain_input_names(self):
        return self.explain_input_schema.names
    ...
    def get_input_and_output_for_profiling(
        ...
        return self._drop_missing_values(reset_index=reset_index, full_x=full_x)

    def _drop_missing_values(
        self, full_x=False, reset_index=False
    ) -> Tuple[pandas.DataFrame, pandas.Series]:
        if full_x:
            x = self.input_schema.get_pandas_representation(reset_index=reset_index)
        else:
            x = self.explain_input_schema.get_pandas_representation(
                reset_index=reset_index
            )

        y = self.output_schema.get_pandas_representation(reset_index=reset_index)
...
class BinaryEndpoint(TabularEndpoint):
    ...
    def __init__(self, func, X, y, profile_columns):
        super().__init__(func, X, y, profile_columns)
    ...
    def endpoint(
        ...
        if kind == "tabular":
            endpoint = TabularEndpoint(
                func=func, X=X, y=y, profile_columns=profile_columns
            )
