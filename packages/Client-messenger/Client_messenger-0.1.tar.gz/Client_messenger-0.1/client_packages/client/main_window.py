...
from client_part.client_packages.client.main_window_conv import Ui_MainClientWindow
from client_part.client_packages.client.add_contact import AddContactDialog
from client_part.client_packages.client.del_contact import DelContactDialog
from client_part.client_packages.common.variables import SENDER, MESSAGE_TEXT
...
class ClientMainWindow(QMainWindow):
    ...
    def select_active_user(self):
        """
            Метод обработчик события двойного клика по списку
            контактов.
        """
    ...
    def add_contact(self, new_contact):
        ...
        try:
            ...
            self.messages.critical(self, 'Ошибка сервера', err.__str__())
    ...
    def send_message(self):
        ...
        try:
            ...
            self.messages.critical(self, 'Ошибка', err.__str__())
        except OSError as err:
            if err.errno:
                self.messages.critical(self, 'Ошибка', 'Потеряно соединение с сервером!')
                self.close()
    ...
    def sig_205(self):
        ...
        if self.current_chat and not self.database.check_user(
                self.current_chat):
            self.messages.warning(self, 'Сочувствую', 'К сожалению собеседник был удалён с '
                                                      'сервера.')
            self.set_disabled_input()
            self.current_chat = None
        self.clients_list_update()
