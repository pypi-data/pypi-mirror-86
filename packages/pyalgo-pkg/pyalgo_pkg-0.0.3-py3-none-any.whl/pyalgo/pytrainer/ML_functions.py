...
class ML_functions_Training(Basic_ML_functions):
    ...
    def Normal_Equation(X, Y, AddBias=True):
        ...
        ArrayType = basic_modules.default_values.ArrayType
    ...
    def forward_propagation(X, Theta, AddBias=True):
        ...
        if len(Theta.shape) >= 3:
            ...
            return X.dot(Theta.T).reshape((-1, 1))

    @staticmethod
    def cost_function(X, Y, Theta, Method='MSE', AddBias=True, Lambda=None):

        ArrayType = basic_modules.default_values.ArrayType   # variables that helps check if the inputs are in the right type
        BoolType = basic_modules.default_values.BoolType     # variables that helps check if the inputs are in the right type

        # // check that the inputs are valid \\
        if not isinstance(X, ArrayType):
            raise TypeError("X needs to be an array type")

        if Method not in basic_modules.pytrainer_default_values. \
                ML_functions_default_values.cost_function_Methods:
        ...
        J = self.forward_propagation(X, Theta, AddBias=AddBias) - Y
        if Method == 'MSE':  J *= J
