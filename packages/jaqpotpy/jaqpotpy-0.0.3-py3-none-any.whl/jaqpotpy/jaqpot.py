...
class Jaqpot:
    ...
    def login(self, username, password):
        ...
        """
        try:
            au_req = jaqlogin.authenticate_sync(self.base_url, username, password, self.log)
    ...
    def predict(self, df=None, id=None, modelId=None):
        ...
        #     feats.append(feat)
        features = model['additionalInfo']['independentFeatures']
        for key, value in features.items():
            featar = key.split("/")
            meta = {'titles': [value]}
            feature = {'meta': meta, '_id': featar[len(featar)-1], }
