...
def doKeepAliveJob(path: str, cmd: str, task_name: str):
    ...
    :return:
    """
    if cmd.startswith('python ') or cmd.startswith('uvicorn '):
        index = cmd.find(' ')+1
        task_name = cmd[index:]
        process_detail = execCmd(
            f'wmic process where caption="python.exe" get commandline,processid|find "{task_name}"')
    elif cmd.endswith('.exe'):
        process_detail = execCmd(f'wmic process where caption="{task_name}" get commandline,processid')
    else:
        logHandler.error('无法识别程序名，请检查命令格式')
        return
...
def doCronJob(path, cmd):
    ...
    try:
        ...
        cmd = cmd.strip()
        # if cmd.startswith('python') or cmd.startswith('uvicorn') or cmd.endswith('.bat'):
        #     os.system(f'start {cmd}')
        # elif
        if cmd.lower().startswith('taskkill'):
            cmd_param = cmd.split(' ')
            if len(cmd_param) != 2:
                if len(path) != 0:
