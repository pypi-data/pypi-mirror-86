...
def get_bars(freq):
    ...
    try:
        bars = data_backend.get_price(order_book_id=order_book_id, start=start_date, end=current_date, freq=freq)
    except KeyError:
        return np.array([])
    ...
    # remove by mapicccy, cryptocurrency doesnot need params up to eight. considering this later
