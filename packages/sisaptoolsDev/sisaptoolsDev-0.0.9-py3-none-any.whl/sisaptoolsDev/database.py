...
def get_selects(sql):
    ...
    sqlSelect = sqlSelect.replace(" ", "").replace(
        "distinct", "").lower().split(",")
    return sqlSelect
...
class Database(object):
        ...
        if self.engine == 'my':
            ...
            sql = "select column_name from all_tab_columns{} \
                   ...
                   order by column_id".format(dblink_txt, owner.upper(),
                                              table.upper())
        columns = [column for column, in self.get_one(sql)]
        return columns
    ...
    def get_column_information(self, column, table, owner=None,
        ...
        conv = {
            ('my', 'date', 'my', 'create'): '{column} {done}',
            ('my', 'date', 'my', 'query'): "date_format({column}, '%Y%m%d')",
            ...
            ('my', 'varchar', 'ora', 'create'): '{column} varchar2({char})',
            ('my', 'varchar', 'ora', 'query'): column,
            ('ora', 'DATE', 'my', 'create'): '{column} date',
            ('ora', 'DATE', 'my', 'query'): "to_char({column}, 'YYYYMMDD')",
            ('ora', 'DATE', 'ora', 'create'): '{column} date',
            ('ora', 'DATE', 'ora', 'query'): column,
            ('ora', 'DATE_J', 'my', 'create'): '{column} date',
            ('ora', 'DATE_J', 'my', 'query'): "to_char(to_date({column}, 'J'), 'YYYYMMDD')",
            ('ora', 'DATE_J', 'ora', 'create'): '{column} date',
            ('ora', 'DATE_J', 'ora', 'query'): "to_date({column}, 'J')",
            ('ora', 'DATETIME', 'my', 'create'): '{column} datetime',
            ('ora', 'DATETIME', 'my', 'query'): "to_char({column}, 'YYYY-MM-DD HH24:MI:SS')",
            ('ora', 'DATETIME', 'ora', 'create'): '{column} date',
            ('ora', 'DATETIME', 'ora', 'query'): column,
            ('ora', 'TIMESTAMP(6)', 'my', 'create'): '{column} datetime',
            ('ora', 'TIMESTAMP(6)', 'my', 'query'): "to_char({column}, 'YYYY-MM-DD HH24:MI:SS')",
            ('ora', 'TIMESTAMP(6)', 'ora', 'create'): '{column} date',
            ('ora', 'TIMESTAMP(6)', 'ora', 'query'): column,
            ('ora', 'NUMBER', 'my', 'create'): '{column} double',
            ('ora', 'NUMBER', 'my', 'query'): column,
            ('ora', 'NUMBER', 'ora', 'create'): '{column} number({precision}, {scale})',
            ('ora', 'NUMBER', 'ora', 'query'): column,
            ('ora', 'VARCHAR2', 'my', 'create'): "{column} varchar({length})",
            ('ora', 'VARCHAR2', 'my', 'query'): 'ltrim(rtrim({column}))',
            ('ora', 'VARCHAR2', 'ora', 'create'): '{column} varchar2({length})',
            ('ora', 'VARCHAR2', 'ora', 'query'): column,
            ('ora', 'CHAR', 'my', 'create'): "{column} varchar({length})",
            ('ora', 'CHAR', 'my', 'query'): 'ltrim(rtrim({column}))',
            ('ora', 'CHAR', 'ora', 'create'): '{column} char({length})',
