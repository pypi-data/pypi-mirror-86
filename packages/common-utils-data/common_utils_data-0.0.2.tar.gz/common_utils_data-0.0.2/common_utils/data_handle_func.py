...
from common_utils.excel_functions import write_format_columns, refresh_excel_calculations
    ...
    if dtype_dict:
        ...
        for dtype_column, dtype_type in dtype_dict.items():
            ...
            if dtype_type_temp == '' or dtype_type_temp == '默认' or dtype_type_temp == 'default':
                ...
                #如果是日期，需要转成固定的2020/09/09的格式 提供给技术提供的java补录工具读取
                if output and dtype_type_temp == 'str' and np.issubdtype(complete_header_df[dtype_column].dtype, np.datetime64):
                    ...
                    try:  #防止有空值concat后全是float转str 其他非空的int也会带个小数点
                        complete_header_df[dtype_column] = complete_header_df[dtype_column].fillna('')\
                                .astype(dtype_type).str.rstrip('.0')
                    except :
...
@catch_and_print
def get_split_config_dict(require_file_path,split_table_df):
    ...
    for row in split_table_values:
        ...
        if split_table_column != '' and split_column.strip() != '' :

            split_file_dict = get_require_files(require_file_path,[split_table_name])
            split_file_path = split_file_dict[split_table_name] 
            temp_df = pd.read_excel(split_file_path)

            temp_df.columns = [ x.strip() if type(x) == str else x for x in temp_df.columns  ]

            if split_table_column not in temp_df.columns:
                enter_exit(f'Failed to find column "{split_table_column}" in table "{split_table_name}" ')
            else:
                split_standard_list = [ x for x in temp_df[split_table_column].tolist() if x == x and x != '' ] 
    ...
    return split_config_dict

@catch_and_print
def get_filter_condition_match(join_table_df, filter_condition, join_columns):
    #匹配的字段条件和模糊匹配(字段标准化)不同
    try:            
        if filter_condition != '' :
            join_table_df = df_query(join_table_df,filter_condition)
            join_table_df = join_table_df.loc[:,join_columns]
    except KeyError: 
        join_column_str = ','.join(join_columns)
        enter_exit(f'Failed to find column {join_column_str} in match table')
...
def replace_by_dict(string, replace_dict={}):
    if replace_dict :    
        ...
        for k,v in replace_dict.items():
            string = re.sub(k, v, string, flags=re.I)
    return string 
...
def standardize_by_similarity( not_standard_str, standard_dict, special_syn_list,
    ...
    # standard_dict = list(set([str(x).strip() for x in standard_dict]))
...
def read_xls_special(table_path):
    df_worksheet = pd.DataFrame([])
    try:
        df_worksheet = pd.read_html(table_path,header=0)
    except:
        print('')
        print(f'***** Failed to read file:{table_path} ***** ')
        print('')
...
def get_partial_not_match(complete_header_df_notna, seq,source_column, 
    ...
    column_order = ['Standization No.','Source_column', 'Standard_column', 'Content needs to standardized', 'Filter condition']
    ...
    partial_match_not_match_df = complete_header_df_notna\
            ...
            |(complete_header_df_notna[target_column_edit]==''), [source_column]] 

    if not partial_match_not_match_df.empty:
        #获取到的原始列内容
        partial_match_not_match_df.columns = ['Content needs to standardized']
...
def fillna_value_func(complete_header_df, complete_header_df_new, fillna_value_str, output_column):
    ...
    if fillna_value_str != '' :
        ...
        #如果是属于表格的字段作为填充
            ...
            fillna_value_str = fillna_value_str.lstrip('[').rstrip(']')
            #注意" 在前面整个字段可能已经被转成string格式，不能单纯通过fillna的方式填充空值,
            #而replace的方式又无法把其他字段值作为入参
            if fillna_value_str in complete_header_df_new.columns :
                complete_header_df_new[output_column] = complete_header_df_new[output_column]\
                                .replace('',float('nan')).fillna(complete_header_df_new[fillna_value_str])
            else:
                try:  #想填充的可能是来自旧complete_header_df的字段
                    complete_header_df_new[fillna_value_str] = complete_header_df[fillna_value_str]
                    #
                    complete_header_df_new[output_column] = complete_header_df_new[output_column]\
                                .replace('',float('nan')).fillna(complete_header_df_new[fillna_value_str])
                except KeyError:
                    enter_exit(f"Fillna error: Column '{fillna_value_str}' doesn't exist!")

                complete_header_df_new = complete_header_df_new.drop(fillna_value_str, axis=1)
...
def sort_value_func(complete_header_df_new, sort_column_list,sort_column_order_list):
    try:
        complete_header_df_new = complete_header_df_new.sort_values(by=sort_column_list,ascending=sort_column_order_list)
    except :
