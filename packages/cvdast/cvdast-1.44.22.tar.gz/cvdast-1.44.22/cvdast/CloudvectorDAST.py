...
class CloudvectorDAST(object):
    ...
    def create_test_data_sets(api, method, status_code="200"):
        ...
        for collection in params_collection:
            for k, v in collection.items():
                if k.startswith("$") or k=="header":
                    if "header" in str(k).lower():
                        input_params[k] = v
                        #input_params["$headers"]= list(v.keys())
                    continue
                if k not in input_params:
                    input_params[k] = {}
                    #continue
                if type(v) is list:
                    if input_params.get(k, {}).get(k, []) and len(v) > len(input_params.get(k, {}).get(k, [])):
                        processed_params.append(k)
                        if input_params:
                            new_collection = deepcopy(collection)
                            new_collection.update(new_collection.get("header", {}))
                            new_collection["$TEST_ID"] = None
                            new_collection["$comment"] = "Maximum length for List " + str(k) + " is " + str(
                                len(v)) + " with " \
                                          "values " + str(v)
                            input_params[k] = new_collection
                elif str(v).lower() in ["true", "false"] or type(v) is bool:
                    processed_params.append(k)
                    new_collection = deepcopy(collection)
                    new_collection.update(new_collection.get("header", {}))
                    new_collection["$TEST_ID"] = None
                    new_collection["$comment"] = "Boolean value picked for " + str(k) + " is " + str(True)
                    new_collection[k] = True
                    input_params[k] = new_collection
                elif type(v) is int:
                    try:
                        if input_params.get(k,{}).get(k) and v > int(input_params.get(k, {}).get(k, 0)):
                            processed_params.append(k)
                            if input_params:
                                new_collection = deepcopy(collection)
                                new_collection.update(new_collection.get("header", {}))
        ...
        for field, field_value in input_params.items():
            if field_value:
                selected_events_data.append(field_value)

        for collection in params_collection:
            for k, v in collection.items():
                if k.startswith("$") or "date" in str(k).lower() or "time" in str(k).lower() or "header" in str(k).lower():
                    if "header" in str(k).lower():
                        input_params[k] = v
                        #input_params["$headers"] = list(v.keys())
                    continue
                ...
                if type(v) is list:
                    ...
                    to_check = input_params.get(k, {}).get(k)
                    to_update = True
                    if to_check in [True,"true"]:
                        to_update = False
                    processed_params.append(k)
                    new_collection = deepcopy(collection)
                    new_collection.update(new_collection.get("header", {}))
                    new_collection["$TEST_ID"] = None
                    new_collection["$comment"] = "Boolean value picked for " + str(k) + " is " + str(to_update)
                    new_collection[k] = True
                    input_params[k] = new_collection
                elif type(v) is int:
        ...
        for field, field_value in input_params.items():
            if field_value:
                if field == "header":
                    field_value["header"] = field_value
                selected_events_data.append(field_value)
            # else:
    ...
    def generate_auto_input_params_file():
        ...
        for api, info in f1.items():
            ...
            for method, method_info in info.items():
                ...
                if method not in fourhundred_data_set[api]["400"]:
                    fourhundred_data_set[api]["400"][method]=CloudvectorDAST.create_test_data_sets(actual_api, method, "400")
                if api not in fivehundred_data_set:
                    fivehundred_data_set[api] = {"500":{}}
                if method not in fivehundred_data_set[api]["500"]:
                    fivehundred_data_set[api]["500"][method]=CloudvectorDAST.create_test_data_sets(actual_api, method, "500")

                for status in ["200", "201", "202", "204"]:
                    data_set_for_status = CloudvectorDAST.create_test_data_sets(actual_api, method, status)
                    if data_set_for_status:
                        if status not in input_params[api]:
                            input_params[api][status] = {}
                        if method not in input_params[api][status]:
                            input_params[api][status][method]=[]
                        data_set+=data_set_for_status
                    else:
                        continue
                    ...
                    for _ in data_set:
                        for k, v in _.items():
                            if v:
                                random_data_set[k] = v
                        #random_data_set.update(_)

                    random_data_set["$CV_EVENT_ID"] = "mixed"
                    ...
                    if header:
                       pass #del random_data_set["header"]
                    #     random_data_set.update(header)
                    header_already_added = False
                    for k, v in random_data_set.items():
                        if k.startswith("$") or "authorization" in str(k).lower() or "header" in str(k).lower():
                            continue

                        if k in random_data_set["header"]:
                            if not header_already_added:
                                one_data_block = deepcopy(random_data_set)
                                one_data_block["headers"] = list(header.keys())
                                one_data_block["$TEST_ID"] = None
                                one_data_block["$comment"] = "This payload will have one data block with all the mandatory headers"
                                if method not in input_params[api][status]:
                                    input_params[api][status][method] = [one_data_block]
                                else:
                                    input_params[api][status][method].append(one_data_block)
                                header_already_added = True
                            continue
                        if str(k).lower() in [str(_).lower() for _ in method_info.get("required")]:
                            ...
                            success_data_block["$TEST_ID"] = None
                            ...
                            if "header" in success_data_block:
                                del success_data_block["header"]
                            if header:
                                #del success_data_block["header"]
                                for i, j in header.items():
                                    if i != k:
                                        success_data_block[i] = j
                            ...
                            if method not in input_params[api][status]:
                                input_params[api][status][method] = [success_data_block]
                            else:
                                input_params[api][status][method].append(success_data_block)
                    ...
                    for _ in range(len(data_set)):
                        ...
                        if "$comment" not in data_set[_]:
                            data_set[_] = {}
                    data_set.remove({})

                    with open("tests/data/param_values_to_pick.json") as fobj:
                        param_values_to_pick = json.load(fobj)

                    enum_data_set = []
                    if api in param_values_to_pick:
                        if method in param_values_to_pick[api]["200"]:
                            for k, v in param_values_to_pick[api]["200"][method].items():
                                if "::" in v:
                                    len_to_process = 1
                                    v = v.split("::")
                                else:
                                    len_to_process = len(v) + 1
                                for L in range(0, len_to_process ):
                                    for _ in combinations(v, L):
                                        if not _:
                                            continue
                                        _ = list(_) #",".join(_)
                                        some_data_set = deepcopy(random.choice(input_params[api]["200"][method]))
                                        some_data_set["$TEST_ID"] = None
                                        some_data_set["$comment"] = "This is for enum field " + str(k) + " for value "+str(_)
                                        some_data_set[k] = _
                                        enum_data_set.append(some_data_set)

                    data_set += enum_data_set

                    if method not in input_params[api][status]:
                        input_params[api][status][method] = data_set
                    else:
                        input_params[api][status][method] += data_set

                    for _ in input_params[api][status][method]:
                        if "header" in _:
                           del _["header"]

                    if method not in input_params[api]["401"]:
                        rand_choice = random.choice(input_params[api][status][method])
                        rand_choice = deepcopy(rand_choice)
                        if rand_choice:
                            rand_choice["Authorization"] = "<delete>"
                            rand_choice["$comment"] = "Missing the Authorization header for triggering a 401 response"
                ...
                for api, info in fourhundred_data_set.items():
                    for status, method_info in info.items():
                        for method, info in method_info.items():
                            ...
                            for each in info:
                                if "header" in each:
                                    each["$headers"] = list(each["header"].keys())
                                    del each["header"]
                                ...
                                if error_code not in rsp_error_codes_found:
                                    ...
                                    if method not in input_params[api]["400"]:
                                        input_params[api]["400"][method] = []
                                    if each not in input_params[api]["400"][method]:
                                        input_params[api]["400"][method].append(each)
                            ...
                            for each in info:
                                ...
                                if error_code not in rsp_error_codes_found:
                                    ...
                                    if method not in input_params[api]["500"]:
                                        input_params[api]["500"][method] = []
                                    if each not in input_params[api]["500"][method]:
                                        input_params[api]["500"][method].append(each)

        headers_from_spec = {}
        with open("tests/data/headers_from_spec.json") as fobj:
            headers_from_spec = json.load(fobj)
            ...
            for status, method_info in info.items():
                for method, info in method_info.items():
                    for _ in range(len(info)):
                        if info[_]:
                            if info[_].get("header"):
                                del input_params[api][status][method][_]["header"]
                            if info[_].get("headers"):
                                input_params[api][status][method][_]["$headers"] = {}
                                for h in input_params[api][status][method][_]["headers"]:
                                    if str(h).lower() in [str(e).lower() for e in headers_from_spec[api][method]]:
                                        input_params[api][status][method][_]["$headers"][h] = input_params[api][status][method][_].get(h)
                                    if h in input_params[api][status][method][_]:
                                        del input_params[api][status][method][_][h]
                                #input_params[api][status][method][_]["$headers"] = input_params[api][status][method][_]["headers"]
                                del input_params[api][status][method][_]["headers"]
                            if info[_].get("$headers"):
                                headers = {}
                                for h in input_params[api][status][method][_]["$headers"]:
                                    if str(h).lower() in [str(e).lower() for e in headers_from_spec[api][method]]:
                                        headers[h] = input_params[api][status][method][_].get(h)
                                    if h in input_params[api][status][method][_]:
                                        del input_params[api][status][method][_][h]
                                #input_params[api][status][method][_]["$headers"] = input_params[api][status][method][_]["headers"]
                                #del input_params[api][status][method][_]["$headers"]
                                input_params[api][status][method][_]["$headers"] = headers
                            test_data_generation_summary.append(
                                str(api) + "-->" + str(method) + "--> "+str(status)+" -->" + str(info[_]["$comment"]))
                        else:
                            info[_] = "TBD"

                    if "TBD" in info:
                        input_params[api][status][method] = list(filter(("TBD").__ne__, info))
        ...
        for api, info in summary.items():
            ...
            print("-------"*15)
        print("\t\t\t\tTotal test case(s) generated: " + str(total_tests))
