...
class ATMServerError(Exception):
...
class ClientArgs(BaseSettings):
    gateway_route_info_url: str = "http://atm-api-gateway.atm.svc.cluster.local:8080/{endpoint_id}/"
...
class Client:
    ...
    Keyword Arguments:
        ...
        gateway_route_info_url (str): defaults to {default_gateway_route_info_url}
        ...
        method_whitelist (List[str]): The HTTP methods to retry on. 
            By default this includes all HTTP methods except POST because POST can result in a new insert. 
            Modify this parameter to include POST because most API's I deal with don't return an error code and perform an insert in the same call. 
    ...
    """.format(**{f"default_{k}": v for k, v in ClientArgs.__field_defaults__.items()})
    ...
    def _request(self, mode, endpoint_id, payload=None):
        ...
        try:
            ...
            if mode == "discovery":
                resp = self._sess.get(self._args.gateway_route_info_url.format(endpoint_id=endpoint_id))
            elif mode == 'job':
    ...
    def info(self, mode="job", endpoint_id=None):
        """Show the information of requests sent by client
        
        Keyword Arguments:
            mode (str): filter with mode. if None is set, retrieve all modes. defaults to 'job'
                {discovery, list, job}
    ...
    def _job_request(self, endpoint_id, payload):
        """Job requests are made in here.
        
        Exception propagates to the caller(ATMFunction)
        """
        data = self._request("job", endpoint_id, payload=payload)
        return data
...
class ATMFunction:
    ...
    def _job_request(self, payload):
        return self._client._job_request(self._endpoint_id, payload)
    ...
    def __call__(self, *args, **kwargs):
        ...
        data = self._job_request(payload)
