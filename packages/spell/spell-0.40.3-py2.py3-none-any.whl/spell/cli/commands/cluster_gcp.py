...
def gke_configure_k8s(ctx, gcp_project_name, gke_cluster_name, zone, service_account, cluster):
    ...
    try:
        ...
        # TODO(caroline): Can we just give the Spell service account blanket permissions to the GKE cluster?
        click.echo("Granting Spell permissions to the ambassador namespace...")
        rbac_yaml = generate_gke_cluster_rbac_yaml(service_account)
        with tempfile.NamedTemporaryFile(suffix=".yaml", mode="w+") as f:
            f.write(rbac_yaml)
            f.flush()
            subprocess.check_call(
                ("kubectl", "apply", "--namespace", "ambassador", "--filename", f.name)
            )
    ...
    click.echo("Giving spell service account permissions to monitoring namespace...")
    rbac_yaml = generate_gke_cluster_rbac_yaml(service_account)
    cluster_utils.kubectl_apply(rbac_yaml, "--namespace", "monitoring", kubectl_context=None)
