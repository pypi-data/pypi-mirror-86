...
_DEFAULT_INCORRECT_METABOLITES: List[str] = [
    ...
    'glycogen', 'bigg.metabolite:glycogen'
]
...
class EquilibratorGibbsEstimator(GibbsEstimatorInterface):
    ...
    def get_drg0_prime(
        ...
        """
        dfg0_prime_mean, dfg0_prime_cov_sqrt, unknowns_basis = \
            self._get_dfg0_prime(
        ...
        dfg0_prime_mean = self._fit_unknown_dfg0(
            S, dfg0_prime_mean, unknowns_basis)
        ...
        # Compute reaction energies from the formation energies.
        drg0_prime_mean = S.transpose() @ (
            dfg0_prime_mean + dfg0_prime_correction)
        drg0_prime_cov_sqrt = S.transpose() @ dfg0_prime_cov_sqrt

        return drg0_prime_mean, drg0_prime_cov_sqrt
    ...
    def _get_dfg0_prime(
        ...
        if covered_rxn_ids != []:
            ...
            _, cov_sqrt = \
                self.eq_api.standard_dg_prime_multi(covered_rxns, 'fullrank')
            basis_size = cov_sqrt.shape[1]
            dfg0_cov_sqrt[covered_rxn_ids, 0:basis_size] = Q(
                cov_sqrt, 'kJ/mol')
            ...
            unreliable_basis_ids = [i for i in range(basis_size) if np.any(
                np.abs(dfg0_cov_sqrt[:, i]) >= self.rmse_inf)
            ]
        ...
        for rxn_id in missing_rxn_ids:
            ...
            if metabolite_id in metabolite_basis_id:
                dfg0_cov_sqrt[rxn_id, metabolite_basis_id[metabolite_id]] = \
                    default_dfg0_std
            else:
    ...
    def _make_metabolite_keys(
        ...
        return [m.key if m.key not in self.incorrect_metabolites
                else '__incorrect__:' + m.key for m in metabolites]
