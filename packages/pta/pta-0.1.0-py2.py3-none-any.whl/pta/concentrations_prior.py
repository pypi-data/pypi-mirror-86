...
class ConcentrationsPrior(object):
    ...
    def load(
        ...
        if not prior_file.is_file() or not prior_file.exists():
            raise FileNotFoundError(
                f'Could not find priors file {prior_file}')
        ...
        with prior_file.open('r') as file:
            ...
            for row in reader:
                ...
                assert isinstance(distribution, LogUniformDistribution) or \
                    isinstance(distribution, LogNormalDistribution), \
                    'Concentration priors must be defined in log-scale'
                if row[0] and row[1]:
                    metabolite_id = row[0]
                    metabolite_distributions[(
                        metabolite_id, row[1])] = distribution
                elif row[1]:
    ...
    def save(self, prior_file: Union[Path, str]):
        ...
        with prior_file.open('w', newline='') as file:
            ...
            writer.writerow(['', '', distribution_to_string(
    ...
    def get(
        ...
        if compound is not None:
            if self._ccache.is_water(compound) or \
                    self._ccache.is_proton(compound):
