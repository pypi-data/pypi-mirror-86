...
class PmoProblem(object):
    ...
    def __init__(
        ...
        objective: Callable[
            [PmoProblem], cp.problems.objective.Objective] = None,
        ...
        assert self.B.to_drg is not None, 'The thermodynamic space basis ' \
        ...
        if np.all(np.logical_or(
            ...
            self._d = cp.Variable((len(self.T.reaction_ids), 1),
                                  'directions', boolean=True)

        # If no objective is specified, maximize the probability of the
        # thermodynamic variables.
        self.objective = objective or (lambda p: cp.Minimize(
    ...
    @property
    def objective(self) -> Optional[Callable[
            [PmoProblem], cp.problems.objective.Objective]]:
        """Gets the function that constructs the PMO objective."""
        return self._objective

    @objective.setter
    def objective(self, value: Optional[Callable[
            [PmoProblem], cp.problems.objective.Objective]]):
    ...
    def rebuild_for_directions(self, directions: np.array) -> PmoProblem:
        ...
        new_lb = self.F.lb[reaction_idxs, ]
        new_ub = self.F.ub[reaction_idxs, ]
        new_lb[directions > 0.5, ] = np.maximum(new_lb[directions > 0.5, ], 0)
        new_ub[directions < 0.5, ] = np.minimum(new_ub[directions < 0.5, ], 0)
    ...
    def _scale_flux_space(F: FluxSpace) -> np.array:
        ...
        if any(row_scale == 0):
            zero_row_mets = [F.metabolite_ids[i] for
                             i in np.where(row_scale == 0)[0].tolist()]
            raise Exception('One row of the stoichiometric matrix is zero, '
    ...
    def _make_big_M_coefficients(self):
        ...
        self._big_M_f = 1.01 * np.maximum(
            np.abs(self.F.lb), np.abs(self.F.ub))[self._rxn_idxs_F_to_T, :]
        ...
        if min_flux_bound < np.max(self._big_M_f) \
                / self._MAX_COEFFICIENT_RANGE * 10:
            logger.warning('The coefficient range of the flux bounds '
    ...
    def _make_constraints(self):
        ...
        self._steady_state_constraint = (
            self.F.S @ self.vs == np.zeros((self.F.n_metabolites, 1)))

        # Thermodynamic space constraints.
        CI_square = chi2.ppf(self._confidence_level, self.B.dimensionality)
        self._m_constraint = cp.atoms.quad_form(
            self.m, np.identity(self.B.dimensionality)) <= CI_square
        ...
        if self._fixed_directions:
            ...
            directions[self.F.lb[self._rxn_idxs_F_to_T, ] >= 0] = 1
