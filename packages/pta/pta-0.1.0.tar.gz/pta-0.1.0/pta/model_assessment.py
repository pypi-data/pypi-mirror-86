...
class StructuralAssessment(object):
    ...
    def summary(self):
        ...
        if len(self.forced_internal_cycles) > 0:
            print('> The following internal cycles are thermodynamically '
    ...
    def autoresolve(
        ...
        for cycle in self.forced_internal_cycles:
            if self._is_semi_disconnected_cycle(model, cycle):
                logger.info('Removing all reactions of the dead-end cycle:'
                            f' {", ".join(cycle)}.')
                model.remove_reactions(cycle, remove_orphans=True)
            else:
                logger.info('Removing bounds for all reactions in the cycle: '
                            f'{", ".join(cycle)}.')
                for rxn_id in cycle:
                    try:
    ...
    def _is_semi_disconnected_cycle(
        self,
        model: cobra.Model,
        cycle: np.array
    ) -> bool:
            ...
            for j in range(i):
                for sense in [-1, 1]:
                    with model as model:
                        ...
                        if (direction_i > 0 and rxn_i.upper_bound <= 0) or \
                            (direction_i < 0 and rxn_i.lower_bound >= 0) or \
                            (direction_j < 0 and rxn_j.upper_bound <= 0) or \
                                (direction_j > 0 and rxn_j.lower_bound >= 0):
                            continue

                        # Set the direction of the cycle for the first reaction.
                        if direction_i > 0:
                            rxn_i.lower_bound = max(
                                rxn_i.lower_bound, min_flux)
                        else:
                            rxn_i.upper_bound = min(
                                rxn_i.upper_bound, -min_flux)

                        # Set the opposite direction (with respect to the direction in
                        # the EFM) for the second reaction.
                        if direction_j < 0:
                            rxn_j.lower_bound = max(
                                rxn_j.lower_bound, min_flux)
                        else:
                            rxn_j.upper_bound = min(
                                rxn_j.upper_bound, -min_flux)
...
class QuantitativeAssessment(object):
    ...
    def __init__(
        ...
        self._is_intracellular = pd.DataFrame([
            m.compartment not in non_intracellular_compartment_ids for
            m in pmo_problem.T.metabolites])
        ...
        if status == 'optimal_inaccurate':
            logger.warning(
                'Inaccurate solution found when computing shadow prices.')

        # Assemble metabolite and reaction data in two data frames.
        self._metabolites_df = pd.DataFrame({
            'id': pmo_problem.F.metabolite_ids,
            'conc': np.exp(pmo_problem.log_c).ravel() * 1000 if
            pmo_problem.log_c is not None else np.nan,
        ...
        self._reactions_df = pd.DataFrame({
            ...
            'drg0': pmo_problem.drg0.ravel() if
            pmo_problem.drg0 is not None else np.nan,
            'drg': pmo_problem.drg.ravel() if
            pmo_problem.drg is not None else np.nan,
            ...
            'sp_drg': np.maximum(
                ...
                orthant_pmo.sign_constraints[1].dual_value).ravel()
        })
    ...
    def summary(self):
        ...
        if not anomalies_m_z.empty:
            print(f'> The following metabolites have been flagged as '
        ...
        if not anomalies_m_m.empty:
            print(f'> The following non-intracellular metabolites have been '
        ...
        anomalies_r_z = anomalies_r_z.reindex(anomalies_r_z[['z_drg', 'z_drg0']]
                                              .abs().max(axis=1).sort_values(ascending=False).index)
        if not anomalies_r_z.empty:
            print(f'> The following reactions have been flagged as anomalies '
        ...
        anomalies_r_s = self.reactions_df[
            (self.reactions_df['sp_drg'] > self.theta_s)]
        anomalies_r_s = anomalies_r_s.reindex(
            anomalies_r_s['sp_drg'].sort_values(ascending=False).index)
        if not anomalies_r_s.empty:
            print(f'> The following reactions are predicted to impose strong '
