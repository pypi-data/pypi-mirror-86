...
class BootstrappingCallback(Callback):
	...
	def make_loss(self):
		...
		elif self.bootstrapping_type == "cross_entropy":
			return cross_entropy
		else:
			assert bootstrapping_type in ["soft", "hard", "cross_entropy"]
		return 
...
class BootstrappingNeuralNetworkClassifier(object):
	"""
	"""
	def __init__(self, clf, batch_size=128, epochs=40, bootstrapping_type="hard", beta=0.95, patience=0, best_model_name="model_check_point_best_model", optimizer=None):
		"""
		:param clf:
		"""
		self.model = clf
	...
	def fit(self, X, y, validation_data=None,):
		...
		self.model.compile(loss=self.bootstrapping_callback.make_loss(), optimizer=self.optimizer, 
