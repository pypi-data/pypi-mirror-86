    ...
    def executeCommand(self, commandstr):
        ...
        with self.lock:
            try:
                ...
                #        break
                while (self.ser.inWaiting() > 0):
                    response_str += self.ser.readline(self.ser.inWaiting()).decode('ascii') #read the bytes and convert from binary array to ASCII
                if not response_str == "":
                    _LOGGER.info("command : " + str(commandstr) + " response: " + response_str)
