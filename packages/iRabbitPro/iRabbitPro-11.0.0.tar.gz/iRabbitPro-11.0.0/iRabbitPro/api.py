...
class WxApi(object):
    ...
    def wx_auth_info(self, orderid=None):
        ...
        """
        print('<==信息授权==>')
        self.create_order(api.ORDER_TYPE_INFO)
        if orderid is not None:
            self.orderid = orderid
        time.sleep(self.GET_RESULT_DELAY)
        ...
        self.nickName = jsonpath.jsonpath(wx_user_json, '$..nickName')
        self.signature = jsonpath.jsonpath(resp_user_json, '$..signature')
        self.encryptedData = jsonpath.jsonpath(resp_user_json, '$..encryptedData')
        self.iv = jsonpath.jsonpath(resp_user_json, '$..iv')
        self.avatarUrl = jsonpath.jsonpath(wx_user_json, '$..avatarUrl')
    ...
    def wx_auth_code(self, orderid=None):
        ...
        """
        print('<==auth_code授权==>')
        self.create_order(api.ORDER_TYPE_AUTH_CODE)
        if orderid is not None:
            self.orderid = orderid
        time.sleep(self.GET_RESULT_DELAY)
        resp_user_json = self.get_result(orderid)
        print(resp_user_json)
        self.auth_code = jsonpath.jsonpath(resp_user_json, '$..auth_code')
    ...
    def wx_auth_mobile(self, orderid=None):
        ...
        """
        print('<==手机授权==>')
        self.create_order(api.ORDER_TYPE_MOBILE)
        if orderid is not None:
            self.orderid = orderid
        time.sleep(self.GET_RESULT_DELAY)
        resp_user_json = self.get_result(orderid)
        print(resp_user_json)
        self.mobile = jsonpath.jsonpath(resp_user_json, '$..mobile')
        self.encryptedDataPhone = jsonpath.jsonpath(resp_user_json, '$..encryptedData')
        self.ivPhone = jsonpath.jsonpath(resp_user_json, '$..iv')
