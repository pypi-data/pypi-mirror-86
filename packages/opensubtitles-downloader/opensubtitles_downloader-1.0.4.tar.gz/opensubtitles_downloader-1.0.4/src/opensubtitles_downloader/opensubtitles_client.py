...
from opensubtitles_downloader import error_handler, OpenSubtitleError
...
class OpenSubtitle(Singleton):
    ...
    def _download_subtitles(self, filenames: list = None) -> (list, None):
        ...
        result = self._search_subtitle('en', filenames) if not any([result]) else result
        if not any([result]):
...
def web_service_login(language: str) -> 'OpenSubtitle':
    """
