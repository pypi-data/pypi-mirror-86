...
print(crayons.red(f'▐█▐▐▌▐▀▀▪▄ ·██·  ▐█.▪▐▀▀▄ ▐█▌▐█▪'))
print(crayons.red(f'██▐█▌▐█▄▄▌▪▐█·█▌ ▐█▌·▐█•█▌ ▐█▀·.'))
print(crayons.red(f'▀▀ █▪ ▀▀▀ •▀▀ ▀▀ ▀▀▀ .▀  ▀  ▀ • ' 'V15.0'))
print(crayons.green(f'Coded By Pirxcy and Mello\nJoin The Discord - https://discord.gg/jXCfx9JEKz\n\n '))
print(crayons.blue(f'Changelog: \nAdded Changeable Admin Code \nUpdate Config on\nhttps://github.com/PirxcyFinal/PirxcyBotFiles/blob/master/config.json\n\nFixed Auto Kick Other Branded Bots!'))
  ...
  if isinstance(message, fortnitepy.message.MessageBase):
        ...
        if "bot" in content:
            if isinstance(message, fortnitepy.PartyMessage):
                ...
                await message.author.kick()        
        client.add_cache(message.author)
        if "lobby bot" in content:
            if isinstance(message, fortnitepy.PartyMessage):
                await message.author.block()
                await message.author.kick()
        client.add_cache(message.author)
        if "hailey" in content:
            if isinstance(message, fortnitepy.PartyMessage):
                await message.author.block()
                await message.author.kick()   
