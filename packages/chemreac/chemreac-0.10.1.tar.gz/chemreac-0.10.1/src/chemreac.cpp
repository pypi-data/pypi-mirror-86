...
ReactionDiffusion<Real_t>::ReactionDiffusion(
    ...
    work3(buffer_factory<double>(nr)),
...
{
    if (m_upper_bounds.size() != static_cast<std::size_t>(n*N)) {
        throw std::runtime_error("trying to use get_dx_max without m_upper_bounds set to correct size.");
    }
    if (m_lower_bounds.size() != static_cast<std::size_t>(n*N)) {
        throw std::runtime_error("trying to use get_dx_max without m_lower_bounds set to correct size.");
    }
    const int ny = get_ny();
...
ReactionDiffusion<Real_t>::rhs(Real_t t, const Real_t * const y, Real_t * const ANYODE_RESTRICT dydt)
{
    ...
    Real_t * const local_r = AnyODE::buffer_get_raw_ptr(work3);
        ...
        if (N>1){
            ...
            for (int si=0; si<n; ++si){ // species index si
                ...
                for (int xi=0; xi<nstencil; ++xi){
                    ...
                    advection_unscaled += DIV_WEIGHT(bi, xi) *                         (LINC(biw, si)*efield[bi] + LINC(bi, si)*efield[biw]);
                }
