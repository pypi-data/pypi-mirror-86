...
class WaitUntilNewInstancesAreBootedStep(BlueGreenStep):
    def execute(self) -> bool:
        ...
        while not all_booted:
            ...
            all_booted = len(asg.instances) == asg.desired_capacity

            if not all_booted:
                self.log(
                    f"Booted instances: {len(asg.instances)}. Desired instances: {asg.desired_capacity}. Waiting.."
                )
                sleep(3)
    ...
    def post_check(self) -> bool:
        ...
        if len(asg.instances) == asg.desired_capacity:
