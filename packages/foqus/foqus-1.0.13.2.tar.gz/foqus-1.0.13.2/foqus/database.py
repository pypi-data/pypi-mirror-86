...
class PostgreSQL:
    ...
    def add_or_update_products(self, table_name="objects", reference="", urlProduit="", product_id="",
                               product_url_mobile="",
                               hash_code="", project_name="", update=False):
        if not update:
            try:
            ...
            try:
                self.cursor.execute(
                    ...
                    "' AND product_id = '" + product_id + "' AND product_url_mobile = '" + product_url_mobile + "';")
    ...
    def get_url_from_hash_client_table(self, table_name="", project_name="", hash=""):
        try:
            self.cursor.execute("SELECT reference, urlProduit, hash_code FROM " + table_name +
    ...
    def add_update_client_parameter(self, customer, customer_type, similars=None, classification=None):
        ...
        default_similars = {"algorithm": "vectors", "classification": True,
                            "vectors": {"model": "VGG16",
                                        "include_top": True,
                                        "classes_numbers": 1000,
                                        "vptree": False,
                                        "vptree_distance": 5,
                                        "nbr_similars": 9,
                                        "seuil": SEUIL_SIMILARITY}}
