...
class VPTree:
    ...
    def __init__(self, points, dist_fn):
        ...
        for p in points:
            try:
                self.dist_fn([float(pi) for pi in self.vp], [float(pi) for pi in p])
        ...
        distances = [self.dist_fn([float(pi) for pi in self.vp], [float(pi) for pi in p]) for p in points]
    ...
    def get_all_in_range(self, query, max_distance):
        ...
        while len(nodes_to_visit) > 0:
            ...
            d = self.dist_fn([float(pi) for pi in query], [float(pi) for pi in node.vp])
            if d < max_distance:
                neighbors.append((d, node.vp))
