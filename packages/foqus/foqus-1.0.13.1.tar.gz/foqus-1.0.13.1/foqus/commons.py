...
def download_or_remove(urls, each_url_product, product_id, product_url_mobile, customer_name=None, customer_type=None, path_out=None,
    ...
    for url in urls.split(' '):
        ...
        if each_url_product == '':
            db.add_or_update_images(db, table_name=replace_special_caractere(customer_name).replace(' ', '_'),
                                    reference=urls, urlProduit="", product_id="", hash_code=hash_files,
                                    project_name=project_name)
        else:
            db.add_or_update_products(db,table_name=replace_special_caractere(customer_name).replace(' ', '_'),
...
def add_layer_to_vgg(customer_name):
    include_top = os.environ['INCLUDE_TOP']
    if customer_name == "shedo":
        include_top = "False"
