...
class NSISScript:
    ...
    def __init__(self,
        ...
        self.comserver_files_exe = [self.chop(p)
                                    for p in comserver_files if p.lower().endswith(".exe")]
        self.comserver_files_dll = [self.chop(p)
                                    for p in comserver_files if p.lower().endswith(".dll")]
    ...
    def create(self, pathname="base.nsi"):
        ...
        ver = [c in '0123456789.' and c or ".%s" %
               (ord(c) - 96) for c in ver] + [rev]
...
class Target():
    ...
    def __init__(self, module, **kw):
        ...
        self.version = module.__version__[
            :-1] + "." + str(ord(module.__version__[-1]) - 96)
        ...
        self.name = "Interfaz PyAfipWs - %s" % os.path.basename(
            module.__file__).replace(".pyc", ".py")
