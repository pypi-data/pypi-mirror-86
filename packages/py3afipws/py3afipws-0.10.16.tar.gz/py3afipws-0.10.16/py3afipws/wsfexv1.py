...
class WSFEXv1(BaseWS):
    ...
    _public_methods_ = [
        'CrearFactura', 'AgregarItem', 'Authorize', 'GetCMP',
        'AgregarPermiso', 'AgregarCmpAsoc', 'GetParamMon',
        'GetParamTipoCbte', 'GetParamTipoExpo', 'GetParamIdiomas',
        'GetParamUMed', 'GetParamIncoterms', 'GetParamDstPais',
        'GetParamDstCUIT', 'GetParamIdiomas', 'GetParamIncoterms',
        'GetParamDstCUIT', 'GetParamMonConCotizacion', 'GetParamPtosVenta',
        'GetParamCtz', 'LoadTestXML', 'AnalizarXml', 'ObtenerTagXml',
        'DebugLog', 'SetParametros', 'SetTicketAcceso', 'GetParametro',
        'GetLastCMP', 'GetLastID', 'Dummy', 'Conectar', 'SetTicketAcceso'
    ]
    _public_attrs_ = [
        'Token', 'Sign', 'Cuit', 'AppServerStatus', 'DbServerStatus',
        'AuthServerStatus', 'XmlRequest', 'XmlResponse', 'Version',
        'Resultado', 'Obs', 'Reproceso', 'CAE', 'Vencimiento', 'Eventos',
        'ErrCode', 'ErrMsg', 'FchVencCAE', 'Excepcion', 'LanzarExcepciones',
        'Traceback', "InstallDir", 'PuntoVenta', 'CbteNro', 'FechaCbte',
        'ImpTotal'
    ]
    ...
    def __analizar_errores(self, ret):
        ...
        if 'FEXErr' in ret:
            ...
            self.ErrCode = ' '.join(
                [str(error['ErrCode']) for error in errores])
            self.ErrMsg = '\n'.join(self.Errores)
        if 'FEXEvents' in ret:
            events = [ret['FEXEvents']]
            self.Eventos = [
                '%s: %s' % (evt['EventCode'],
                            evt.get('EventMsg', "")) for evt in events]

    def CrearFactura(
        self, tipo_cbte=19, punto_vta=1, cbte_nro=0, fecha_cbte=None,
    ...
    def AgregarItem(
        self, codigo, ds, qty, umed, precio, importe, bonif=None, **kwargs
    ):
    ...
    def GetCMP(self, tipo_cbte, punto_vta, cbte_nro):
        ...
        if 'FEXResultGet' in result:
            ...
            # Obs, cae y fecha cae
            self.Obs = resultget.get('Obs') and \
                resultget['Obs'].strip(" ") or ''
            self.CAE = resultget.get('Cae', '')
            vto = str(resultget.get('Fch_venc_Cae', ''))
            self.Vencimiento = "%s/%s/%s" % (vto[6:8], vto[4:6], vto[0:4])
            # .strftime("%Y/%m/%d")
            self.FechaCbte = resultget.get('Fecha_cbte', '')
    ...
    def GetLastCMP(self, tipo_cbte, punto_vta):
        ...
        if 'FEXResult_LastCMP' in result:
            ...
            # .strftime("%Y/%m/%d")
            self.FechaCbte = resultget.get('Cbte_fecha')
            return self.CbteNro
    ...
    def GetParamUMed(self, sep="|"):
        ...
        if sep:
            return [
                ("\t%(id)s\t%(ds)s\t%(vig_desde)s\t%(vig_hasta)s\t" % it)
                .replace("\t", sep) for it in umeds
            ]
        else:
            return umeds
    ...
    def GetParamTipoCbte(self, sep="|"):
        """Recuperador de valores referenciales de códigos de Tipo de
        comprobantes"""
        ...
        if sep:
            return [
                ("\t%(codigo)s\t%(ds)s\t%(vig_desde)s\t%(vig_hasta)s\t" % it)
                .replace("\t", sep) for it in ret
            ]
        else:
            return ret
    ...
    def GetParamPtosVenta(self, sep="|"):
        ...
        log.warn("FEXGetPARAM_PtoVentaResult Result: " + str(res))
        ret = []
        for pu in res:
            log.warn("pu Result: " + str(pu))
            p = pu['ClsFEXResponse_PtoVenta']
            log.warn("p Result: " + str(p))
            try:
                r = {
                    'nro': p.get('Pve_Nro'),
                ...
                print(e)
            ret.append(r)
        log.warn("FEXGetPARAM_PtoVentaResult ret: " + str(ret))
        return [
            ("%(nro)s\tBloqueado:%(bloqueado)s\tFchBaja:%(baja)s" % rt)
            .replace("\t", sep) for rt in ret
        ]
...
class WSFEX(WSFEXv1):
    ...
    def __init__(self):
        ...
        self.Version = "%s %s WSFEXv1" % (
            __version__, HOMO and 'Homologación' or '')
    ...
    #    win32com.server.localserver.serve([WSFEXv1._reg_clsid_])
        ...
        if "--prueba" in sys.argv:
            try:
                ...
                tipo_cbte = '--nc' in sys.argv and 21 or 19
                punto_vta = 7
                ...
                ok = wsfexv1.CrearFactura(
                    tipo_cbte, punto_vta, cbte_nro, fecha_cbte, imp_total,
                    tipo_expo, permiso_existente, dst_cmp, cliente,
                    cuit_pais_cliente, domicilio_cliente, id_impositivo,
                ...
                # lo agrego a la factura (internamente, no se llama al
                # WebService):
                ...
                ok = wsfexv1.AgregarItem(
                    codigo, ds, qty, umed, precio, imp_total, bonif)
                ok = wsfexv1.AgregarItem(
                    codigo, ds, 0, 99, 0, -float(imp_total), 0)
                ...
                if tipo_cbte in (20, 21):
                    ...
                    wsfexv1.AgregarCmpAsoc(
                        cbteasoc_tipo, cbteasoc_pto_vta, cbteasoc_nro,
                        cbteasoc_cuit)
                ...
                if wsfexv1.Resultado and False:
                    ...
                    print("NRO consulta", wsfexv1.CbteNro,
                          wsfexv1.CbteNro == cbte_nro)
                    print("TOTAL consulta", wsfexv1.ImpTotal,
        ...
        if "--params" in sys.argv:
            ...
            for t in tipos:
                print(
                    "||%(codigo)s||%(ds)s||%(vig_desde)s||%(vig_hasta)s||" % t)
            # umeds = dict([(u.get('id', ""),u.get('ds', "")) for u in
            # umedidas])
