...
"Almacenamiento de duplicados electr�nicos RG1361/02 y RG1579/03 AFIP"
...
(Sistema Res�men Electr�nico de Datos / Almacenamiento de Duplicados)
...
categorias = {"responsable inscripto": "01", # IVA Responsable Inscripto
              ...
              "responsable inscripto - agente de percepci�n": "11", # IVA Responsable Inscripto - Agente de Percepcion
}
...
def generar_detalle(items):
    ...
    # recorro las facturas y detalles de art�culos vendidos:
    for item in items:
        for it in item.get('detalles', [{}]):
            ...
            if 'iva_id' in it and it['iva_id']:
                # mapear alicuota de iva seg�n c�digo usado en MTX
                ...
                if float(item.get('impto_liq', item.get('imp_iva', 0))) == 0:
                    ...
                    vals['gravado'] = 'G'
            # dise�o libre: c�digo de barras y descripci�n:
...
def generar_ventas(items):
    ...
    for item in items:
        ...
        for i, iva in enumerate(ivas):
            ...
            # datos generales de la factura:
            vals['tipo_reg'] = '1'
            # copio los campos que no var�an para las distintas alicuotas de IVA
...
class SIRED():
    "Componente para Sistema Res�men Electr�nico de Datos RG1361/02 RG1579/03"
