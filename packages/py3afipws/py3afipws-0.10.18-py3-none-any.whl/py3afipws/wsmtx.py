...
"""M�dulo para obtener c�digo de autorizaci�n electr�nico CAE/CAEA webservice
WSMTX de AFIP (Factura Electr�nica Mercado Interno con codificaci�n de
productos) seg�n RG2904 (opci�n A con detalle) y RG2926/10 (CAE anticipado).
"""
...
class WSMTXCA(BaseWS):
    "Interfaz para el WebService de Factura Electr�nica Mercado Interno \
        WSMTXCA"
    _public_methods_ = [
        'CrearFactura', 'EstablecerCampoFactura', 'AgregarIva', 'AgregarItem',
        ...
        'ConsultarPtosVtaCAEANoInformados', 'ConsultarComprobante',
        'ConsultarTiposComprobante', 'ConsultarTiposDocumento',
        'ConsultarAlicuotasIVA', 'ConsultarCondicionesIVA', 'ConsultarMonedas',
        'ConsultarUnidadesMedida', 'ConsultarTiposTributo',
        'ConsultarTiposDatosAdicionales', 'ConsultarCotizacionMoneda',
        'ConsultarPuntosVentaCAE', 'ConsultarPuntosVentaCAEA', 'AnalizarXml',
        'ObtenerTagXml', 'LoadTestXML', 'SetParametros', 'SetTicketAcceso',
        'GetParametro', 'Dummy', 'Conectar', 'DebugLog', 'SetTicketAcceso'
    ]
    _public_attrs_ = [
        'Token', 'Sign', 'Cuit',
        'AppServerStatus', 'DbServerStatus', 'AuthServerStatus',
        'XmlRequest', 'XmlResponse', 'Version', 'InstallDir',
        'LanzarExcepciones', 'Resultado', 'Obs', 'Observaciones', 'ErrCode',
        'ErrMsg', 'EmisionTipo', 'Reproceso', 'Reprocesar', 'Evento',
        'CAE', 'Vencimiento', 'Evento', 'Errores', 'Traceback', 'Excepcion',
        'CAEA', 'Periodo', 'Orden', 'FchVigDesde', 'FchVigHasta', 'FchTopeInf',
        'FchProceso', 'CbteNro', 'FechaCbte', 'PuntoVenta', 'ImpTotal'
    ]
    ...
    def CrearFactura(
        self, concepto=None, tipo_doc=None, nro_doc=None, tipo_cbte=None,
        punto_vta=None, cbt_desde=None, cbt_hasta=None, imp_total=None,
        imp_tot_conc=None, imp_neto=None, imp_subtotal=None, imp_trib=None,
        imp_op_ex=None, fecha_cbte=None, fecha_venc_pago=None,
        fecha_serv_desde=None, fecha_serv_hasta=None, moneda_id=None,
        moneda_ctz=None, observaciones=None, caea=None, fch_venc_cae=None,
        **kwargs
    ):
    ...
    def AgregarTributo(
        self, tributo_id, desc, base_imp, alic, importe, **kwargs
    ):
    ...
    def AgregarItem(
        self, u_mtx=None, cod_mtx=None, codigo=None, ds=None, qty=None,
        umed=None, precio=None, bonif=None, iva_id=None, imp_iva=None,
        imp_subtotal=None, **kwargs
    ):
    ...
    def AutorizarComprobante(self):
        ...
        fact = {
            ...
            'arrayComprobantesAsociados': f['cbtes_asoc'] and [{
                'comprobanteAsociado': {
            ...
            'arrayOtrosTributos': f['tributos'] and [{
                'otroTributo': {
            ...
            'arraySubtotalesIVA': f['iva'] and [{
                'subtotalIVA': {
                    'codigo': iva['iva_id'],
                    'importe': iva['importe'],
                }
            } for iva in f['iva']] or None,
            'arrayItems': f['detalles'] and [{
                'item': {
                    ...
                    'importeIVA': it['imp_iva'] if int(f['tipo_cbte'])
                    not in (6, 7, 8) and it['imp_iva'] is not None
                    else None,
    ...
    def AutorizarAjusteIVA(self):
        """Env�a la informaci�n del comprobante de ajuste de IVA que desea
        autorizar"""
        f = self.factura
        # contruyo la estructura a convertir en XML:
        fact = {
            ...
            'arrayItems': f['detalles'] and [{
                'item': {
                    ...
                    'codigoCondicionIVA': it['iva_id'],
                    'importeIVA': it['imp_iva'] if int(f['tipo_cbte'])
                    not in (6, 7, 8) and it['imp_iva'] is not None else None,
                    'importeItem': it['imp_subtotal'],
                }
            } for it in f['detalles']] or None,
            'arrayDatosAdicionales': f['opcionales'] and [{'datoAdicional': {
    ...
    def SolicitarCAEA(self, periodo, orden):
        "Obtener un CAEA y su respectivo per�odo de vigencia"
    ...
    def ConsultarCAEAEntreFechas(self, fecha_desde, fecha_hasta):
        ...
        if 'arrayCAEAResponse' in ret:
            return [
                res['CAEAResponse']['CAEA'] for res in ret['arrayCAEAResponse']
            ]
        return []

    @inicializar_y_capturar_excepciones
    def InformarComprobanteCAEA(self):
        "Env�a la informaci�n del comprobante emitido y asociado a un CAEA"
        f = self.factura
        # contruyo la estructura a convertir en XML:
        fact = {
            'codigoTipoDocumento': f['tipo_doc'],
        ...
        # fecha de vencimiento opcional (igual al �ltimo d�a de vigencia
        # del CAEA)
    ...
    def InformarAjusteIVACAEA(self):
        "Env�a la informaci�n del comprobante de ajuste de IVA emitidos"
        f = self.factura
        # contruyo la estructura a convertir en XML:
        fact = {
            'codigoTipoDocumento': f['tipo_doc'],
    ...
    @inicializar_y_capturar_excepciones
    def ConsultarComprobante(
        self, tipo_cbte, punto_vta, cbte_nro, reproceso=False
    ):
        ...
        if 'comprobante' in ret:
                ...
                if reproceso:
                    # verifico los campos registrados coincidan con los
                    # enviados:
                    f = self.factura
                    verificaciones = {
                        'codigoTipoComprobante': f['tipo_cbte'],
                        ...
                        'importeNoGravado': decimal.Decimal(
                            str(f['imp_tot_conc'])
                        ),
                        'importeGravado': decimal.Decimal(str(f['imp_neto'])),
                        'importeExento': decimal.Decimal(str(f['imp_op_ex'])),
                        'importeOtrosTributos': f['tributos'] and
                        decimal.Decimal(str(f['imp_trib'])) or None,
                        'importeSubtotal': f['imp_subtotal'],
                        'fechaServicioDesde': f.get('fecha_serv_desde'),
                        'fechaServicioHasta': f.get('fecha_serv_hasta'),
                        'fechaVencimientoPago': f.get('fecha_venc_pago'),
                        'codigoMoneda': f['moneda_id'],
                        'cotizacionMoneda': str(
                            decimal.Decimal(str(f['moneda_ctz']))
                        ),
                        'arrayItems': [
                            {'item': {
                                'unidadesMtx': it['u_mtx'],
                                ...
                                'codigo': it['codigo'],
                                'descripcion': it['ds'],
                                'cantidad': it['qty'] and decimal.Decimal(
                                    str(it['qty'])
                                ),
                                'codigoUnidadMedida': it['umed'],
                                'precioUnitario': it['precio'] is not None
                                and decimal.Decimal(str(it['precio'])) or None,
                                # 'importeBonificacion': it['bonif'],
                                'codigoCondicionIVA': decimal.Decimal(
                                    str(it['iva_id'])
                                ),
                                'importeIVA': decimal.Decimal(
                                    str(it['imp_iva'])
                                ) if int(f['tipo_cbte']) not in (6, 7, 8) and
                                it['imp_iva'] is not None else None,
                                'importeItem': decimal.Decimal(
                                    str(it['imp_subtotal'])
                                ),
                            }}
                ...
                # 60423794871430L
                self.CAE = str(cbteresp['codigoAutorizacion'])
                self.EmisionTipo = cbteresp['codigoTipoAutorizacion'] == 'A' \
                    and 'CAEA' or 'CAE'
        self.__analizar_errores(ret)
        if not difs:
            return self.CAE
    ...
    def ConsultarTiposComprobante(self):
        """Este m�todo permite consultar los tipos de comprobantes habilitados
        en este WS"""
    ...
    def ConsultarPuntosVentaCAE(
        self,
        fmt="%(numeroPuntoVenta)s: bloqueado=%(bloqueado)s baja=%(fechaBaja)s"
    ):
        """Este m�todo permite consultar los puntos de venta habilitados para
        CAE en este WS"""
    ...
    def ConsultarPtosVtaCAEANoInformados(self, caea):
        """Este m�todo permite  consultar que puntos de venta a�n no fueron
        informados para  un  CAEA determinado."""
        ...
        return [" ".join(
            [("%s=%s" % (k, v))for k, v in list(p['puntoVenta'].items())]
        )
...
def main():
    ...
    cache = ""
    if "--prod" in sys.argv:
        wsdl = "https://serviciosjava.afip.gov.ar/wsmtxca/services/"
        "MTXCAService?wsdl"
    else:
        wsdl = WSDL
    ...
    if "--prueba" in sys.argv:
        try:
            ...
            imp_subtotal = 242.00
            wsmtxca.AgregarItem(
                u_mtx, cod_mtx, codigo, ds, qty, umed, precio, bonif, iva_id,
                imp_iva, imp_subtotal
            )

            if "--caea" not in sys.argv:
                # ejemplo descuento (sin precio unitario)
                wsmtxca.AgregarItem(
                    None, None, None, 'bonificacion', None, 99, None, None,
                    5, -21, -121
                )
                # ejemplo item solo descripci�n:
            ...
            if cae:
                ...
                print(
                    "NRO consulta", wsmtxca.CbteNro,
                    wsmtxca.CbteNro == cbte_nro
                )
                print(
                    "TOTAL consulta",
                    wsmtxca.ImpTotal,
                    wsmtxca.ImpTotal == imp_total
                )

                wsmtxca.AnalizarXml("XmlResponse")
                assert wsmtxca.ObtenerTagXml(
                    'codigoAutorizacion'
                ) == str(wsmtxca.CAE)
                assert wsmtxca.ObtenerTagXml('codigoConcepto') == str(concepto)
                assert wsmtxca.ObtenerTagXml(
                    'arrayItems', 0, 'item', 'unidadesMtx'
                ) == '123456'
