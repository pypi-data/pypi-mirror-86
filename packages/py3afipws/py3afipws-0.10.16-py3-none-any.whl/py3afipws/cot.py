...
"M�dulo para obtener remito electr�nico autom�tico (COT)"
...
HOMO = False
CACERT = "conf/arba.crt"   # establecimiento de canal seguro (en producci�n)

##URL = "https://cot.ec.gba.gob.ar/TransporteBienes/SeguridadCliente/"
#"presentarRemitos.do"
# Nuevo servidor para el "Remito Electr�nico Autom�tico"
URL = "http://cot.test.arba.gov.ar/TransporteBienes/SeguridadCliente/"
"presentarRemitos.do"  # testing
#URL = "https://cot.arba.gov.ar/TransporteBienes/SeguridadCliente/"
#"presentarRemitos.do"  # prod.
...
class COT:
    ...
    _public_attrs_ = [
        'Usuario', 'Password', 'XmlResponse',
        'Version', 'Excepcion', 'Traceback', 'InstallDir',
        'CuitEmpresa', 'NumeroComprobante', 'CodigoIntegridad',
        'NombreArchivo', 'TipoError', 'CodigoError', 'MensajeError',
    ...
    def Conectar(
        self, url=None, proxy="", wrapper=None, cacert=None, trace=False
    ):
        if HOMO or not url:
            url = URL
        ...
        try:
            ...
            if 'tipoError' in self.xml:
                ...
                self.MensajeError = str(self.xml.mensajeError)\
                    .decode('latin1').encode("ascii", "replace")
...
# busco el directorio de instalaci�n (global para que no cambie si usan
# otra dll)
...
if __name__ == "__main__":
    ...
    if "--register" in sys.argv or "--unregister" in sys.argv:
        ...
        print("Se debe especificar el nombre de archivo, usuario y clave como\
                argumentos!")
        sys.exit(1)
    ...
    print(
        "Error General:", cot.TipoError, "|", cot.CodigoError,
        "|", cot.MensajeError
    )
    ...
    while cot.LeerValidacionRemito():
        ...
        while cot.LeerErrorValidacion():
            print(
                "Error Validacion:", "|", cot.CodigoError,
                "|", cot.MensajeError
            )
    ...
    if False:
        ...
        print(
            "p0",
            cot.ObtenerTagXml('validacionesRemitos', 'remito', 0, 'procesado')
        )
        print(
            "p1",
            cot.ObtenerTagXml('validacionesRemitos', 'remito', 1, 'procesado')
        )
