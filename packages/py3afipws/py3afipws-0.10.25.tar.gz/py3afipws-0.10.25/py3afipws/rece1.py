...
LICENCIA = """
rece1.py: Interfaz de texto para generar Facturas Electr�nica Mercado Interno
V1
...
TIPO_CBTE = {
    1: "FAC A", 2: "N/D A", 3: "N/C A", 6: "FAC B", 7: "N/D B",
            ...
            if str(linea[0]) == '0':
                ...
                if DEBUG:
                    print(len(encabezados),
                          "Leida factura %(cbt_desde)s" % encabezado)
            elif str(linea[0]) == '1':
    ...
    for encabezado in encabezados:
        ...
        if not DEBUG or raw_input("Facturar (S/n)?") == "S":
            if not informar_caea:
                ...
                dic = ws.factura
                print(
                    "Procesando %s %04d %08d %08d %s %s $ %0.2f IVA: $ \
                    %0.2f" % (
                        TIPO_CBTE.get(
                            dic['tipo_cbte'],
                            dic['tipo_cbte']
                        ...
                        float(dic['imp_total']),
                        float(
                            dic['imp_iva'] if dic['imp_iva'] is not None
                            else 'NaN'
                        )
                    ))
            ...
            dicts.append(dic)
            print(
                "NRO:", dic['cbt_desde'], "Resultado:", dic['resultado'],
                "%s:" % ws.EmisionTipo, dic['cae'], "Obs:",
                dic['motivos_obs'].encode("ascii", "ignore"), "Err:",
...
if __name__ == "__main__":
    if '/ayuda' in sys.argv:
        ...
        print(
            " /prueba: genera y autoriza una factura de prueba (no usar \
            en producci�n!)")
        ...
        print(
            " /get: recupera datos de un comprobante autorizado \
            previamente (verificaci�n)")
        print(
    ...
    # si se pasa el archivo de configuraci�n por par�metro, confirmar que
    # exista
    # y descartar que sea una opci�n
    if len(sys.argv) > 1 and (sys.argv[1][0] not in "-/"
                              or os.path.exists(sys.argv[1])):
    ...
    privatekey = config.get('W SAA', 'PRIVATEKEY')
    ...
    if config.has_section('PROXY') and not HOMO:
        ...
        proxy_dict = {}
    CACERT = config.has_option(
        'WSFEv1', 'CACERT') and config.get('WSFEv1', 'CACERT') or None
    WRAPPER = config.has_option(
        'WSFEv1', 'WRAPPER') and config.get('WSFEv1', 'WRAPPER') or None

    if '/xml'in sys.argv:
        XML = True

    RUTA_XML = config.has_option(
        'WSFEv1', 'XML') and config.get('WSFEv1', 'XML') or "."

    if DEBUG:
        print(
            "wsaa_url %s\nwsfev1_url %s\ncuit %s" % (
                wsaa_url, wsfev1_url, cuit)
        )
        if proxy_dict:
    ...
    if '/x' in sys.argv:
        escribir_facturas(
            [{'err_msg': "Prueba", }], open("x.txt", "w"))

    try:
        ws = wsfev1.WSFEv1()
        ws.LanzarExcepciones = True
        ws.Conectar(
            "", wsfev1_url, proxy=proxy_dict, cacert=CACERT, wrapper=WRAPPER)
        ws.Cuit = cuit
        ...
        if '/formato' in sys.argv:
            ...
            for msg, formato in [('Encabezado', ENCABEZADO),
                                 ...
                                 ('Opcionales', OPCIONAL)]:
                if '/dbf' not in sys.argv:
                    comienzo = 1
                    print("== %s ==" % msg)
                    for fmt in formato:
                        clave, longitud, tipo = fmt[0:3]
                        dec = len(fmt) > 3 and fmt[3] or (
                            tipo == 'I' and '2' or '')
                        print(" * Campo: %-20s Posici�n: %3d Longitud: %4d \
                            Tipo: %s Decimales: %s" % (
        ...
        wsaa = WSAA()
        ta = wsaa.Autenticar(
            "wsfe", cert, privatekey, wsaa_url, proxy=proxy_dict,
            cacert=CACERT, wrapper=WRAPPER)
        if not ta:
        ...
        if '/solicitarcaea' in sys.argv:
            ...
            if DEBUG:
                print(
                    "Solicitando CAEA para periodo %s orden %s" % (
                        periodo, orden)
                )
            ...
            if not caea:
                if DEBUG:
                    print(
                        "Consultando CAEA para periodo %s orden %s" % (
                            periodo, orden)
                    )
        ...
        if '/informarcaeanoutilizadoptovta' in sys.argv:
            ...
            if DEBUG:
                print(
                    "Informando CAEA no utilizado: %s pto_vta %s" % (
                        caea, pto_vta)
                )
        ...
        print(
            "SoapFault:", e.faultcode, e.faultstring.encode("ascii", "ignore"))
        sys.exit(3)
