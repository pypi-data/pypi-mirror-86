...
class WSSrPadronA4(BaseWS):
    ...
    def Consultar(self, id_persona):
        ...
        log.warn("____ws_sr_padron CONSULTAR____")
        log.warn("___sign: " + str(self.Sign))
        log.warn("___token: " + str(self.Token))
        log.warn("___cuitRepresentada: " + str(self.Cuit))
        log.warn("___idPersona: " + str(id_persona))
        ...
        if isinstance(data, list):
            data = data[0]
        self.data = data
        log.warn("Resultado del AFIP: " + str(self.data))
        # lo serializo
...
def main():
    ...
    padron.Conectar(cache, url_ws, cacert="")
