...
class WSMTXCA(BaseWS):
    ...
    def AutorizarAjusteIVA(self):
        "Env�a la informaci�n del comprobante de ajuste de IVA que desea autorizar"
        f = self.factura
        # contruyo la estructura a convertir en XML:
        fact = {
            'codigoTipoDocumento': f['tipo_doc'], 'numeroDocumento': f['nro_doc'],
    ...
    def ConsultarCAEA(self, periodo=None, orden=None, caea=None):
        ...
        if caea:
            ret = self.client.consultarCAEA(
                authRequest={'token': self.Token,
                             'sign': self.Sign, 'cuitRepresentada': self.Cuit},
                CAEA=caea,
            )
    ...
    def ConsultarTiposDatosAdicionales(self):
        "Este m�todo permite consultar los tipos de datos adicionales."
...
def main():
    ...
    if "--solicitar-caea" in sys.argv:
        ...
        if DEBUG:
            print("Consultando CAEA para periodo %s orden %s" %
                  (periodo, orden))
