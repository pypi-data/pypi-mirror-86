...
URL = "http://www.afip.gob.ar/genericos/cInscripcion/archivos/"
...
class PadronAFIP():
    ...
    _public_methods_ = [
        'Buscar', 'Descargar', 'Procesar', 'Guardar', 'ConsultarDomicilios',
        'Consultar', 'Conectar', 'DescargarConstancia', 'MostrarPDF',
        'ObtenerTablaParametros',
    ]
    _public_attrs_ = [
        'InstallDir', 'Traceback', 'Excepcion', 'Version', 'cuit', 'dni',
        'denominacion', 'imp_ganancias', 'imp_iva', 'monotributo',
        'integrante_soc', 'empleador', 'actividad_monotributo', 'cat_iva',
        'domicilios', 'tipo_doc', 'nro_doc', 'LanzarExcepciones',
        'tipo_persona', 'estado', 'impuestos', 'actividades', 'direccion',
        'localidad', 'provincia', 'cod_postal', 'data', 'response'
    ]
    ...
    @inicializar_y_capturar_excepciones_simple
    def Conectar(
        self, url=URL_API, proxy="", wrapper=None, cacert=None, trace=False
    ):
    ...
    def Descargar(self, url=URL, filename="padron.txt", proxy=None):
        ...
        try:
            ...
            if e.code == 304:
                log.warn(("No modificado desde", http_date))
                return 304
            else:
                raise
        ...
        while True:
            ...
            if not data:
                ...
                break
            tmp.write(data)
        log.warn("Abriendo ZIP...")
        tmp.close()
        web.close()
        uf = open(filename + ".zip", "rb")
        zf = zipfile.ZipFile(uf)
        for fn in zf.namelist():
            log.warn(("descomprimiendo", fn))
            tf = open(filename, "wb")
            tf.write(zf.read(fn))
            tf.close()
        return 200
    ...
    @inicializar_y_capturar_excepciones_simple
    def Guardar(
        self, tipo_doc, nro_doc, denominacion, cat_iva, direccion, email
    ):
    ...
    def Consultar(self, nro_doc):
        ...
        if result['success']:
            ...
            if domicilio:
                ...
                self.localidad = domicilio.get("localidad", "")  # no usado en
                # CABA
                self.provincia = PROVINCIAS.get(
                    domicilio.get("idProvincia"), ""
                )
            ...
            self.domicilios = [
                "%s - %s (%s) - %s" % (
                    self.direccion, self.localidad, self.cod_postal,
                    self.provincia,)
                ]
            # analizo impuestos:
    ...
    def DescargarConstancia(self, nro_doc, filename="constancia.pdf"):
        ...
        self.response = self.client(
            "sr-padron", "v1", "constancia", str(nro_doc)
        )
    ...
    def ObtenerTablaParametros(self, tipo_recurso, sep="||"):
        ...
        if sep:
            return [
                "%s%%s%s%%s%s" % (sep, sep, sep) % it for it in sorted(
                    ret.items())
            ]
        else:
            return ret
...
# busco el directorio de instalación (global para que no cambie si usan otra
# dll)
