...
"M�dulo de Servidor COM para interfaz con Windows"
...
from php import (
    # SimpleXMLElement,
    SoapFault, SoapClient, parse_proxy
)
import traceback
...
def checkError(result):
    if int(result.RError.percode) != 0:
        raise COMException(
            scode=vbObjectError + int(result.RError.percode),
...
def raisePythonException(e):
    ...
    raise COMException(
        scode=vbObjectError, desc=''.join(ex), source=u"Python")


class WSFE:
    "Interfaz para el WebService de Factura Electr�nica"
    _public_methods_ = [
        'RecuperarQty', 'UltNro', 'RecuperaLastCMP', 'Aut', 'Dummy', 'Conectar'
    ]
    _public_attrs_ = ['Token', 'Sign', 'Cuit',
                      'AppServerStatus', 'DbServerStatus', 'AuthServerStatus',
                      'XmlRequest', 'XmlResponse', 'Version',
                      'Resultado', 'Motivo', 'Reproceso',
                      'CbtDesde', 'CbtHasta', 'FechaCbte', 'ImpTotal',
                      'ImpNeto', 'ImptoLiq', 'LastID', 'LastCMP', 'CAE',
                      'Vencimiento']
    _reg_progid_ = "WSFE"
    ...
    def Conectar(self, url="", proxy=""):
        "Establecer la conexi�n a los servidores de la AFIP"
        if HOMO or not url:
            url = wsfe.WSFEURL
    ...
    def RecuperarQty(self):
        "Recuperar cantidad m�xima de registros de detalle"
        try:
    ...
    def UltNro(self):
        "Recuperar el �ltimo n�mero de transacci�n (id)"
    ...
    def RecuperaLastCMP(self, ptovta, tipocbte):
        "Recuperar �ltimo n�mero de comprobante autorizado"
        ...
        try:
            ...
            results = self.client.FEAutRequest(
                argAuth={
                    ...
                    'id': long(id),
                    'cantidadreg': 1,
                    'presta_serv': int(presta_serv)},
                    'Fedr': {'FEDetalleRequest': detalle, }})

            if 'FEAutRequestResult' not in results:
                return ''
            ...
            # Motivo del detalle:
            motivo = str(
                results.FEAutRequestResult.FedResp.FEDetalleResponse.motivo)
            if self.Motivo in ("NULL", '00') and motivo not in ("NULL", "00"):
                self.Motivo = motivo
            self.Reproceso = str(results.FEAutRequestResult.reproceso)
            self.CAE = str(
                results.FEAutRequestResult.FedResp.FEDetalleResponse.cae)
            vto = str(
                results.FEAutRequestResult.FedResp.FEDetalleResponse.fecha_vto)
            self.Vencimiento = "%s/%s/%s" % (vto[6:8], vto[4:6], vto[0:4])

            # completo los campos devueltos
            self.CbtDesde = str(
                results.FEAutRequestResult.FedResp.FEDetalleResponse.cbt_desde)
            self.CbtHasta = str(
                results.FEAutRequestResult.FedResp.FEDetalleResponse.cbt_hasta)
            self.FechaCbte = str(
                results.FEAutRequestResult.FedResp.FEDetalleResponse.fecha_cbte
            )
            self.ImpTotal = str(
                results.FEAutRequestResult.FedResp.FEDetalleResponse.imp_total)
            self.ImpNeto = str(
                results.FEAutRequestResult.FedResp.FEDetalleResponse.imp_neto)
            self.ImptoLiq = str(
                results.FEAutRequestResult.FedResp.FEDetalleResponse.impto_liq)
            ...
            # guardo los mensajes para depuraci�n
...
class WSBFE:
    ...
    _public_attrs_ = ['Token', 'Sign', 'Cuit',
                      ...
                      'ErrCode', 'ErrMsg', 'CAE', 'Vencimiento', 'Eventos',
    ...
    def CrearFactura(self, tipo_doc=80, nro_doc=23111111113,
                     zona=0, tipo_cbte=1, punto_vta=1, cbte_nro=0,
                     fecha_cbte=None, imp_total=0.0, imp_neto=0.0,
                     impto_liq=0.0, imp_tot_conc=0.0, impto_liq_rni=0.00,
                     imp_op_ex=0.00, imp_perc=0.00, imp_iibb=0.00,
                     imp_perc_mun=0.00, imp_internos=0.00, imp_moneda_id=0,
                     imp_moneda_ctz=1.0):
    ...
    def AgregarItem(
        self, ncm, sec, ds, qty, umed, precio, bonif, iva_id, imp_total
    ):
    ...
    def Authorize(self, id):
        ...
        try:
            ...
            auth, events = wsbfe.authorize(
                self.client, self.Token, self.Sign, self.Cuit, id=id,
    ...
    def GetLastCMP(self, tipo_cbte, punto_vta):
        ...
        try:
            ...
            cbte_nro, cbte_fecha, events = wsbfe.get_last_cmp(
                self.client, self.Token, self.Sign, self.Cuit,
                tipo_cbte, punto_vta)
            return cbte_nro
        except wsbfe.BFEError as e:
            self.ErrCode = unicode(e.code)
    ...
    def GetParamMon(self):
        ...
        params = wsbfe.get_param_mon(
            self.client, self.Token, self.Sign, self.Cuit)
        return [
            '%(id)s: %(ds)s (%(vig_desde)s - %(vig_hasta)s)' % p for p in
            params]

    def GetParamTipoCbte(self):
        "Recuperar lista de valores referenciales de c�digos de Tipos de \
        comprobante"
        params = wsbfe.get_param_tipo_cbte(
            self.client, self.Token, self.Sign, self.Cuit)
        return [
            '%(id)s: %(ds)s (%(vig_desde)s - %(vig_hasta)s)' % p for p in
            params]
    ...
    def GetParamTipoIVA(self):
        ...
        params = wsbfe.get_param_tipo_iva(
            self.client, self.Token, self.Sign, self.Cuit)
        return [
            '%(id)s: %(ds)s (%(vig_desde)s - %(vig_hasta)s)' % p for p in
            params]
    ...
    def GetParamNCM(self):
        "Recuperar lista de valores referenciales de c�digos del Nomenclador \
        Com�n del Mercosur"
        params = wsbfe.get_param_ncm(
            self.client, self.Token, self.Sign, self.Cuit)
        return [
            '%(codigo)s: %(ds)s (%(vig_desde)s - %(vig_hasta)s)' % p for p in
            params]


class WSFEX:
    "Interfaz para el WebService de Factura Electr�nica Exportaci�n"
    _public_methods_ = ['CrearFactura', 'AgregarItem', 'Authorize', 'GetCMP',
                        ...
                        'GetParamCtz', 'Dummy', 'Conectar', 'GetLastCMP',
                        'GetLastID']
    _public_attrs_ = ['Token', 'Sign', 'Cuit',
                      'AppServerStatus', 'DbServerStatus', 'AuthServerStatus',
                      'XmlRequest', 'XmlResponse', 'Version',
    ...
    def CrearFactura(self, tipo_cbte=19, punto_vta=1, cbte_nro=0,
                     fecha_cbte=None, imp_total=0.0, tipo_expo=1,
                     permiso_existente="N", dst_cmp=None,
    ...
    def GetParamTipoCbte(self):
        "Recuperar lista de valores referenciales de c�digos de Tipos de \
        comprobante"
        params = wsfex.get_param_tipo_cbte(
            self.client, self.Token, self.Sign, self.Cuit)
        return [
            '%(id)s: %(ds)s (%(vig_desde)s - %(vig_hasta)s)' % p for p in
            params]
    ...
    def GetParamTipoExpo(self):
        ...
        exportaci�n"
        params = wsfex.get_param_tipo_expo(
            self.client, self.Token, self.Sign, self.Cuit)
        return [
            '%(id)s: %(ds)s (%(vig_desde)s - %(vig_hasta)s)' % p for p in
            params]
    ...
    def GetParamIdiomas(self):
        ...
        params = wsfex.get_param_idiomas(
            self.client, self.Token, self.Sign, self.Cuit)
        return [
            '%(id)s: %(ds)s (%(vig_desde)s - %(vig_hasta)s)' % p for p in
            params]
    ...
    def GetParamIncoterms(self):
        ...
        params = wsfex.get_param_incoterms(
            self.client, self.Token, self.Sign, self.Cuit)
        return [
            '%(id)s: %(ds)s (%(vig_desde)s - %(vig_hasta)s)' % p for p in
            params]
    ...
    def GetParamDstPais(self):
        "Recuperar lista de valores referenciales de c�digos de Pa�ses"
        params = wsfex.get_param_dst_pais(
            self.client, self.Token, self.Sign, self.Cuit)
        return ['%s: %s' % (p['codigo'], p['ds']) for p in params]

    def GetParamDstCUIT(self):
        "Recuperar lista de valores referenciales de CUIT de Pa�ses"
        params = wsfex.get_param_dst_cuit(
            self.client, self.Token, self.Sign, self.Cuit)
        return ['%s: %s' % (p['cuit'], p['ds']) for p in params]

    def GetParamCtz(self, moneda_id='DOL'):
        "Recuperar cotizaci�n de moneda"
        try:
            param = wsfex.get_param_ctz(
                self.client, self.Token, self.Sign, self.Cuit, moneda_id)
...
class WSCTG:
    "Interfaz para el WebService de C�digo de Trazabilidad de Granos"
    ...
    def SolicitarCTG(self, numero_carta_de_porte, codigo_especie,
                     cuit_remitente_comercial, cuit_destino, cuit_destinatario,
                     codigo_localidad_origen, codigo_localidad_destino,
                     codigo_cosecha, peso_neto_carga, cant_horas,
                     patente_vehiculo, cuit_transportista):
        "Obtener N�mero de CTG"
        try:
            ret = wsctg.solicitar_ctg(
                self.client, self.Token, self.Sign,
                self.Cuit,
    ...
    def ConfirmarCTG(self, numero_carta_de_porte, numero_CTG, cuit_transportista, peso_neto_carga):
        "Confirma n�mero de CTG"
        try:
            ret = wsctg.confirmar_ctg(
                self.client, self.Token, self.Sign, self.Cuit,
                numeroCartaDePorte=numero_carta_de_porte,
...
class wDigDepFiel:
    ...
    _public_methods_ = [
        'Conectar', 'Dummy', 'AvisoDigit', 'AvisoRecepAcept',
        'IniciarAviso', 'AgregarFamilia']
    _public_attrs_ = ['Token', 'Sign', 'Cuit',
                      'AppServerStatus', 'DbServerStatus', 'AuthServerStatus',
                      'XmlRequest', 'XmlResponse', 'Version',
                      'CodError', 'DescError']
    ...
    def AvisoRecepAcept(self, tipo_agente, rol,
        ...
        "Aviso de Recepci�n y Aceptaci�n"
        try:
            ret = wdigdepfiel.aviso_recep_acept(
                self.client, self.Token, self.Sign, self.Cuit, tipo_agente,
                rol, nro_legajo, cuit_declarante, cuit_psad, cuit_ie,
                codigo, fecha_hora_acept, ticket)
    ...
    def IniciarAviso(self):
        "Inicializar familias para Aviso de Digitalizaci�n"
        self.familias = []

    def AgregarFamilia(self, codigo, cantidad):
        "Agregar famila para Aviso de Digitalizaci�n"
        self.familias.append(
            {'Familia': {'codigo': codigo, 'cantidad': cantidad}})
    ...
    def AvisoDigit(self, tipo_agente, rol,
        ...
        "Aviso de Digitalizaci�n"
        try:
            familias = self.familias
            ret = wdigdepfiel.aviso_digit(
                self.client, self.Token, self.Sign, self.Cuit, tipo_agente,
                rol, nro_legajo, cuit_declarante, cuit_psad, cuit_ie, cuit_ata,
...
if __name__ == '__main__':
    ...
    warnings.warn(
        "Este modulo es obsoleto (mantenido por compatibilidad).\n"
