...
"""M�dulo para obtener CAE/CAEA, c�digo de autorizaci�n electr�nico
webservice WSFEv1 de AFIP (Factura Electr�nica Nacional - Proyecto
Version 1 - 2.10) Seg�n RG 2485/08, RG 2757/2010, RG 2904/2010 y
RG2926/10 (CAE anticipado), RG 3067/2011 (RS - Monotributo), RG 3571/2013
(Responsables inscriptos IVA), RG 3668/2014 (Factura A IVA F.8001),
RG 3749/2015 (R.I. y exentos) RG 4004-E Alquiler de inmuebles con
destino casa habitaci�n). RG 4109-E Venta de bienes muebles registrables.
M�s info:
...
import sys
from .utils import (
    verifica, inicializar_y_capturar_excepciones, BaseWS, get_install_dir
)
import logging
...
# WSDL = "https://www.sistemasagiles.com.ar/simulador/wsfev1/call/soap?WSDL=
# None"
...
class WSFEv1(BaseWS):
    ...
    _public_methods_ = [
        'CrearFactura', 'AgregarIva', 'CAESolicitar',
    ...
    _public_attrs_ = [
        ...
        'LanzarExcepciones', 'Resultado', 'Obs', 'Observaciones', 'Traceback',
        'InstallDir', 'CAE', 'Vencimiento', 'Eventos', 'Errores', 'ErrCode',
        'ErrMsg', 'Reprocesar', 'Reproceso', 'EmisionTipo', 'CAEA', 'CbteNro',
    ...
    def inicializar(self):
        ...
        self.AppServerStatus = self.DbServerStatus = \
            self.AuthServerStatus = None
        self.Resultado = self.Motivo = self.Reproceso = ''
        self.LastID = self.LastCMP = self.CAE = self.CAEA = \
            self.Vencimiento = ''
        self.CbteNro = self.CbtDesde = self.CbtHasta = self.PuntoVenta = None
        self.ImpTotal = self.ImpIVA = self.ImpOpEx = self.ImpNeto = \
            self.ImptoLiq = self.ImpTrib = None
        self.EmisionTipo = self.Periodo = self.Orden = ""
        self.FechaCbte = self.FchVigDesde = self.FchVigHasta = \
    ...
    def __analizar_errores(self, ret):
        ...
        if 'Errors' in ret:
            ...
            self.ErrCode = ' '.join(
                [str(error['Err']['Code']) for error in errores]
            )
            self.ErrMsg = '\n'.join(self.Errores)
        if 'Events' in ret:
            events = ret['Events']
    ...
    def Dummy(self):
        ...
        log.warn("### Calling Metodo Dummy para verificacion de \
        funcionamiento de la infraestructura (FEDummy)")
        result = self.client.FEDummy()['FEDummyResult']
        log.warn("FEDummy Result: " + str(result))
    ...
    def CrearFactura(
        self, concepto=1, tipo_doc=80, nro_doc="", tipo_cbte=1, punto_vta=0,
        cbt_desde=0, cbt_hasta=0, imp_total=0.00, imp_tot_conc=0.00,
        imp_neto=0.00, imp_iva=0.00, imp_trib=0.00, imp_op_ex=0.00,
        fecha_cbte="", fecha_venc_pago=None, fecha_serv_desde=None,
    ...
    def EstablecerCampoFactura(self, campo, valor):
        if campo in self.factura or campo in (
            'fecha_serv_desde', 'fecha_serv_hasta', 'caea', 'fch_venc_cae'
        ):
    ...
    def AgregarCmpAsoc(self, tipo=1, pto_vta=0, nro=0, cuit=None, fecha=None,
                       **kwargs):
    ...
    def AgregarComprador(
        self, doc_tipo=80, doc_nro=0, porcentaje=100.00, **kwarg
    ):
    ...
    def CAESolicitar(self):
        ...
        (FECAESolicitar)"""
        log.warn("### Calling Metodo de autorizacion de comprobantes \
                electronicos por CAE (FECAESolicitar)")
        f = self.factura
        log.warn("### Factura: " + str(f))

        try:
            ret = self.client.FECAESolicitar(
                Auth={
                    'Token': self.Token.decode('utf8'),
                    'Sign': self.Sign.decode('utf8'),
                    'Cuit': self.Cuit
                },
                FeCAEReq={
                    'FeCabReq': {
            ...
            log.warn('explotando el FECAESolicitar')
            msg = str(e)
            log.warn('msg: %s' % msg)

        result = ret['FECAESolicitarResult']
        log.warn("FECAESolicitar Result: " + str(result))
        if 'FeCabResp' in result:
            ...
            if self.Reprocesar and (
                'Errors' in result or 'Observaciones' in fedetresp
            ):
    ...
    def CompTotXRequest(self):
        ...
        log.warn("### Calling Recuperador de cantidad máxima de registros \
        FECAESolicitar - FECAEARegInformativo (FECompTotXRequest)")
        ...
        log.warn("FECompTotXRequest Result: " + str(result))
    ...
    def CompUltimoAutorizado(self, tipo_cbte, punto_vta):
        "Recuperador de ultimo valor de comprobante registrado \
                (FECompUltimoAutorizado)"
        log.warn("### Calling Recuperador de ultimo valor de comprobante \
        registrado (FECompUltimoAutorizado)")
        ...
        log.warn("FECompUltimoAutorizado Result: " + str(result))
    ...
    def CompConsultar(self, tipo_cbte, punto_vta, cbte_nro, reproceso=False):
        "M�todo para consultar Comprobantes Emitidos y su c�digo \
                (FECompConsultar)"
        log.warn("### Calling Metodo para consultar Comprobantes Emitidos \
        y su codigo (FECompConsultar)")
        ...
        log.warn("FECompConsultar Result: " + str(result))
        if 'ResultGet' in result:
            ...
            if reproceso:
                ...
                verificaciones = {
                    ...
                    'CbteHasta': f['cbt_hasta'],
                    'CbteFch': f['fecha_cbte'],
                    'ImpTotal': f['imp_total'] and (
                        float(f['imp_total']) or 0.0
                    ),
                    'ImpTotConc': f['imp_tot_conc'] and (
                        float(f['imp_tot_conc']) or 0.0
                    ),
            ...
            cod_aut = resultget['CodAutorizacion'] and \
                str(resultget['CodAutorizacion']) or ''  # 60423794871430L
    ...
    def ParamGetTiposCbte(self, sep="|"):
        """Recuperador de valores referenciales de c�digos de Tipos de
        Comprobantes"""
        log.warn("### Calling Recuperador de valores referenciales de \
        códigos de Tipos de comprobante (FEParamGetTiposCbte)")
        ...
        log.warn("FEParamGetTiposCbte Result: " + str(res))
        return [
            (
                "%(Id)s\t%(Desc)s\t%(FchDesde)s\t%(FchHasta)s" % (
                    p['CbteTipo']
                )
            ).replace("\t", sep) for p in res.get('ResultGet', [])
        ]
    ...
    def ParamGetTiposConcepto(self, sep="|"):
        ...
        log.warn("### Calling Recuperador de valores referenciales de \
        códigos de Tipos de Conceptos FEParamGetTiposConcepto)")
        ...
        log.warn("FEParamGetTiposConcepto Result: " + str(res))
        return [
            (
                "%(Id)s\t%(Desc)s\t%(FchDesde)s\t%(FchHasta)s" % (
                    p['ConceptoTipo']
    ...
    def ParamGetTiposDoc(self, sep="|"):
        ...
        log.warn("FEParamGetTiposDoc Result: " + str(res))
        return [
            (
                "%(Id)s\t%(Desc)s\t%(FchDesde)s\t%(FchHasta)s" % (
                    p['DocTipo']
    ...
    def ParamGetTiposIva(self, sep="|"):
        ...
        log.warn("### Calling Recuperador de valores referenciales de \
        codigos de Tipos de Alicuotas (FEParamGetTiposIva)")
        ...
        log.warn("FEParamGetTiposIva Result: " + str(res))
        return [
            (
                "%(Id)s\t%(Desc)s\t%(FchDesde)s\t%(FchHasta)s" % (
                    p['IvaTipo']
    ...
    def ParamGetTiposMonedas(self, sep="|"):
        ...
        log.warn("### Calling Recuperador de valores referenciales de \
        codigos de Tipos de Monedas (FEParamGetTiposMonedas)")
        ...
        log.warn("FEParamGetTiposMonedas Result: " + str(res))
        return [
            (
                "%(Id)s\t%(Desc)s\t%(FchDesde)s\t%(FchHasta)s" % (
                    p['Moneda']
    ...
    def ParamGetTiposOpcional(self, sep="|"):
        """Recuperador de valores referenciales de c�digos de Tipos de datos
        opcionales"""
        log.warn("### Calling Recuperador de valores referenciales de \
        codigos de Tipos de datos Opcionales (FEParamGetTiposOpcional)")
        ...
        log.warn("FEParamGetTiposOpcional Result: " + str(res))
        return [
            (
                "%(Id)s\t%(Desc)s\t%(FchDesde)s\t%(FchHasta)s" % (
                    p['OpcionalTipo']
    ...
    def ParamGetTiposTributos(self, sep="|"):
        """Recuperador de valores referenciales de c�digos de Tipos de
        Tributos Este m�todo permite consultar los tipos de tributos
        habilitados en este WS"""
        log.warn("### Calling Recuperador de valores referenciales de \
        codigos de Tipos de Tributos (FEParamGetTiposTributos)")
        ...
        log.warn("FEParamGetTiposTributos Result: " + str(res))
        return [
            (
                "%(Id)s\t%(Desc)s\t%(FchDesde)s\t%(FchHasta)s" % (
                    p['TributoTipo']
    ...
    def ParamGetTiposPaises(self, sep="|"):
        ...
        Este m�todo permite consultar los tipos de tributos habilitados en
        este WS"""
        log.warn("### Calling Recuperador de valores referenciales de \
                 codigos de Paises")
        ...
        log.warn("FEParamGetTiposPaises Result: " + str(res))
        return [
            (
                "%(Id)s\t%(Desc)s" % (
                    p['PaisTipo']
    ...
    def ParamGetCotizacion(self, moneda_id):
        ...
        log.warn("### Recuperador de cotizaci�n de moneda \
        ...
        log.warn("FEParamGetCotizacion Result: " + str(res))
    ...
    def ParamGetPtosVenta(self, sep="|"):
        ...
        log.warn("### Calling Recuperador de los puntos de venta asignados a \
        Facturaci�n Electronica que soporten CAE y CAEA via Web Services \
        (FEParamGetPtosVenta)")
        ...
        log.warn("FEParamGetPtosVenta Result: " + str(res))
        return [
            (
                """%(Nro)s\tEmisionTipo:%(EmisionTipo)s\tBloqueado:
                %(Bloqueado)s\tFchBaja:%(FchBaja)s""" % (p['PtoVenta'])
            ).replace("\t", sep) for p in res.get('ResultGet', [])
        ]
...
def main():
    ...
    if DEBUG:
        from py3simplesoap.client import __version__ as soapver
        print("py3simplesoap.__version__ = ", soapver)
    ...
    if "--prueba" in sys.argv:
        ...
        concepto = 2 if (
            '--usados' not in sys.argv and '--rg4109' not in sys.argv
        ) else 1
        ...
        for i in range(reg_x_req):
            ...
            # datos opcionales para RG 3668 Impuesto al Valor Agregado -
            # Art.12:
            if '--rg3668' in sys.argv:
                ...
                # Car�cter del Firmante
                wsfev1.AgregarOpcional(7, "01")
        ...
        if "--consultar" in sys.argv:
            ...
            p_assert_eq(
                wsfev1.ObtenerCampoFactura('imp_total'), float(imp_total)
            )
            # comparar primer alicuota de IVA
            p_assert_eq(wsfev1.ObtenerCampoFactura('iva', 0, 'importe'), 21)
            # comparar primer tributo
            p_assert_eq(
                wsfev1.ObtenerCampoFactura('tributos', 0, 'importe'), 1
            )
            # comparar primer opcional
            if '--rg3668' in sys.argv:
                p_assert_eq(
                    wsfev1.ObtenerCampoFactura('opcionales', 0, 'valor'),
                    "02"
                )
    ...
    if "--solicitar-caea" in sys.argv:
        ...
        if DEBUG:
            print(
                "Solicitando CAEA para periodo %s orden %s" % (periodo, orden)
            )
...
# busco el directorio de instalaci�n (global para que no cambie si usan otra
# dll)
