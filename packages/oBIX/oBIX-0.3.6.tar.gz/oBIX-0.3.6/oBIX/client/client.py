...
class Client:
    ...
    def __init__(self, host, user_name, password, port=443, enable_proxy=False, proxy_dict=None):
    ...
    def __get_url(self, url_path: str, operation=""):
        ...
        if self.__port == 80:
            ...
            host_url = "https://{0}:{1}".format(self.__host, self.__port)
        if operation == "":
            url = host_url + "/obix{0}".format(url_path)
        else:
            url = host_url + "/obix{0}{1}".format(url_path, operation)
        return url
