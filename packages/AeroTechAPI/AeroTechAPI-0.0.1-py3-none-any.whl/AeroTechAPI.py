...
class Decorators:
    ...
    def accept_multiple_axes(_func=None, *, transform_second_arg=False):
        '''
        This function is used to decorate a function that accepts an axes argument.
    ...
    def validate_axes(original_function):
        '''
        This function is used to decorate a function that accepts an axes argument.
        It validates the axes against a list of valid axes stored in the 
        A3200Controller Class. Raises a ValueError if any axis is not a valid.
        Args:
            original_function (function): Function that accepts an axes argument
        Returns:
        ...
        '''
        def wrapper_function(self, axes, *args, **kwargs):
            axes_valid = all([str(axis) in A3200Controller.valid_axes for axis in axes])
            if axes_valid:
                return original_function(self, axes, *args, **kwargs)
            else:
...
class A3200Controller:
    ...
    def load_camtable(self, master_axis, slave_axis, path):
        self.send_command(f'LOADCAMTABLE {master_axis}, 1, {slave_axis}, 1, 1, "{path}" NOWRAP')
...
class MultiAxes:
    ...
    def __add__(self, other):
        if isinstance(other, Axis):
            identifiers = [str(axis) for axis in self._axes] + [str(other)]
