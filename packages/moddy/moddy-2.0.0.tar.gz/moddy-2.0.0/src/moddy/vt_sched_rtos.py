...
class VtSchedRtos(SimPart):
    ...
    * Threads with same priority will be scheduled round robin \
    ...
    def run_vthread_til_sys_call(self, v_thread):
        ...
        #
        ret_val = self._sc_call_event.wait(
            timeout=self.sched_vthread_com_timeout
        )
        ...
        if ret_val:
            ...
            print(
                "Timeout waiting for v_thread %s to issue sys_call" % v_thread
            )
    ...
    def _update_all_state_indicators(self):
        ...
        for v_thread in self._list_vthreads:
            ...
            if (
                v_thread.sched_data.last_app_status is None
                or v_thread.sched_data.last_app_status != new_app_status
            ):

                v_thread.set_state_indicator(
                    new_app_status[1], new_app_status[2]
                )
    ...
    def _wake(self, v_thread, event):
        ...
        if v_thread.sched_data.wait_events is not None:
            ...
            if (
                event is None
                or event in v_thread.sched_data.wait_events
                or (
                    io_port_event is not None
                    and io_port_event in v_thread.sched_data.wait_events
                )
            ):
    ...
    def schedule(self):
        ...
        if self._running_vthread is None:
            ...
            if highest_ready_vt is None:
                ...
                if (
                    highest_ready_vt.sched_data.prio
    ...
    def vt_state_machine(self, v_thread, event):
        ...
        if old_state == "READY":
            ...
            if event == "wait":
                ...
                elapsed = (
                    self._sim.time() - v_thread.sched_data.busy_start_time
                )
    ...
    def vt_remote_control(self, v_thread, action):
        ...
        if action == "start":
            ...
            raise RuntimeError(
                "vTRemoteControl %s bad action %s" % (v_thread, action)
            )
        self.schedule()
        self._update_all_state_indicators()
    ...
    def sys_call(self, v_thread, call, args):
        ...
        ret_val = v_thread.sched_data.return_event.wait(
            ...
            if self.sched_vthread_com_timeout is not None
            else None
        )
...
class VSimpleProg(VThread):
    ...
    :param sim: Simulator instance
    :param v_thread_args: see :class:`.VThread` for parameters
