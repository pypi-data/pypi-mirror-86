...
class DotFsm(object):
    ...
    def fsm_gen(self, level, fsm, name="", is_sub_fsm=False):
        ...
        for state, list_trans in dict_transitions.items():
            if state == "":
                sstr = "%s [style=invisible];" % map_state_names(name, state)
                if is_sub_fsm:
                    sstr = None
            elif state == "ANY":
                sstr = (
                    '%s [label="from any state" shape=none];'
                    % map_state_names(name, state)
                )
            else:
                sstr = "%s [label=%s];" % (map_state_names(name, state), state)
            if sstr is not None:
        ...
        for from_state, list_trans in dict_transitions.items():
            for trans in list_trans:
                ...
                if sub_fsm_cls is not None:
                    ...
                    lines.append(
                        [level, "subgraph cluster_%s {  " % (sub_fsm_name)]
                    )
                    lines.append([level + 1, 'label="%s";' % (sub_fsm_name)])

                    # draw subfsm
                    sub_lines, sub_initial_state = self.fsm_gen(
                        level + 1, sub_fsm, name=sub_fsm_name, is_sub_fsm=True
                    )
                    lines += sub_lines
                    lines.append([level, "}"])
