...
class SimFsmPart(SimPart):
    ...
    def __init__(
        self, sim, obj_name, fsm, parent_obj=None, status_box_repr_map=None
    ):
    ...
    def create_ports(self, ptype, list_port_names):
        ...
        if ptype == "in":
            ...
            for port_name in list_port_names:
                setattr(
                    self, port_name, self.new_io_port(port_name, self.msg_recv)
                )
        else:
    ...
    def create_timers(self, list_timer_names):
        ...
        for tmr_name in list_timer_names:
            setattr(self, tmr_name, self.new_timer(tmr_name, self.tmr_expired))
