 ...
 PyObject * createInput_wrapper(PyObject * self, PyObject * args)
{
    ...
    if (value == Py_None){
        result = igs_createInput(name, type, NULL, 0);
        ...
        result = igs_createInput(name, type, value_c, strlen(value_c));
    }else if (type == IGS_INTEGER_T){
        int value_c;
        if (!PyArg_ParseTuple(args, "siii", &name, &_type, &value_c, &size)) {
            return NULL;
        }
        result = igs_createInput(name, type, &value_c, sizeof(int));
    }else if (type == IGS_DOUBLE_T){
        double value_c;
        if (!PyArg_ParseTuple(args, "sidi", &name, &_type, &value_c, &size)) {
            return NULL;
        }
        result = igs_createInput(name, type, &value_c, sizeof(double));
    }else{
        result = igs_createInput(name, type, value, size);
    } 
    return PyLong_FromLong(result);
}
