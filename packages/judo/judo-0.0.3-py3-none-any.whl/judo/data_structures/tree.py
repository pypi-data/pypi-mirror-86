...
class NetworkxTree(BaseTree):
    ...
    def reset(self, root_id: NodeId = None, **kwargs) -> None:
        """
        Delete all the data currently stored and reset the internal state of \
        the tree and reset the root node with the provided data.
        """
        node_data = self._extract_data_from_states(index=0, only_node_data=True, **kwargs)
        self.reset_graph(root_id=to_node_id(root_id), node_data=node_data, epoch=-1)

    def get_states_ids(self, **kwargs) -> Tuple[Tensor, States]:
