...
class Coworker(object):
    ...
    def close(self):
        ...
        for p in self._current_task_processes.values():
            kill_process(p)

    def close_task_process(self, tid: str):
    ...
    def handle_canceled_tasks(self, canceled_task_ids):
        ...
        for tid in canceled_task_ids:
            self.close_task_process(tid)
            self._current_task_infos.pop(tid, None)
    ...
    def handle_added_tasks(self, added_task_ids, func):
        ...
        for tid in added_task_ids:
            ...
            task_zk_path = f"{self.namespace}/assign/{self.worker_id}/{tid}"
            updated_func = partial(self.handle_updated_task, task_zk_path, handle_func=func)
            self.zk.DataWatch(task_zk_path)(updated_func)

    def up_task_process(self, task_id, func):
        task: Task = self._current_task_infos.get(task_id)
        if task is None:
            try:
                task_content, stat = self.zk.get(f"{self.namespace}/assign/{self.worker_id}/{task_id}")
                task = json.loads(task_content.decode('utf-8'))
                task['version'] = stat.version
    ...
    def handle_updated_task(self, path: str, data, stat, handle_func=None):
        next_task = None
        try:
            next_task: Task = json.loads(data.decode('utf-8'))
            next_task['version'] = stat.version
            assert Path(path).name == next_task['tid']
        except JSONDecodeError as e:
            log.exception(e.msg)

        if next_task is None:
            log.exception("Get task failed....")
            return None

        tid = next_task['tid']
        curr_task_info: Task = self._current_task_infos.get(tid, None)

        if curr_task_info is not None and next_task['version'] > curr_task_info['version']:
            log.info(f"task<{tid}> data version[{next_task['version']}] update, upgrade")
            self.close_task_process(tid)
            self._current_task_infos[tid] = next_task
            p = self.up_task_process(tid, handle_func)
            state: TaskState = TaskState(tid=tid, state=STATE_RUNNING, ip=ip, pid=p.pid)
            self.update_task_state(tid, state)
