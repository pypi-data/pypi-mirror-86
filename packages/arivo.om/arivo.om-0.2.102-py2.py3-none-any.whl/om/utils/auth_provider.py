...
class ExternalAuthorizationProvider(object):
    ...
    def _handle_has_access_from_message(self, message):
        ...
        for medium in mediums:
            ...
            results.append(ResultEntry(medium=medium, access=result))
        return results
    ...
    def _handle_access_reject(self, message):
        ...
        for result in results:
            if result is False:
                ...
                access["id"] = result.medium.medium_id
                message = Message(
                    name=self.config.NAME,
                    type="auth",
                    id=result.medium.medium_id,
                    medium_type=result.medium.medium_type,
                    gateway=gateway,
                    access=result
                )
    ...
    def rpc_auth(self, request, message):
        ...
        if not results:
            return {"success": True, "name": self.config.NAME, "access": False}
        for result in results:
            if result.access:
                access = result.access
                access["id"] = result.medium.medium_id
        ...
        return {
            "success": True,
            "name": self.config.NAME,
            "id": results[0].medium.medium_id,
            "medium_type": results[0].medium.medium_type,
            "gateway": gateway,
            "access": False
        }
