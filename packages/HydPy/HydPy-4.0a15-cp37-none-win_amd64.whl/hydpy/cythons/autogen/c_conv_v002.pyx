    ...
    cpdef inline void calc_outputs_v2(self)  nogil:
        ...
        for idx_out in range(self.parameters.derived.nmboutputs):
            ...
            for idx_try in range(self.parameters.control.maxnmbinputs):
                ...
                if not isnan(self.sequences.fluxes.inputs[idx_in]):
                    if isinf(self.parameters.derived.weights[idx_out, idx_try]):
                        d_sumvalues_inf = d_sumvalues_inf + (self.sequences.fluxes.inputs[idx_in])
                        counter_inf = counter_inf + (1)
                    else:
                        d_sumweights = d_sumweights + (self.parameters.derived.weights[idx_out, idx_try])
                        d_sumvalues = d_sumvalues + ((                            self.parameters.derived.weights[idx_out, idx_try] * self.sequences.fluxes.inputs[idx_in]                        ))
            if counter_inf:
                self.sequences.fluxes.outputs[idx_out] = d_sumvalues_inf / counter_inf
            elif d_sumweights:
                self.sequences.fluxes.outputs[idx_out] = d_sumvalues / d_sumweights
            else:
                self.sequences.fluxes.outputs[idx_out] = nan
