...
class DatasetProfile:
    ...
    def track(self, columns, data=None):
        ...
        if data is not None:
            ...
            for column_name, data in list(columns.items()):
    ...
    def to_summary(self):
        ...
        self.validate()
        column_summaries = {
            name: colprof.to_summary() for name, colprof in list(self.columns.items())
        }
    ...
    def _column_message_iterator(self):
        self.validate()
        for col in list(self.columns.items()):
    ...
    def validate(self):
        ...
        assert all(isinstance(tag, str) for tag in list(self.tags.values()))
    ...
    def to_protobuf(self) -> DatasetProfileMessage:
        ...
        return DatasetProfileMessage(
            ...
            columns={k: v.to_protobuf() for k, v in list(self.columns.items())},
        )
    ...
    def from_protobuf(message: DatasetProfileMessage):
        ...
        return DatasetProfile(
            ...
            columns={
                k: ColumnProfile.from_protobuf(v) for k, v in list(message.columns.items())
            },
...
def flatten_dataset_quantiles(dataset_summary: DatasetSummary):
    ...
    """
    quants = {}
    for col_name, col in list(dataset_summary.columns.items()):
        try:
...
def get_dataset_frame(dataset_summary: DatasetSummary, mapping: dict = None):
    ...
    col_out = {}
    for _k, col in list(dataset_summary.columns.items()):
