...
def getLogger(name=None, formatter="%(asctime)s %(levelname)s %(message)s", logPath="logs", level=logging.DEBUG):
    """
    获取logger
...
class ConsoleLogger:
    ...
    def debug(self, msg):
        record = self._formatMsg("DEBUG", msg)
        print(record)

    def info(self, msg):
        print(self._formatMsg("INFO", msg))

    def warn(self, msg):
        print(self._formatMsg("WARN", msg))

    def error(self, msg):
        print(self._formatMsg("ERROR", msg))

    def _formatMsg(self, level, msg):
        return self.formatter %{"asctime":time.asctime(time.localtime(time.time())), "levelname":level, "message":msg}
