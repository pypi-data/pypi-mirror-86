...
def load_state_vectors(statevectors, result, index):
    statevectors[index] = result.data.statevector.copy()
...
def get_qiskit_state_vectors(quantum_instance, feature_map, x, batchsize=None):
    ...
    parallel_run(load_state_vectors, itertools.repeat(statevectors), results.results, range(data_size))
    '''
