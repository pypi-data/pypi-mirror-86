...
class QSVM:
    ...
    def get_state_vectors(self, x, **kwargs):
        if self.circuit_type == 'quple':
            ...
            if 'batchsize' in kwargs:
                ...
                batchsize = None
            return get_qiskit_state_vectors(self.backend, self.encoding_circuit, x, batchsize=batchsize)
        else:
            return None
    ...
    def run(self, x_train, y_train, x_test, y_test, 
        ...
        if logger:
            logger.info('Evaluating state vectors for train data...')             
        train_state_vectors = self.get_state_vectors(x_train, **kwargs)
        if logger:
            logger.info('Evaluating kernel matrix for train data...')
        train_kernel_matrix = self.get_kernel_matrix(train_state_vectors, train_state_vectors)
        if logger:
        ...
        if logger:
            logger.info('Evaluating state vectors for test data...')             
        test_state_vectors = self.get_state_vectors(x_test, **kwargs)
        if logger:
            logger.info('Evaluating kernel matrix for test data...')
