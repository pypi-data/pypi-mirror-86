...
from quple.utils.mathext import abs2, DataPrecision
...
class QSVM:
    ...
    def __init__(self, encoding_circuit:Union[quple.ParameterisedCircuit, qiskit.QuantumCircuit], 
                 precision:DataPrecision=DataPrecision.DOUBLE, **kwargs):
    ...
    def backend(self):
        if self._precision == DataPrecision.SINGLE:
            backend.backend.set_options(precision='single')
        elif self._precision == DataPrecision.DOUBLE:
            backend.backend.set_options(precision='double')
        else:
    ...
    def get_kernel_matrix(state_vectors_left, state_vectors_right):
        return abs2(state_vectors_left @ state_vectors_right.conjugate().T)
    ...
    def run(self, x_train, y_train, x_test, y_test, 
            random_seed = 0, logger=None, precision='double', **kwargs):
        ...
        train_kernel_matrix = self.get_kernel_matrix(train_state_vectors, train_state_vectors)
        if precision =='single':
            train_kernel_matrix = train_kernel_matrix.astype('float32')   
        if logger:
        ...
        test_kernel_matrix = self.get_kernel_matrix(test_state_vectors, train_state_vectors)
        if precision =='single':
            test_kernel_matrix = test_kernel_matrix.astype('float32')    
        
        train_state_vectors = None # Free memory
        test_state_vectors = None  # Free memory 

        logger.on_train_end(clf, train_kernel_matrix, test_kernel_matrix, y_test)
        return clf