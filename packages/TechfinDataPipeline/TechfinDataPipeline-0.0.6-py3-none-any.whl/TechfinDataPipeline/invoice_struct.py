...
class InvoiceStruct():
    ...
    def add_payment(self, org_id, invoice_id, installment_id, value, code, sequence_code, docissue, duedate, paymentdate, issuedate):
        ...
        if code in self.predicted_codes:
            ...
            if code in ['90', '95']:
                self.struct[org_id][invoice_id][installment_id].update({'anticipated':True, 'anticipation_date':issuedate})
            elif code in ['91', '96']:
                self.struct[org_id][invoice_id][installment_id].update({'anticipated':False, 'anticipation_date':None})
