...
class StreamCache:
    ...
    def __repr__(self):
        rep = str({"name": self.name, "service": self.service, "fields": dict(list(self.items()))})
        return rep

    def __str__(self):
        if self.service:
            return str("|".join([self.service, self.name]) + "[" + ",".join([f"{f}:{v}" for f, v in self.items()]) + "]")
        else:
            return str("|".join(["Unknown service", self.name]) + "[" + ",".join([f"{f}:{v}" for f, v in self.items()]) + "]")
