...
def get_user_permissions(user):
    ...
    if user.has_perm("afat.manage_afat"):
        can_manage = True
    ...
    if (
        user.has_perm("afat.manage_afat")  # has all permissions
        or user.has_perm("afat.add_afatlink")  # can add fatlinks
        or user.has_perm("afat.change_afatlink")  # can change fatlinks
        or user.has_perm("afat.delete_afatlink")  # can delete fatlinks
    ):
        has_fatlink_permissions = True
        can_manipulate_fatlink = True
    ...
    if (
        user.has_perm("afat.manage_afat")  # has all permissions
        or user.has_perm("afat.add_afat")  # can add fats
        or user.has_perm("afat.delete_afat")  # can delete fats
    ):
        has_fat_permissions = True
        can_manipulate_fat = True
    ...
    # can add fatlinks
    if user.has_perm("afat.manage_afat") or user.has_perm("afat.add_afatlink"):
        can_add_fatlink = True

    # Can change fatlinks
    if user.has_perm("afat.change_afatlink"):
        can_manipulate_fatlink = True
        can_change_fatlink = True

    # can delete fatlinks
    if user.has_perm("afat.delete_afatlink"):
        can_manipulate_fatlink = True
        can_delete_fatlink = True

    # can add fats
    if user.has_perm("afat.add_afat"):
        can_add_fat = True

    # can delete fats
    if user.has_perm("afat.delete_afat"):
        can_manipulate_fat = True
        can_delete_fat = True

    # can view other corp stats
    if user.has_perm("afat.stats_corp_own"):
        can_view_owncorpstats = True

    # can view other corp stats
    if user.has_perm("afat.stats_corp_other"):
        can_view_other_corpstats = True

    # can view other character stats
    if user.has_perm("afat.stats_char_other"):
