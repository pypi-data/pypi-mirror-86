...
def create_shapley_schema(
    ...
    example = x.head(1)
    data = create_explainer_schema(example, explainer, input_descriptions,)
...
def create_explainer_schema(
    ...
    example = {
        "inputs": df_to_dict(explainer_schema_example),  # schema_example,
...
def create_importance(
    ...
    if kind == "regression":
        chart, varlist = varimp(func, x, y, profile_columns, metric="Rmse")
    elif kind == "binary":
        chart, varlist = varimp(func, x, y, profile_columns, metric="AUC")
    else:
