...
from dli.aws import match_partitions, filter_by_load_type_full
...
class StructuredDatasetModel(DatasetModel):
    ...
    def __custom_filter(
        ...
        paths: List[Tuple[str, int]] = \
            filter_by_load_type_full(paths, self.load_type)
    ...
    def list(
        ...
        * Files with the wrong extension for the dataset's type i.e for
        ...
        if filter_path is not None:
            warnings.warn(
                'Parameter `filter_path` will be deprecated in a future '
                'release, please use `partitions` instead.',
    ...
    def download(
        ...
        if filter_path is not None:
            warnings.warn(
                'Parameter `filter_path` will be deprecated in a future release, please use `partitions` instead.',
                PendingDeprecationWarning
            )
