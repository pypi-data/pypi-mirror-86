...
class FileSystem:
    ...
    def list(cls,
        ...
        not_links = []
        for path in paths:
            if include_files and path.exists() and path.is_file():
                not_links.append(File(path))
            elif include_folders and path.exists() and path.is_folder():
                not_links.append(Folder(path))
            # TODO: Handle links
        return not_links
