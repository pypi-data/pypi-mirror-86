...
class CoCitation:
    ...
    def __init__(
        self, articles_list: List[str], sd_api_key: str = "", graph: str = "", wait=None
    ):
        """
    ...
    def create_citation_graph(self, articles_list: List[str]) -> nx.Graph:
        ...
        for article in articles_list:
            ...
            citations = self.get_citation_journal(article)
    ...
    def get_journal(self, article_url: str) -> Union[str, "NotImplemented"]:
        """
        ...
        if "arxiv" in article_url:
            ...
            return requests.get(url).json()["venue"]
        if "sciencedirect" in article_url:
            ...
            journal = (
                ...
                .find("prism:publicationname")
                .text
            )

            return (
                self.abbreviations[journal.lower()]
                if journal.lower() in self.abbreviations
                else journal
            )

        raise NotImplementedError("URL {} not implemented".format(article_url))

    def get_citation_journal(self, article_url: str) -> List[str]:
        """
        Get all journal citations for an article

        This function does two things:

        1. Get the citations
        2. For each citation, get the journal

        Args:
            article_url (str): The URL of the article. At the moment only arXiv,
                ScienceDirect and PubMed are supported
        Returns:
            list: The list of journals cited
        """
        if "pubmed" in article_url:
            pmid = article_url.split("/")[-1]
            url = (
            ...
            return [
                self.get_journal("https://www.ncbi.nlm.nih.gov/pubmed/" + idx.text)
