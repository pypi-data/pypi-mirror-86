...
stripeSi = rm.Material('Si', rho=2.33, t=1)
stripeSiO2 = rm.Material(('Si', 'O'), quantities=(1, 2), rho=2.2, t=1)
stripeIr = rm.Material('Ir', rho=22.42, t=1)


def build_beamline(nrays=raycing.nrays, hkl=(1, 1, 1), stripe='Si',
                   eMinRays=None, eMaxRays=None):
    filterDiamond = rm.Material('C', rho=3.52, kind='plate', t=1)
    ...
    si_1 = rm.CrystalSi(hkl=hkl, tK=-171+273.15, t=0.1)
    si_2 = rm.CrystalSi(hkl=hkl, tK=-140+273.15, t=0.1)
    height = 0
...
#        xPrimeMax=0.22, zPrimeMax=0.06, nx=40, nz=10)
    ...
    beamLine.dcmX1 = roe.OE(
        beamLine, 'DCM-X1', [0, 27060, height], surface='Si111', material=si_1,
        limPhysX=(-10, 10), limPhysY=(-20, 20), alarmLevel=0.)

    beamLine.dcmX2 = roe.OE(
        beamLine, 'DCM-X2', [0, 27060, height], surface='Si111', material=si_2,
        positionRoll=np.pi,
        limPhysX=(-10, 10), limPhysY=(-30, 30), alarmLevel=0.)
    ...
    beamLine.slitEH = ra.RectangularAperture(
        ...
        ('left', 'right', 'bottom', 'top'), [-1, 1, -1, 1], alarmLevel=0.5)
...
#    beamFSMVCM = beamLine.fsmVCM.expose(beamVCMglobal)
    ...
    beamDCMX1global, beamDCMX1local = beamLine.dcmX1.reflect(beamVCMglobal)
    beamDCMX1local.absorb_intensity(beamVCMglobal)
    beamDCMX2global, beamDCMX2local = beamLine.dcmX2.reflect(beamDCMX1global)

#    beamBSBlocklocal = beamLine.BSBlock.propagate(beamDCMglobal)
    beamSlitAfterDCMlocal = beamLine.slitAfterDCM.propagate(beamDCMX2global)
#    beamFSMDCM = beamLine.fsmDCM.expose(beamDCMglobal)

    beamVFMglobal, beamVFMlocal = beamLine.vfm.reflect(beamDCMX2global)
...
#               'beamFSMVCM': beamFSMVCM,
               'beamDCMX2global': beamDCMX2global,
               'beamDCMX1local': beamDCMX1local,
               'beamDCMX2local': beamDCMX2local,
...
def align_beamline(beamLine, pitch=None, bragg=None, energy=9000.,
    ...
    p = raycing.distance_xy(beamLine.dcmX1.center, beamLine.vcm.center)
    beamLine.dcmX1.center[2] = beamLine.height + p*math.tan(2*pitch)
    aCrystal = beamLine.dcmX1.material
    dSpacing = aCrystal.d
    ...
    print('DCM.realThetaAngle = DCM.bragg - 2*VCM.pitch = {0:.6f} deg'.format(
          math.degrees(theta)))
    beamLine.dcmX1.pitch = bragg
    beamLine.dcmX2.pitch = -bragg
    ...
    beamLine.dcmX2.center[1] = beamLine.dcmX1.center[1] +\
        fixedExit/np.tan(2*bragg)
    beamLine.dcmX2.center[2] = beamLine.dcmX1.center[2] + fixedExit
    print('DCM.pitch = {0:.6f} mrad'.format(beamLine.dcmX1.pitch*1e3))
    print('DCM.roll = {0:.6f} mrad'.format(beamLine.dcmX1.roll*1e3))
    print('DCM.yaw = {0:.6f} mrad'.format(beamLine.dcmX1.yaw*1e3))
    print('DCM.z = {0:.3f}'.format(beamLine.dcmX1.center[2]))
...
#    s = abs(1. - qr / q) * p * aceptanceH / 2
    dx = beamLine.slitEH.opening[1] - beamLine.slitEH.opening[0]
    beamLine.slitEH.opening[0] = -dx/2
    beamLine.slitEH.opening[1] = dx/2
