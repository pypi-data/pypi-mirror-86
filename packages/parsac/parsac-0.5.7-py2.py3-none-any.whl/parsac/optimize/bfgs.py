...
def line_search(f, myfprime, xk, pk, gfk, old_fval, old_old_fval,
                args=(), c1=1e-4, c2=0.9, amax=50, alpha1=None):
    ...
    if alpha1==None: alpha1 = pymin(1.0,1.01*2*(phi0-old_old_fval)/derphi0)
...
def fmin_bfgs(f, x0, fprime=None, args=(), gtol=1e-5, norm=numpy.Inf,
    ...
    *See Also*:
    ...
    #Hk = numpy.zeros((N,N),dtype=float)
    #pert = numpy.zeros((N,),dtype=float)
    #for i in range(N):
    #    pert[i] = sqrt(_epsilon)
    #    Hk[i,i] = pert[i]*pert[i]/(f(x0+pert)+f(x0-pert)-2.*f(x0))
    #    pert[i] = 0.
    alpha1 = None
    #print Hk
    
    old_fval = f(x0)
    old_old_fval = old_fval + abs(old_fval)/5.
    xk = x0
    if retall:
        allvecs = [x0]
    sk = [2*gtol]
    ...
    while (gnorm > gtol) and (k < maxiter):
        ...
        alpha_k, fc, gc, old_fval, old_old_fval, gfkp1 = \
           line_search(f,myfprime,xk,pk,gfk,
                                  old_fval,old_old_fval, alpha1=alpha1)
