...
class Importer(object):
    ...
    def importer(self):
        ...
        kwargs = {
            ...
            'min_post_length': str(min_post_length),
        ...
        (before['settings'], after['settings']) = self.discourse.settings_set(**kwargs)
        (before['category'], _) = self.discourse.category_create(self.name_get())
        if len(self.mailman.info['description']) > min_post_length:
            (before['category-info'], after['category-info']) = self.discourse.category_info_set(
                self.name_get(), self.mailman.info['description'])
        else:
            logger.error(f"ignore list info: '{self.mailman.info['description']}' because it is "
                         f"less than {min_post_length} characters long")
