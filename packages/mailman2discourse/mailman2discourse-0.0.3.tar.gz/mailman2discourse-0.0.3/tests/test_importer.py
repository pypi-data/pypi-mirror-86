...
def test_main(test_options, capsys):
    i = importer.Importer(test_options)
    assert i.main() == 0
    try:
        assert i.discourse.category_get(i.name_get()) is not None
        assert i.discourse.group_get(i.group_moderators_name()) is not None
        captured = capsys.readouterr()
        assert "Type of root" in captured.out
    finally:
...
def test_users_add(test_options):
    i = importer.Importer(test_options)
...
def test_importer_moderation_no(test_options):
    ...
    try:
        ...
        mailman_write_config(f'{test_options.mailman_dir}/{test_options.list}', c)
        i.mailman.load()
...
def test_importer_category_info(test_options):
    i = importer.Importer(test_options)
    try:
        too_short_description = 'SHORT'
        c = i.mailman.load_pickle()
        c['description'] = too_short_description
        mailman_write_config(f'{test_options.mailman_dir}/{test_options.list}', c)
        i.mailman.load()

        before0, after0 = i.importer()
        assert 'category-info' not in before0

        long_description = 'This is an description long enough to be taken into account'
        c = i.mailman.load_pickle()
