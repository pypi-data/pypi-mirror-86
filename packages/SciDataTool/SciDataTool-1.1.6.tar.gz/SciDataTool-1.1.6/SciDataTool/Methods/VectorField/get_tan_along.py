...
def get_tan_along(self, *args, unit="SI", is_norm=False, axis_data=[]):
    ...
    *args: list of strings
    ...
    if "tangential" in self.components.keys():
        return_dict = self.components["tangential"].get_along(
        ...
        # Extract from DataND
        resultx = self.components["x"].get_along(
            args, unit=unit, is_norm=is_norm, axis_data=axis_data
        )
        resulty = self.components["y"].get_along(
            args, unit=unit, is_norm=is_norm, axis_data=axis_data
        )
