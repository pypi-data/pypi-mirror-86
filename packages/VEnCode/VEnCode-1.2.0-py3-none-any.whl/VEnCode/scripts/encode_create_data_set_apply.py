...
def check_if_in_set_promoters(chr_, start_, end_, coordinates_dict_):
    try:
        coordinates = coordinates_dict_[chr_]
...
def check_if_in_set_enhancers(chr_, start_, end_, coordinates_dict_):
...
if DATA_TYPE == "enhancers":
    ...
    FILE_NAME_PROMOTER = cv.test_promoter_file_name
...
for filename in tqdm(glob.glob(os.path.join(ENCODE_DATA_PATH, '*.bed'))):
    ...
    coordinates_set = set()
    for chromosome in file_data["Chromosome"].unique():
        ...
        for start, end in zip(data_filtered_chr["Start"], data_filtered_chr["End"]):
            coordinates_set.update(range(start, end + 1))
        coordinates_dict[chromosome] = coordinates_set
        coordinates_set = set()

    if DATA_TYPE == "promoters":
        data_matches = fantom_coord.apply(lambda row: check_if_in_set_promoters(row["Chromosome"], row["Start"],
                                                                                row["End"], coordinates_dict), axis=1)
    else:
        data_matches = fantom_coord.apply(lambda row: check_if_in_set_enhancers(row["Chromosome"], row["Start"],
                                                                                row["End"], coordinates_dict), axis=1)
...
filename = "ENCODE DNase expression in FANTOM5 {}_apply_nrm.csv".format(DATA_TYPE)
