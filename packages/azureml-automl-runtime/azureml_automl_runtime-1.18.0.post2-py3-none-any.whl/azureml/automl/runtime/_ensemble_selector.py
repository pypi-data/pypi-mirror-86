    ...
    def _compute_models_predictions(self):
        ...
        if self.training_type == constants.TrainingType.TrainAndValidation:
            ...
            for index, model_tuple in enumerate(self.fitted_models):
                ...
                train_labels = pipeline.classes_ if hasattr(pipeline, "classes_") else None  # type: ignore
            ...
            for index, model_tuple in enumerate(self.fitted_models):
                ...
                for split_index, (_, _, _, X_valid_fold, y_valid_fold,
                    ...
                    logger.info('Getting out of fold predictions of pipeline index {} for CV fold {}'.format(
                        index, split_index))
                    cv_pipelines = model_tuple[pipeline_tuple_index]
                    pipeline = cv_pipelines[split_index]  # type: ignore
    ...
    def _get_model_predictions(self, model, validation_set, trained_class_labels):
        if self.task_type == constants.Tasks.CLASSIFICATION:
            Contract.assert_value(
                ...
                "trained_class_labels",
                reference_code=ReferenceCodes._ENSEMBLE_SELECTOR_MISSING_TRAIN_LABELS
            )
            ...
            result = _scoring_utilities.pad_predictions(result, trained_class_labels, self.class_labels)
        else:
