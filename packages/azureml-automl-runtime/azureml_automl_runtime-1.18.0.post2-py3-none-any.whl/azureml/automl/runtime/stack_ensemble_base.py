...
class StackEnsembleBase(ensemble_base.EnsembleBase, ABC):
    ...
    def _create_stack_ensembles_train_validation(self,
        ...
        for model_index in selector.unique_ensemble:
            ...
            training_percent = int(child_run.get_properties().get('training_percent', '100'))
        ...
        scoring_meta_learner = self._train_meta_learner_for_scoring_ensemble_with_train_validate(
            ...
            sample_weight_meta_train, pipeline_spec_subsample_tuples, selector.dataset)
        ...
        for i, (scoring_base_learner, train_fraction) in enumerate(scoring_base_learners_fraction_tuples):
            if need_subsampling:
                ...
                runner.ClientRunner.fit_pipeline(
                    scoring_base_learner, X_train_sampled, y_train_sampled, sample_weight_sampled)
            else:
...
class Scorer:
    ...
    def __call__(self, estimator, X, y=None):
        ...
        scores = scoring.score_classification(y, y_pred_proba, [self._metric], cv_labels, cv_labels)
