...
class _MultiGrainForecastBase:
    ...
    def _raise_import_error_maybe(self, module_name: str) -> None:
        """
        Raise the exception if module_name can not be imported.

        :param module_name: The name of a module to try.
        :raise: ClientException if model can not be imported.
        """
        if module_name not in sys.modules:
            raise exceptions.ClientException._with_error(
                AzureMLError.create(
                    AutoMLImportError, target='forecasting_import',
