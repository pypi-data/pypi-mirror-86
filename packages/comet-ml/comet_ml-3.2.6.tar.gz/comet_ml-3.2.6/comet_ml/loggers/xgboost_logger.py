...
def _comet_xgboost_callback(experiment, env):
    ...
    try:
        if experiment.auto_param_logging:
            if not experiment._storage["xgboost"].get("env_parameter_set", False):
    ...
    try:
        if experiment.auto_param_logging:
            if not experiment._storage["xgboost"].get("env_model_parameter_set", False):
    ...
    try:
        if experiment.log_graph:
            if not experiment._storage["xgboost"].get("model_graph_set", False):
                booster = env.model
...
def _log_xgboost_train_parameters(experiment, xgboost_params):
    ...
    if not experiment._storage["xgboost"].get("train_parameter_set", False):
        params = {
