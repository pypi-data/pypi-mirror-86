...
from email_master.compat import as_string
...
class PGPConfig(object):
    ...
    def unlock(self, msg_as_string, parser_kwargs):
        ...
        if EmailMasterParser(base64_encode_bytes(str(mime_msg.msg).encode()), **parser_kwargs).parse()["subject"].startswith("Test 1d"):
            tw = 2
        ...
        if len(mime_msg.msg._payload) == 2 and message_type.signed:
            ...
            email_data = EmailMasterParser(base64_encode_bytes(as_string(mime_msg.msg).encode()), **parser_kwargs).parse()
