...
class SegmentData_2:
    ...
    def random_split_05(df):
        """ create 2 dataframes with random split 0.5-0.5,  without replacement 
        to be used in optimize_sampling_train_and_test """
        a = df.sample(frac=0.5, replace=False, random_state = 10)
        b = df.drop(a.index)  
        return a, b
    ...
    def calculate_uplift_test_canal_2(self, df):
        """
        ...
        uplift = SegmentData_2.delta_pc(a,b)
        
        return uplift


    def calculate_test_canal_otro_uplift(self, df):
        """
        Cálculo del uplift dado un dataset para datos de test de canal:
        Utiliza la columna de CANAL_FINAL y sn_conv_digital
        ...
        # compute delta uplift
        uplift = SegmentData_2.delta_pc(b,a)
        return uplift
        
    def optimize_sampling_train_and_test(self, df, number_of_trials):
        ...
        for i in range(number_of_trials) : # number of trials 
            df_train, df_test = SegmentData_2.random_split_05(df)
            ...
            deltas_u_pc[i] = abs(1 - abs(self.calculate_test_canal_otro_uplift(df_train) / self.calculate_test_canal_otro_uplift(df_test)))
        ...
        summary = pd.DataFrame({'00_rows': ['Full_dataset', 'Train_dataset', 'Test_dataset'], 
                                ...
                                '02_uplifts_pc': [self.calculate_test_canal_otro_uplift(i) for i in S]
                                })
...
##################Funciones para obtener grupos que maximicen el uplift######################
    ...
    def _regressor_split_summary(self, df_train, df_test, explanatory_var, cutoff):
        ...
        summary_T = pd.DataFrame({'00_groups':['T_base', 'T_groupA_var<' + str(round(cutoff,2)), 'T_groupB_var>=' + str(round(cutoff,2))], 
                                ...
                                '02_uplifts_pc': [self.calculate_test_canal_otro_uplift(i) for i in S_T]}) 
    ...
    def SelfOptimize_regressor(self, df_train, df_test, explanatory_var, q = 0.3):
        ...
        def compute_uplift_group_difference_pc(cutoff):
            ...
            uplift_groupA = self.calculate_test_canal_otro_uplift(df[df[explanatory_var] < cutoff])
            uplift_groupB = self.calculate_test_canal_otro_uplift(df[df[explanatory_var] >= cutoff])
    ...
    def SelfOptimize_regressor_under_stability_constraints(self, df_train, df_test, explanatory_var, q, s):
        ...
        for n in n_steps :
            ...
            uplift_groupA, uplift_groupA_T, uplift_groupB, uplift_groupB_T             = (self.calculate_test_canal_otro_uplift(df) for df in (groupA, groupA_T, groupB, groupB_T))
...
######################### =======================================================================
    ...
    def SelfOptimize_classifier_simple(self, df, criterion, offer, explanatory_var, mini_segment = 0.35):
        ...
        for i in modalities_list :
            uplift = self.calculate_test_canal_otro_uplift(df[df[explanatory_var].map(lambda x : x == i)], criterion, offer)
            if uplift :
        ...
        for l in range(1, len(modalities_list_sorted)):
            ...
            uplift_groupA = self.calculate_test_canal_otro_uplift(df[df[explanatory_var].map(lambda x : x in modalities_groupA)], criterion, offer)
            uplift_groupB = self.calculate_test_canal_otro_uplift(df[df[explanatory_var].map(lambda x : x in modalities_groupB)], criterion, offer)
    ...
    def optimizer_otro_uplift(self, df_train, df_test, explanatory_var, mini_segment, s, m):
        ...
        for n in n_steps :
            ...
            uplift_groupA, uplift_groupA_T, uplift_groupB, uplift_groupB_T             = (self.calculate_test_canal_otro_uplift(grupo) for grupo in (groupA, groupA_T, groupB, groupB_T))
    ...
    def SelfOptimize_regressor_v2(self, df_train, df_test, explanatory_var, q = 0.3):
        """ Finds the optimal cutoff value and tests the two groups - uplift summary"""
        # ---------------------------------------------------------------------------------
        # Optimize on train test
        # ---------------------------------------------------------------------------------
        ...
        def compute_uplift_group_difference_pc(cutoff):
            ...
            uplift_groupA = self.calculate_test_canal_otro_uplift(df[df[explanatory_var] < cutoff])
            uplift_groupB = self.calculate_test_canal_otro_uplift(df[df[explanatory_var] >= cutoff])
            ABS_delta_AB = uplift_groupA - uplift_groupB
            return -ABS_delta_AB # negative value to achieve minimization
