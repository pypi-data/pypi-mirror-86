...
class Spec:
    ...
    def get_options(cls) -> []:
        ...
        for key in fields.keys():
            val = fields[key]
            if isinstance(val, Credentials) or (isinstance(val, Variable) and not isinstance(val, InVariable) and not isinstance(val, OutVariable) and not isinstance(val, OptVariable) and val.option):
