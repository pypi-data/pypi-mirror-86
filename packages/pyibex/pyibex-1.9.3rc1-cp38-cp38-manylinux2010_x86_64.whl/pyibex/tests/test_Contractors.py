...
class TestContractors(unittest.TestCase):
    ...
    def test_function_eval_with_vector(self):
        f=Function("x1","x2","(x1;x1+x2)")
        P=IntervalVector([[1,2],[1,2]])
        f.eval(P)

    def test_function_jacobian(self):
        f=Function("x1","x2","(2*x1;x1+x2)")
        J = f.jacobian(IntervalVector([[1], [1]]))
        self.assertEqual(J, IntervalMatrix(2, 2, [2, 0, 1, 1]))

    def test_function_jacobian_2line(self):
        f=Function("x1","x2","(2*x1;x1+x2)")
        J = f.jacobian(IntervalVector([[1], [1]]), v=0)
        self.assertEqual(J, IntervalMatrix(2, 2, [Interval(2), Interval(), Interval(1), Interval()]))

    def test_function_gradient(self):
        f=Function("x1","x2","(2*x1-x2)")
        G = f.gradient(IntervalVector([1, 1]))
        self.assertEqual(G, IntervalVector([Interval(2), Interval(-1)]))
    ...
    def test_function_eval_with_vector(self):
        f=Function("x1","x2","(x1;x1+x2)")
        P=IntervalVector([[1,2],[1,2]])
        self.assertEqual(f.eval_vector(P), IntervalVector([[1, 2], [2, 4]]))
