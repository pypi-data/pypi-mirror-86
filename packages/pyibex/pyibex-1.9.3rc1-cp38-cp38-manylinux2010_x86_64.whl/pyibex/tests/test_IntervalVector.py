...
class TestIntervalVector(unittest.TestCase):
    ...
    def test_constructor_list_single(self):
        v = [[1],[2],[3]]
        c = IntervalVector(v)
        self.assertEqual(c, IntervalVector([Interval(1),Interval(2), Interval(3)]))

    @unittest.skipUnless(has_np, "Numpy not found")
    @unittest.expectedFailure
    def test_constructor_numpy_float32(self):
    ...
    def test_constructor_numpy_float64(self):
        v = np.array([1.,2.,3.], np.float64)
        c = IntervalVector(v)
        self.assertEqual(c, IntervalVector([Interval(1),Interval(2), Interval(3)]))

    @unittest.skipUnless(has_np, "Numpy not found")
    @unittest.expectedFailure
    def test_constructor_numpy_int(self):
        v = np.array([1,2,3])
        c = IntervalVector(v)
        self.assertEqual(c, IntervalVector([Interval(1),Interval(2), Interval(3)]))

    @unittest.skipUnless(has_np, "Numpy not found")
    def test_constructor_numpy_2(self):
        v = np.array([[1.0],[2.0],[3.0]])
        c = IntervalVector(v)
        self.assertEqual(c, IntervalVector([Interval(1),Interval(2), Interval(3)]))


    def test_constructor_list_of_double(self):
        c = IntervalVector([2,4.0, 2.0])
        self.assertEqual(c, IntervalVector([Interval(2),Interval(4.0), Interval(2.0)]))
    ...
    def test_op_numpy(self):
        v1 = IntervalVector([1,2,3])
        v2 = np.array([1., 2., 3.])
        self.assertEqual(v1 - v2.tolist(), IntervalVector(3, Interval(0)))
        # self.assertEqual(v2 - v1, IntervalVector(3, Interval(0)))
