...
def read_data(filename, fp):
    ...
    # some versions put mono/ana/stt/sth first, which gets the scans not
    # recognized as qscans: as a workaround, recognize the scaninfo
    is_qscan = meta['subtitle'].startswith(('qsscan', 'qcscan'))

    if 'h' in colnames and is_qscan:
        meta['old_qscan'] = True
