<template>
    <div>
        ...
        <b-table
            ...
            <template v-for="f in footer_elements" :slot="`foot(${f.key})`" :slot-scope="f">
...
export default class Table extends ListComponent {
            ...
            if (this.isObject(hkey)) {
                return {
                    ...
                    formatter: (value: any, key: any, item: any) => {
                        return `${hkey.prepend || ''} ${value} ${hkey.append || ''}`
                    },
                    ...hkey
                }
            } else {
    ...
    async fetchFooter() {
        ...
        if (this.list_api_info.has_footer) {
            await Vue.axios.get(`${this.baseUrl}djvue/footer/`).then((res) => {
                ret = res.data.footer
            })
        }
        return ret
