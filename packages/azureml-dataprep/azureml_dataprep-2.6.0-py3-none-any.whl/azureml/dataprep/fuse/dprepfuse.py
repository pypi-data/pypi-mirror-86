...
def mount(dataflow: Optional[Dataflow],
    ...
    if foreground:
        ...
        if dataflow is not None:
            _LoggerFactory.trace(log, 'Creating _DPrepFuse.', {'sessionId': session_id})
            fuse_opts = _DPrepFuse(dataflow, files_column, base_path, options, invocation_id, caller_session_id, span_context)
            _LoggerFactory.trace(log, 'Created _DPrepFuse.', {'sessionId': session_id})
        else:
            from azureml.dataprep.fuse._writable_fuse import WritableFuse
            _LoggerFactory.trace(log, 'Creating WritableFuse.', {'sessionId': session_id})
            fuse_opts = WritableFuse(destination, options, invocation_id, span_context, caller_session_id)
            _LoggerFactory.trace(log, 'Created WritableFuse.', {'sessionId': session_id})
        try:
            _LoggerFactory.trace(log, 'Initializing FUSE with fuse options.', {'sessionId': session_id})
            ...
            _LoggerFactory.trace(log, 'Initialized FUSE with fuse options.', {'sessionId': session_id})
            return None
        except Exception as e:
