    ...
    if sequenced_filename:
        ...
        try:
            ...
            if using_s3():
                ...
                existing_in_seq = DatabaseFile.objects.filter(app_code=app_code, filename__startswith=sequenced_filename)
            ...
            if existing_in_seq:
                ...
                for ff in existing_in_seq:
                    if using_s3():
                        ...
                        no_ext, ext = os.path.splitext(ff.filename)
                        remainder = ff.filename.replace(s3_path_no_ext, '').replace(ext, '')
...
def upload_file(file_instance, specified_filename=None, parent_directory=None, tag=None):
    ...
    try:
        ...
        if using_s3():
            ...
            uf.filename = specified_filename if specified_filename else original_file_name
            uf.original_name = original_file_name
            uf.tag = tag
            uf.save()
            return uf
