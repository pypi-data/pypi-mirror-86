...
class Analysis(Event):
    def __init__(self, rulesConfig, logging=None):
    ...
    def search(self, params:[]= []):
        if params:       
                        ...
                        for k,v in config["search_cfg"]["params"].items():
                            if not v:
                                if offset < len(params):
                                    ...
                                    if self.__logging:
                                        self.__logging.error('Parameter:array length exceeded.')
                                    else:
            ...
            if self.__logging:
                self.__logging.error('parameter is none.')
            else:
            ...
            for item in rulesConfig:
                if item['path']:
                    ...
                    if os.path.exists(item['path']):
                        ...
                        if self.__logging:
                            self.__logging.error("can't found config file. path(%s)" % str(item['path']))
                        else:
                            raise Exception("can't found config file. path(%s)" % str(item['path']))
                else:
                    if self.__logging:
                        self.__logging.error("config path is null.")
                    else:
                        raise Exception('config path is null.')
        else:
            if self.__logging:
                self.__logging.error("rules config is null.")
            else:
