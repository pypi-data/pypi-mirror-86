...
def detect_contamination(args: argparse.Namespace):
    ...
    reads, num_fragments, sequence_length, primary_file = fragmentation(args.contamination, args.fragment_length, args.overlap_length, args.index, centrifuge_path)
    ...
    centrifuge_command = centrifuge_path + " -p " + args.threads + "  -f -x " + args.index + " " + primary_file + " -k 1 " + " --report-file " + summary.name + " >> " + to_write.name
