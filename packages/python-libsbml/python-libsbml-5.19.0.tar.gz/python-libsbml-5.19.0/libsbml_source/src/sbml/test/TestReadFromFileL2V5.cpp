/**
 * \file    TestReadFromFileL2V5.cpp
 * \brief   Reads test-data/l2v5-all.xml into memory and tests it.
...
START_TEST (test_read_l2v5_all)
{
  ...
  if (d == NULL)
  {
    fail("readSBML(\"l2v5-all.xml\") returned a NULL pointer.");
  }
  ...
  fail_unless( d->getVersion() == 5, NULL );
  ...
  fail_unless(m->getId() == "l2v5_all", NULL);
  ...
  ast = kl->getMath();
  formula = SBML_formulaToString(ast);
  fail_unless(!strcmp(formula, "s * k / p"), NULL);
  safe_free(formula);
  ...
  ast = stoich->getMath();
  formula = SBML_formulaToString(ast);
  fail_unless(!strcmp(formula, "s * p"), NULL);
  safe_free(formula);
...
Suite *
create_suite_TestReadFromFileL2V5 (void)
{ 
  Suite *suite = suite_create("test-data/l2v5-all.xml");
  TCase *tcase = tcase_create("test-data/l2v5-all.xml");


  tcase_add_test(tcase, test_read_l2v5_all);
  tcase_add_test(tcase, test_echo_l2v5_all);
