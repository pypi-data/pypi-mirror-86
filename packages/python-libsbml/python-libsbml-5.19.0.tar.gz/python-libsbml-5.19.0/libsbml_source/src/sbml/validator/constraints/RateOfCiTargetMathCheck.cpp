 ...
 */
RateOfCiTargetMathCheck::RateOfCiTargetMathCheck (unsigned int id, Validator& v) : MathMLBase(id, v)
{
}
 ...
 */
RateOfCiTargetMathCheck::~RateOfCiTargetMathCheck ()
{
}
 ...
 */
const char*
RateOfCiTargetMathCheck::getPreamble ()
{
  return PREAMBLE;
}
...
/*
  ...
  */
void
RateOfCiTargetMathCheck::checkMath (const Model& m, const ASTNode& node, const SBase & sb)
{
  ...
  switch (type) 
  {
    case AST_FUNCTION_RATE_OF:
      ...
      {
        // incorrect number of children should already be logged
        return;
      }
                                        ...
                                        const SBase & sb)
{
  ...
  {
    logRateOfChildTypeConflict(node, sb);
    return;
  }
  ...
  if (m.getCompartment(name) == NULL &&
      ...
      m.getSpeciesReference(name) == NULL  )
  {
                                             ...
                                             const SBase & sb)
{
  ...
  msg += "contains an argument that is not a <ci> element.";
  safe_free(formula);
