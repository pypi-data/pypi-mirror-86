  ...
  else
  {
    switch (ASTNode_getType(node))
    {
      ...
      default:
        ...
        if (node)
        {
          const ASTBasePlugin* baseplugin = node->getASTPlugin(ASTNode_getType(node));
          if (baseplugin != NULL)
          {
            // this needs to know what the parent node is so do actually
            // clone it 
            ASTBasePlugin* plugin = baseplugin->clone();
            plugin->connectToParent((ASTNode*)(node));
            precedence = plugin->getL3PackageInfixPrecedence();
            foundInPlugin = true;
            delete plugin;
          }
        }
  ...
  default:
  {
    ...
    if (node)
    {
      ...
      if (baseplugin != NULL)
      {
        ...
        {
          foundInPlugin = true;
          StringBuffer_append(sb, baseplugin->getConstCharFor(type));
        }
      }
    }
    if (!foundInPlugin)
  ...
  else
  {
    ...
    if (node != NULL)
    {
      ...
      if (baseplugin != NULL)
      {
        ...
        ASTBasePlugin* plugin = baseplugin->clone();
        plugin->connectToParent((ASTNode*)(node));
        if (plugin->hasPackageOnlyInfixSyntax())
        {
                               ...
                               const L3ParserSettings_t *settings)
{
  ...
  if (node != NULL)
  {
    ...
    if (baseplugin != NULL)
    {
      ...
      ASTBasePlugin* plugin = baseplugin->clone();
      plugin->connectToParent((ASTNode*)(node));
      if (plugin->hasUnambiguousPackageInfixGrammar(child))
      {
        delete plugin;
        return 1;
      }
  ...
  default:
    if (node != NULL)
    {
      ...
      if (baseplugin != NULL)
      {
        ...
        ASTBasePlugin* plugin = baseplugin->clone();
        plugin->connectToParent((ASTNode*)(node));
        if (plugin->isFunction(type) && !plugin->hasPackageOnlyInfixSyntax())
        {
          delete plugin;
          return 1;
        }
