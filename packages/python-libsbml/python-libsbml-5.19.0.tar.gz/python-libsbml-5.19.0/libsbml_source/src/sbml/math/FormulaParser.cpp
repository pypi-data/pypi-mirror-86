...
SBML_parseFormula (const char *formula)
{
  ...
  while (1)
  {
    ...
    {
      node = (ASTNode_t*)(Stack_peekAt(stack, 1));
      break;
    }
    ...
    else if (action == ERROR_STATE)
    {
      ...
      {
        Stack_pop(stack);
        ASTNode_free((ASTNode_t*)(Stack_pop(stack) ));
      }
...
FormulaParser_reduceStackByRule (Stack_t *stack, long rule)
{
  ...
  ASTNode_t *lexpr, *rexpr, *operator_expr;
  ...
  {
    Stack_pop(stack);
    result = (ASTNode_t*)(Stack_pop(stack));
  ...
  {
    Stack_pop(stack);
    rexpr = (ASTNode_t*)(Stack_pop(stack));

    Stack_pop(stack);
    operator_expr = (ASTNode_t*)(Stack_pop(stack));

    Stack_pop(stack);
    lexpr = (ASTNode_t*)(Stack_pop(stack));

    ASTNode_addChild(operator_expr, lexpr);
    ASTNode_addChild(operator_expr, rexpr);

    result = operator_expr;
  }
