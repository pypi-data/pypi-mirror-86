...
class lyftron:
    ...
    def filterData(self, table, columns, value, displayValues, gs):
    ...
    def queryMaker(self, query):
        ...
        if (commands[0].lower() == slct.lower()):
            ...
            if ("*" in commands[1]):
                # print()
                displayValues = '*'
                self.filterData(table[0], col, values, displayValues, gs)
            else:
                print()
                self.filterData(table[0], col, values, displayValues, gs)
            ...
            for c in columns:
                col.append(c.split('=')[0])
                values.append(c.split('=')[1])
            self.updateData(table, col, values, updValues, gs)
            ...
            for c in columns:
                col.append(c.split('=')[0])
                values.append(c.split('=')[1])
            self.deleteData(table[0], col, values, gs)
