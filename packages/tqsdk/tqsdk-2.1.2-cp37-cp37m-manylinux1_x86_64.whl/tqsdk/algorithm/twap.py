...
class Twap(object):
    ...
    **注意**：
    ...
    async def _insert_order(self, volume, end_time, strict_end_time):
        volume_left = volume
        try:
        ...
        try:
            trade_chan = TqChan(self._api)
            self._order_task = InsertOrderUntilAllTradedTask(self._api, self._symbol, self._direction, self._offset,
                                                        volume=volume_left, price="ACTIVE", trade_chan=trade_chan)
            async for v in trade_chan:
                volume_left = volume_left - (v if self._direction == "BUY" else -v)
                if volume_left == 0:
                    break
...
def _rangeset_slice(rangeset, start, end=None):
    """
