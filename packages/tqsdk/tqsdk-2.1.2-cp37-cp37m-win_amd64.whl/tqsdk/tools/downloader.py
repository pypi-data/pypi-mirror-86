...
class DataDownloader:
    ...
    def __init__(self, api: TqApi, symbol_list: Union[str, List[str]], dur_sec: int, start_dt: Union[date, datetime],
                 end_dt: Union[date, datetime], csv_file_name: Union[str, None] = None) -> None:
        """
        创建历史数据下载器实例
        ...
        Args:
            ...
            csv_file_name (str/None): 指定 csv 文件名，下载的数据结果会保存到该文件中。默认为 None，不会下载到文件。
    ...
    def get_data_series(self) -> pandas.DataFrame:
        """
    ...
    async def _run(self):
        ...
        try:
            ...
            async for item in self._data_chan:
                array.append(item)
            self._data_series = pandas.DataFrame(array, columns=header)
            if self._csv_file_name:
                self._data_series.to_csv(self._csv_file_name)
        finally:
            await task
    ...
    def _get_headers(self):
        data_cols = self._get_data_cols()
        return ["datetime", "datetime_nano"] + [f"{symbol}.{col}" for col in data_cols for symbol in self._symbol_list]
