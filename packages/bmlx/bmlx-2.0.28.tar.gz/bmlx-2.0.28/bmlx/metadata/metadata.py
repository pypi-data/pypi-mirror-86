...
class Metadata(with_metaclass(abc.ABCMeta)):
    ...
    def build_execution_obj(
        ...
        if experiment_id is None:
            experiment = self.store.get_experiment_by_name("Default")
            if not experiment:
                raise RuntimeError("Failed to get experiment by name Default")
    ...
    _METRICS_PATH = (
        "{base_metrics_path}/{experiment_id}/{component_run_id}"
    )

    @classmethod
    def upload_metrics(cls,
        ...
        logging.info("copy local metrics dir %s to %s" % (local_path, output_path))
