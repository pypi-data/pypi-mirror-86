...
class PyGuppyClient(GuppyClient):
    ...
    def pass_read(
        ...
        if return_code == self.success:
            return True
        elif return_code == self.not_ready:
            return False
        elif return_code == self.failed:
            raise ValueError(
        ...
        elif return_code == self.no_connection:
            raise ConnectionError(
                "Not connected to the server. return_code: {!r}".format(return_code)
            )
        else:
    ...
    def get_completed_reads(self) -> List[dict]:
        ...
        results, return_code = super().get_completed_reads()
        if return_code != self.success:
            if return_code == self.no_connection:
                raise ConnectionError(
                    "Not connected to server. return_code: {!r}".format(return_code)
                )
            elif return_code == self.failed:
                # Could a better exception be raised instead?
                raise RuntimeError(
                    "Something went wrong. return_code: {!r}".format(return_code)
                )
            else:
                # Shouldn't get here
