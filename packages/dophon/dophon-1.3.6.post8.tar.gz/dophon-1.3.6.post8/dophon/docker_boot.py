...
def run_as_docker(
    ...
    :param docker_port: 容器内部端口 -> 集群模式下的暴露端口/配置文件定义的端口
    ...
    try:
                ...
                with open('./requirements.txt', 'w') as final_file:
                    ...
                    for line in file.readlines():
                        ...
                        for key in sys.modules.keys():
                            ...
                            #     'dophon') else None
                            if re.search(module_path.lower(), key.lower()) or re.search(key.lower(),
                                                                                        module_path.lower()):
        ...
        def process_docker_file():
            ...
            with open('./Dockerfile', 'w') as file:
                file.write('FROM callmee/dophon\n')
                ...
                file.write(
                    'CMD ["python","./' + (entity_file_name if entity_file_name else 'Bootstrap.py') + '"]' + '\n')
                # file.write('CMD ["/bin/bash"]' + '\n')

        def process_docker_command():
            # 生成docker command
            docker_command = f'docker -H {dH} ' if dH else 'docker '
            os.system('cd ' + root)
            logger.info('暂停已运行的实例')
            os.system(f'{docker_command}stop {base_name}')
            logger.info('移除已运行的实例')
            os.system(f'{docker_command}rm {base_name}')
            logger.info('移除旧镜像')
            os.system(f'{docker_command}rmi {base_name}')
            logger.info('检测配置合法性')
            if IsOpen('127.0.0.1', int(docker_port)):
                # 端口被占用
                logger.warn('映射端口被占用!!')
            logger.info('建立镜像')
            listen(os.system(f'{docker_command}build -t {base_name} .'))
            logger.info('运行镜像')
            os.system(
                f'{docker_command}run -p {container_port}:{docker_port} -d --name {container_alias if container_alias else base_name} {os.path.basename(root)}'
            )
            if save_image:
                logger.info('检测容器状态')
                ...
                while __status == 'running':
                    p = Popen(
                        f"{docker_command}inspect {base_name} -f {status_code}",
                        stdout=PIPE,
                        stderr=PIPE
                    )
                    p.wait()
                ...
                # 保存镜像
                os.system(f"""
                        {docker_command}commit --author "{img_author}" --message "{img_message}" {base_name} {__commit_image_name}
                        """)
                logger.info('生成镜像')
                os.system(f"""{docker_command}save -o {base_name}.img.bak.__ {__commit_image_name}""")
                exit(0)
                return
            logger.info('打印容器内部地址(空地址代表启动失败)')
            os.system(f'{docker_command} inspect --format=' + '\'{{.NetworkSettings.IPAddress}}\'' + f' {base_name}')
        ...
        logger.debug('process_prepare_file')
        process_prepare_file()
        logger.debug('process_docker_file')
        process_docker_file()
        logger.debug('process_docker_command')
        process_docker_command()
    except Exception as e:
        logger.error(e)
