...
PAGE_NUM_REG_STR = '.*[nN][uU][mM].*'
PAGE_SIZE_REG_STR = '.*[nN][uU][mM].*'
...
def pkg_page_info(page_num=1, page_size=1, page_model=[]):
    if not page_model:
        ...
        for key in page_model:
            if re.match(PAGE_NUM_REG_STR, string=key):
                page_info[key] = page_num
            if re.match(PAGE_SIZE_REG_STR, string=key):
...
def depkg_page_info(page_info: dict):
    ...
    return 'limit ' + str(
        0 if int(_page_num) * int(_page_size) < 0 else int(_page_num) * int(_page_size)) + ',' + str(
        _page_size)
