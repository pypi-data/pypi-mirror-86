...
def pontuacion():
    ...
    single = open(f"{cwd}/single.txt","r")
    survival = open(f"{cwd}/survival.txt","r")
...
def survival():
    ...
    pontos = open(f"{cwd}/survival.txt","r")
    score = 0
    tempo = 0
    fps = pygame.time.Clock()
    class snake:
            ...
            for i in range(1,len(cobra)-1):
                if cobra[0][0] == cobra[i][0] and cobra[0][1] == cobra[i][1]:
                    ...
                    while not len(pontos_atual) == 5:
                        pontos_atual.pop(0)
                    novos_pontos = open(f"{cwd}/survival.txt","w")
...
def singleplayer():
    pontos_atual = [0, 0, 0, 0, 0]
    pontos = open(f"{cwd}/single.txt","r")
    score = 0
    fps = pygame.time.Clock()
    class snake:
        def __init__(self, cor, vel, larg, comp):
        ...
        if len(cobra)>1:   #Morte     
            for i in range(1,len(cobra)-1):
                if cobra[0][0] == cobra[i][0] and cobra[0][1] == cobra[i][1]:
                    ...
                    while not len(pontos_atual) == 5:
                        pontos_atual.pop(0)
                    novos_pontos = open(f"{cwd}/single.txt","w")
                    for i in range(len(pontos_atual)-1,-1,-1):
                        if not i == 0:
                            novos_pontos.write(f"{pontos_atual[i]}\n")
                    score = 0
