...
def init_bluetooth_settings(min_interval=6,max_interval=9,latency=0):
    ...
    resp = os.system("sudo {} {} {} {}".format(os.path.join(pypath,shname),min_interval,max_interval,latency))
    if resp != 0:
        print("Error setting the BLE parameters!  Perhaps you need to set the `initGanglionBluetoothSettings.sh` to run as root?")
    # try:
