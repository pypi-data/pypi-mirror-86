...
wifi = OpenBCIWifi()
wifi.start_stream(handle_sample)
asyncore.loop()
...
import requests
import xmltodict
# EXTRACTED FROM : www.github.com/pyopenBCI
import ssdp
...
class OpenBCIWiFi(object):
    ...
    def __init__(self, ip_address=None, shield_name=None, sample_rate=None, log=True, timeout=3,
                 ...
                 num_channels=8, local_ip_address=None, local_port=None):
        # these one are used
        self.daisy = False
        self.gains = None
        ...
        if not self.local_port:
            # Intentionally bind to port 0 -- so auto allocate unused port
            self.local_port=0

        self.local_wifi_server = WiFiShieldServer(self.local_ip_address, self.local_port)
...
class WiFiShieldHandler(asyncore.dispatcher_with_send):
    def __init__(self, sock, callback=None, high_speed=True,
        ...
        # small recieve buffer so drop samples early if going tooo
        self.socket.setsockopt(socket.SOL_SOCKET,socket.SO_RCVBUF,30000)
                ...
                if self.daisy:
                    ...
                    for sample in samples:
                        ...
                        #  otherwise drop the packet
                            ...
                            daisy_samples.append(self.parser.make_daisy_sample_object_wifi(
                                self.last_odd_sample, sample))
                    samples=daisy_samples;
                    
                # now send all the samples to the callback function
...
class ParseRaw(object):
    ...
    def parse_packet_standard_accel(self, raw_data_to_sample):
        ...
        # Verify the correct start byte.
        if raw_data_to_sample.raw_data_packet[0] != 0xA0:
