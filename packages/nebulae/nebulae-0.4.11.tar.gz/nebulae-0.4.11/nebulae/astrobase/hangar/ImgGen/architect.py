...
class SimpleRes(dock.Craft):
    def __init__(self, in_shape, neck_chs, stride=1, width_multp=4., norm_fn=BN, attention=False, scope='SIMRES'):
        ...
        if attention:
            self.att = NonLocal(self.body_chs, self.body_chs//8, self.body_chs//2)
        else:
...
class ResG(dock.Craft):
    def __init__(self, in_shape, latent_dim, norm_fn=BN, attention=False, scope='RESG'):
        ...
        if norm_fn == BN:
            self.nf_1 = dock.BN(64, dim=2)
            self.nf_2 = dock.BN(16, dim=2)
        elif norm_fn == IN:
            self.nf_1 = dock.IN(64, dim=2)
            self.nf_2 = dock.IN(16, dim=2)
        elif norm_fn == LN:
            self.nf_1 = dock.LN((H // 4, W // 4, 64))
            self.nf_2 = dock.LN((H, W, 16))
        pad = dock.autoPad((H // 4, W // 4), (3, 3), stride=2)
        self.conv_2 = dock.Conv(64, 128, (3, 3), stride=2, padding=pad)
        self.res_1 = SimpleRes(((H // 8), (W // 8), 128), 128, width_multp=0.5, attention=attention)
        self.res_2 = SimpleRes(((H // 4), (W // 4), 64), 64, width_multp=0.5, attention=attention)
        self.res_3 = SimpleRes(((H // 2), (W // 2), 32), 32, width_multp=0.5, attention=attention)
        pad = dock.autoPad((H, W), (3, 3))
...
class ResD(dock.Craft):
    def __init__(self, in_shape, norm_fn=BN, attention=False, scope='RESD'):
        ...
        self.res_1 = SimpleRes((H // 2, W // 2, 32), 16, norm_fn=norm_fn, attention=attention)

        pad = dock.autoPad((H // 2, W // 2), (2, 2), stride=2)
        self.apool_2 = dock.AvgPool((2, 2), padding=pad)
        self.res_2 = SimpleRes((H // 4, W // 4, 64), 32, norm_fn=norm_fn, attention=attention)

        pad = dock.autoPad((H // 4, W // 4), (2, 2), stride=2)
        self.apool_3 = dock.AvgPool((2, 2), padding=pad)
        self.res_3 = SimpleRes((H // 8, W // 8, 128), 64, norm_fn=norm_fn, attention=attention)
        if norm_fn == BN:
...
class SNSimpleRes(dock.Craft):
    def __init__(self, in_shape, neck_chs, stride=1, width_multp=4.,
                 norm_fn=BN, attention=False, scope='SIMRESSN'):
...
class SNResG(dock.Craft):
    def __init__(self, in_shape, latent_dim, norm_fn=BN, attention=False, scope='SNRESG'):
        super(SNResG, self).__init__(scope)
        H, W, C = in_shape
        assert H % 8 == 0 and W % 8 == 0
        self.H = H
        self.W = W
        ...
        self.up = dock.Upscale(2)
        self.relu = dock.Relu()
        self.fc = dock.SN(dock.Dense(latent_dim, 64 * (H // 4) * (W // 4)))
        pad = dock.autoPad((H // 4, W // 4), (3, 3))
        self.conv_1 = dock.SN(dock.Conv(64, 64, (3, 3), padding=pad))
        if norm_fn == BN:
            self.nf_1 = dock.BN(64, dim=2)
            self.nf_2 = dock.BN(16, dim=2)
        elif norm_fn == IN:
            ...
            self.nf_2 = dock.LN((H, W, 16))
        pad = dock.autoPad((H // 4, W // 4), (3, 3), stride=2)
        self.conv_2 = dock.SN(dock.Conv(64, 128, (3, 3), stride=2, padding=pad))
        self.res_1 = SNSimpleRes(((H // 8), (W // 8), 128), 128, width_multp=0.5)
        self.res_2 = SNSimpleRes(((H // 4), (W // 4), 64), 64, width_multp=0.5)
        self.res_3 = SNSimpleRes(((H // 2), (W // 2), 32), 32, width_multp=0.5)
        pad = dock.autoPad((H, W), (3, 3))
        self.conv_3 = dock.SN(dock.Conv(16, C, (3, 3), padding=pad))
        self.tanh = dock.Tanh()
        self.reshape = dock.Reshape()
...
class SNResD(dock.Craft):
    def __init__(self, in_shape, norm_fn=BN, attention=False, scope='SNRESD'):
        ...
        pad = dock.autoPad((H, W), (2, 2), stride=2)
        self.apool_1 = dock.AvgPool((2, 2), padding=pad)
        self.res_1 = SNSimpleRes((H // 2, W // 2, 32), 16, norm_fn=norm_fn, attention=attention)

        pad = dock.autoPad((H // 2, W // 2), (2, 2), stride=2)
        self.apool_2 = dock.AvgPool((2, 2), padding=pad)
        self.res_2 = SNSimpleRes((H // 4, W // 4, 64), 32, norm_fn=norm_fn, attention=attention)

        pad = dock.autoPad((H // 4, W // 4), (2, 2), stride=2)
        self.apool_3 = dock.AvgPool((2, 2), padding=pad)
        self.res_3 = SNSimpleRes((H // 8, W // 8, 128), 64, norm_fn=norm_fn, attention=attention)
        if norm_fn == BN:
            self.nf = dock.BN(256, dim=2)
        elif norm_fn == IN:
            self.nf = dock.IN(256, dim=2)
...
class NonLocal(dock.Craft):
    def __init__(self, in_shape, att_chs, neck_chs, scope='NONLOCAL'):
        ...
        self.theta = dock.Conv(self.C, self.att_chs, (1, 1))
        self.phi = dock.Conv(self.C, self.att_chs, (1, 1))
        self.mpool_p = dock.MaxPool((2, 2), padding=pad)
        self.g = dock.Conv(self.C, self.neck_chs, (1, 1))
        self.mpool_g = dock.MaxPool((2, 2), padding=pad)
        self.o = dock.Conv(self.neck_chs, self.C, (1, 1))
