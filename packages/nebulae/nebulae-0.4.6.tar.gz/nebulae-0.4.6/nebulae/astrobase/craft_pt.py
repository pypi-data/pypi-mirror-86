...
class BN(Craft):
    def __init__(self, out_chs, dim, mmnt=0.9, beta=False, gamma=False, scope='BN'):
        ...
        if dim == 1:
            ...
            raise Exception('NEBULAE ERROR ⨷ %d-d BN is not supported.' % dim)
        self.norm = norm_fn(out_chs, momentum=1 - mmnt, affine=beta or gamma, eps=1e-5)
...
class CBN(Craft):
    def __init__(self, out_chs, dim, beta, gamma, mmnt=0.9, scope='CBN'):
        ...
        if dim == 1:
            ...
            raise Exception('NEBULAE ERROR ⨷ %d-d BN is not supported.' % dim)
        self.norm = norm_fn(out_chs, momentum=1 - mmnt, affine=True, eps=1e-5)
        self.norm.weight = gamma
        self.norm.bias = beta
...
class IN(Craft):
    def __init__(self, out_chs, dim, mmnt=0.9, beta=False, gamma=False, scope='IN'):
