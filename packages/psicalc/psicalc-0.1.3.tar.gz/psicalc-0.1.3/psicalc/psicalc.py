...
def return_sr_mode(df: pd.DataFrame, c: list, csv_dict: dict, list_store, k) -> tuple:
    """Calculates the sr_mode and new mode of a cluster and returns both.
    Pairwise operation is O(1) and the others are O((n^2-n)/2)"""
    ...
    if len(c) < 2:
        ...
        i, j = c[0], c[1]
        max_sum = nmis(df[i], df[j], average_method='geometric')
        new_mode = c
    else:
        ...
        for loc, i in enumerate(c):
            for location, j in enumerate(c):
                if location != loc and location > shift:
                    A[loc][1].append(nmis(df[i], df[j], average_method='geometric'))
            t = loc
            q = loc + 1
            if q != D:
                while q != D:
        ...
        mode_map = {k: sum(v) for k, v in A}
        mode_loc, max_sum = max(mode_map.items(), key=operator.itemgetter(1))
...
def find_clusters(spread: int, df: pd.DataFrame) -> dict:
    ...
    print("\nProcessing MSA. This may take a while.")
    csv_dict = dict()
    k = "pairwise"
    ...
    for item, each in enumerate(subset_list):
        ...
        for location, cluster in enumerate(msa_index):
            ...
            if subset_mode != cluster_mode:
                rii = nmis(df[subset_mode], df[cluster_mode], average_method='geometric')
                if rii > max_rii:
                    max_rii, best_cluster = rii, location
        subset_list[item].append(msa_index[best_cluster])

    for cluster in subset_list:
        return_sr_mode(df, cluster, csv_dict, hash_list, k)
    ...
    for cluster in final_df_set:
        return_sr_mode(df, cluster, csv_dict, unranked, k)

    stg_ranked_list = sorted(unranked, key=lambda x: x[0], reverse=True)
    remove = [col for x, j in stg_ranked_list for col in j]
    for k in remove:
        msa_index.remove(k)
    ...
    for remaining in msa_index:
        stg_ranked_list.append([0, [remaining]])
    ...
    while len(stg_ranked_list) >= 2:
        ...
        while i < num_clusters:
            location = None
            cluster_mode = stg_ranked_list[i][1][0]
            max_rii = 0
            for loc, entry in enumerate(stg_ranked_list):
                sr_mode, cluster = entry
                attr_mode = cluster[0]
                if cluster_mode != attr_mode:
                    rii = nmis(df[attr_mode], df[cluster_mode], average_method='geometric')
                    if rii > max_rii:
                        max_rii, best_cluster, location = rii, cluster, loc
            stg_ranked_list[i][1] = stg_ranked_list[i][1] + best_cluster
            stg_ranked_list[i][0], stg_ranked_list[i][1] = return_sr_mode(df, stg_ranked_list[i][1], csv_dict, 0, k)
