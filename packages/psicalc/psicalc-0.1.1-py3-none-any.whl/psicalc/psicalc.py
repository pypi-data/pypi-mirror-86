...
def return_pairwise_cluster(c: pd.DataFrame, list_store: list, pretty_list: list, k, csv_dict: dict) -> pd.DataFrame:
    ...
    if len(c.columns) < 2:
        ...
        for loc, i in enumerate(c):
            for location, j in enumerate(c):
                if location != loc and location > shift:
                    A[loc][1].append(nmis(c[i], c[j], average_method='geometric'))
            t = loc
            q = loc + 1
            if q != D:
                while q != D:
        ...
        mode_loc = max(mode_map.items(), key=operator.itemgetter(1))[0]
        max_sum = max([sum(each[1]) for each in A])
        c = return_new_mode(mode_loc, c)
...
def return_sr_mode(c: pd.DataFrame, csv_dict: dict, k) -> tuple:
    """Calculates the sr_mode and new mode of a cluster and returns the sr_mode.
...
def find_clusters(spread: int, df: pd.DataFrame) -> dict:
    ...
    while len(stg_ranked_list) >= 2:
        ...
        while i < num_clusters:
            ...
            stg_ranked_list[i][0], stg_ranked_list[i][1] = return_sr_mode(stg_ranked_list[i][1], csv_dict, k)
