...
def return_sr_mode(c: pd.DataFrame, csv_dict: dict, list_store, k) -> tuple:
    ...
    if len(c.columns) < 2:
        ...
        A = [[column, []] for column in range(len(c.columns))]
        D = len(A)
        shift = 0
        for loc, (name, i) in enumerate(c.iteritems()):
            for location, (names, j) in enumerate(c.iteritems()):
                if location != loc and location > shift:
                    A[loc][1].append(nmis(i, j, average_method='geometric'))
            t = loc
            q = loc + 1
            if q != D:
                while q != D:
...
def return_new_mode(location: int, c: pd.DataFrame) -> pd.DataFrame:
    ...
    mode = c[c.columns[location]]
    c = c.drop(c.columns[location], axis=1)
...
def find_clusters(spread: int, df: pd.DataFrame) -> dict:
    ...
    Provide a dataframe and a sample spread-width. Writes output csv files and
    ...
    for cluster in subset_list:
        return_sr_mode(cluster, csv_dict, hash_list, k)
    ...
    for cluster in final_df_set:
        return_sr_mode(cluster, csv_dict, unranked, k)
    ...
    while len(stg_ranked_list) >= 2:
        ...
        while i < num_clusters:
            ...
            stg_ranked_list[i][0], stg_ranked_list[i][1] = return_sr_mode(stg_ranked_list[i][1], csv_dict, 0, k)
