...
Быстро пофиксить эту проблему можно просто закинув файл [caos](https://github.com/DoomzD/caos-reborn/blob/work_in_progress/caos) из этого репозитория в папку, которая входит в `$PATH` (Например, сохранить файл ```/usr/local/bin/caos```)
