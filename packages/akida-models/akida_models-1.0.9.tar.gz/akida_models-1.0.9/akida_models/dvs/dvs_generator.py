...
class DVS_DataGenerator:
    ...
    def __init__(self,
        ...
        Args:
            ...
            inversion (bool): flag to flip x/y coords.
            packets_fixed_by (str, def: 'duration'): value used to fix packet
                length, one of 'duration' or (number of) 'events'.
            subpacket_length (int, def: 100): subpacket length (milliseconds if
                packets_fixed_by='duration', event count if 'events').
            subpackets_per_packet (int, def: 1): number of sub-packets
                (~ time bins) to concatenate to generate packets.
            camera_dims (tuple of ints, def: (128, 128, 2)): dimensions of the
                raw data from the camera.
            include_polarity (bool, def: True): keep event polarity.
                If False, On and Off events will be treated as if identical.
            allow_duplicate_events (bool, def: False): allow events to be
                superimposed. If False, only a single event will be allowed per
                pixel for each timebin (subpacket) and polarity.
            downscale (int, def: 1): downscaling of inputs, implemented as
                integer division of incoming coordinate values.
    ...
    def get_random_samples(self,
                           ...
                           class_key=None,
                           samples_per_trial=5):
        # Generate [samples_per_trial] samples from each subject/lighting
        # condition/class
        ...
        outlabels = []
        sample_windows = []
        for ind, trial in enumerate(self.dataset):
            if int(trial['label']) in class_list:
                ...
                if self.packets_fixed_by.startswith('dur'):
                    ...
                    if latest_start_time < 0:
                        print(
                            'ERROR: Sample durations requested is longer than the available data for this trial'
                        )
                        continue
                ...
                for rr in range(samples_per_trial):
    ...
    def get_continuous_sample(self,
                              ...
                              start_time=0,
                              stop_time=None):
        # Generate series of temporally contiguous samples from a single
        # subject+class
        if class_key == None:
        ...
        for ind, trial in enumerate(self.dataset):
            ...
            if (int(trial['label']) == class_label) and (int(trial['subject'])
                ...
                # Get latest start point (valid across all repeats)
                num_events = trial['ev_times'].shape[0]
                if local_stop_time == None:
                    local_stop_time = trial['ev_times'][-1]
                elif local_stop_time > trial['ev_times'][-1]:
                    print(
                        'ERROR: requested stop time later than the end of the trial'
                    )
                ...
                if self.packets_fixed_by.startswith('dur'):
                    ...
                    if latest_start_time < 0:
                        print(
                            'ERROR: Sample durations requested is longer than the available data for this trial (or stop time is set too early)'
                        )
                        continue
                    latest_start_point = np.searchsorted(np.squeeze(
                        trial['ev_times']),
