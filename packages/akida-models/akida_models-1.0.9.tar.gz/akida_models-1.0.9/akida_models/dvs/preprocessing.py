...
def _parse_version_string(version_string):
    ...
    if v2.match(str(version_string)):
        ...
        return 3
    elif v4.match(str(version_string)):
        return 4
    else:
...
def _read_aedat_v4(datafile):
    ...
    with AedatFile(datafile) as f:
        np_events = np.hstack([packet for packet in f['events'].numpy()])
    ...
    if dataset_name == "dvs_handy":
        for subj in subject_list:
            ...
            for idx in range(len(datafile)):
                data = get_gesture_data(dataset_name, datafile[idx])
                if data:
                    dataset.append(data)
    elif dataset_name == "dvs_gesture":
        for subj in subject_list:
            ...
            for idx in range(len(datafile)):
                dataset_tmp = get_gesture_data(dataset_name, datafile[idx],
                                               labelfile[idx])
