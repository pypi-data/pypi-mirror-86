...
from .interface import nodealiasproperty
...
def axis_type(value):
    ...
    """
    try:
        return getattr(AXIS_TYPE, value) if isinstance(value, str) else value
...
class Motor(Device):
    ...
    def stat(self):
        if self._stat is None:
            ...
            self._stat._mot_positions = self._config.get('positions', None)
        return self._stat
...
Manager.record_cfg_template('Motor', """{name}:
  type: Motor
  interface: Softing
  identifier: PLC1
  prefix: MAIN.Motor1 # TO BE UPDATED  
  simulated: false
  ignored: false
  address: {address}   # TO BE UPDATED 
  simaddr: opc.tcp://127.0.0.1:7578
  mapfile: "{cfg_dir}/mapMotor.yml"  # TO BE UPDATED 
  fits_prefix: "MOT1"
  ctrl_config:
    velocity:              3.0
    min_pos:               0.0
