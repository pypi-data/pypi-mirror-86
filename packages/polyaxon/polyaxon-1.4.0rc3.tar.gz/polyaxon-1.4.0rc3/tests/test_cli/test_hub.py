...
class TestCliHub(BaseCommandTestCase):
    @patch("polyaxon.client.HubClient.create")
    ...
    @patch("polyaxon.client.HubClient.list")
    def test_list_hubs(self, list_hub):
        self.runner.invoke(hub, ["ls", "--owner=owner"])
        assert list_hub.call_count == 1

    @patch("polyaxon.client.HubClient.refresh_data")
    def test_get_hub(self, get_hub):
        self.runner.invoke(hub, ["get", "--hub=admin/foo"])
        assert get_hub.call_count == 1

    @patch("polyaxon.client.HubClient.update")
    def test_update_hub(self, update_hub):
        self.runner.invoke(hub, ["update", "--hub=admin/foo", "--description=foo"])
