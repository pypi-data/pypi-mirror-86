...
class Heroku(cl1.CLI):
	def __init__(self, package=Formats.FilePath(__file__).base()):
		...
		# variables.
		self.package = Files.Directory(package)
		cl1.CLI.__init__(self, 
			modes={
				...
				"--get-env-variable MY_VARIABLE":"Get the specfied env variables from heroku.",
	...
	def start(self):
		...
		elif '--get-env-variable' in sys.argv:
			keys = self.get_argument("--get-env-variable")
			i = {}
			for key in keys.split(","):
				i[key] = ""
			self.get_environment_variables(i)
		else:
			print(self.documentation)
