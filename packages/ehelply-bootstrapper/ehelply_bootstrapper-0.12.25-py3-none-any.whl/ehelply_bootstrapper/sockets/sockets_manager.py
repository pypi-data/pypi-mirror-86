...
from ehelply_bootstrapper.sockets.sockets_backbone import SocketConnectionBackbone, SCBRedis
from ehelply_bootstrapper.drivers.aws_utils.aws_apigateway import APIGateway
from ehelply_bootstrapper.sockets.sockets_schemas import *
...
class SocketConnectionManager(EventController):
    ...
    async def disconnect(self, identifier: str, connection=None):
        """
...
class SCMFastApi(SocketConnectionManager):
    ...
    async def connect(self, identifier: str, connection):
        ...
        This method will handle the remainder of the lifecycle of the FastAPI socket connection

        ```
        @router.websocket('/sockets')
        async def websocket_endpoint(websocket: WebSocket):
            ...
            try:
                while not authorized:
                    message: dict = await websocket.receive_json(mode='binary')

                    if message:
                        try:
                            message: ChannelSocketMessage = ChannelSocketMessage(**(message))
                        except:
                            message = None
                        ...
                        if message and message.action == "authorization":
                            ...
                            ehelply_active_participant: str = message.data['ehelply_active_participant']
                ...
                connection_manager.disconnect(participant_uuid)
                await network_controller.on_disconnect(participant_uuid, websocket)

            if authorized:
                await sockets_manager.connect(participant_uuid, websocket)
        ```
        """
        ...
        connection_string: str = str(hash(connection))
        ...
        try:
            while True:
                is_forwarded: bool = await self.forward_messages(identifier=identifier, connection=connection, blocking=False)
                message: ChannelSocketMessage = await self.get_message(identifier=identifier, connection=connection, blocking=False)
                if message:
    ...
    async def disconnect(self, identifier: str, connection=None):
        if connection:
            ...
            connection: str = str(hash(connection))
...
class SCMApiGateway(SocketConnectionManager):
    ...
    In the HTTP endpoint which API Gateway forwards to, add the following pseudo code:
    ```
    async def apigateway_websocket_endpoint(message: dict, connectionId: str):
        try:
            message: ChannelSocketMessage = ChannelSocketMessage(**(message))
        except:
            sockets_manager.disconnect(identifier=None, connection=connectionId)
        ...
        await sockets_manager.connect(participant_uuid, connectionId)
        await sockets_manager.on_receive(ehelply_active_participant, message)
    ```
    ...
    async def connect(self, identifier: str, connection):
        ...
        if len(identifier_connections) == 0 or connection in identifier_connections:
            return
