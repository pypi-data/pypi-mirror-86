    ...
    def convert(self, yaml=False, references=True, ignorespec=None, **options):
        ...
        for requestitem in self.postman_collection.get_requestitems():
            try:
                ...
                protocol_dict = requestitem_dict['requestitem']['request']['url']['protocol']
                host_dict = requestitem_dict['requestitem']['request']['url']['host']
                server_url.append(protocol_dict + '://' + '/'.join(host_dict))
                spec.path(
