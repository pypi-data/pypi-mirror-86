...
class AliSmsBase(object):
    ...
    def _compute_signature(d: dict, method: str, access_secret: str) -> str:
        ...
        # 2. 根据参数Key排序（顺序）
        # 3. 构造待签名的请求串
        query = "&".join([parse.urlencode({k: d[k]}) for k in sorted(d.keys())])
        data = f"{method}&%2F&{query}"
        sign = f"{access_secret}&{data}"

        signature = hmac.digest(access_secret.encode(), sign.encode(), "sha1")
