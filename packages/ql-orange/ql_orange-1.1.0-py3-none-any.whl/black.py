...
class FileMode:
    ...
    def get_cache_key(self) -> str:
        if self.target_versions:
            version_str = ",".join(
                str(version.value)
                for version in sorted(self.target_versions, key=lambda v: v.value)  # type: ignore
            )
        else:
            version_str = "-"
        parts = [
...
class Line:
    ...
    def is_collection_with_optional_trailing_comma(self) -> bool:
        ...
        if self.leaves[0].value in {'async', 'def'}:
            # function definition that fits on one line
            for r_index, leaf in enumerate(reversed(self.leaves)):
                if leaf.type == token.RPAR:
                    closer = leaf
                    close_index = len(self.leaves) - 1 - r_index
                    break
            else:
                return False
        elif self.leaves[-1].type == token.COLON:
            closer = self.leaves[-2]
            close_index = -2
        elif self.leaves[-1].type == token.COMMA and self.leaves[-3].type == token.COMMA:
            # Situation for nested collections
            closer = self.leaves[-2]
            close_index = -2
        else:
