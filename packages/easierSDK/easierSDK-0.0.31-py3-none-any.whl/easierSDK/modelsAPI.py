...
class ModelsAPI():
    ...
    def upload(self, category:Categories, easier_model:EasierModel, public=False, remove_dir=True) -> bool:
        ...
        minio_path = self._MODELS + '/' + category.value + '/' + easier_model.metadata.name
        ...
        try:
            for obj in os.listdir(path):
                ...
                last_experimentID = self._count_model_experiments(bucket, category.value, easier_model.metadata.name)
                # TODO check if models will be saved in HDF5 .h5 or tf SavedModel format (folder)
    ...
    def compile_tpu(self,  easier_model:EasierModel, calibration_data, path:str=_BASE_MODELS_PATH + "/storage/"):
        ...
        if easier_model.model_dir is not None:
            open('/tmp/storage/' + easier_model.model_dir + "/" + model.metadata.name + "." + constants.TF_LITE_EXTENSION, "wb").write(
                tflite_model)
            try:
                K.clear_session()
            except Exception as e:
