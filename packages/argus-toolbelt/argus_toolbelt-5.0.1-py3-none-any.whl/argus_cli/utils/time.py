...
# we need to disable try_previous_locales, otherwise sequential date parsing
# will yield different results.
date_parser = DateDataParser(try_previous_locales=False)
...
def time_parser(datetime_string: str):
    ...
    parser_data = date_parser.get_date_data(datetime_string)
    if parser_data:
        parsed = parser_data['date_obj']
...
def time_diff(datetime_string: str):
    ...
    return int(
        (datetime.now() - time_parser(datetime_string)).total_seconds() * 1e3)
