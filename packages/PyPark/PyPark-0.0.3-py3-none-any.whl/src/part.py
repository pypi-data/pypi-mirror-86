...
class Part(object):
    ...
    def __init__(self, zk_host, **kwargs):
        ...
        :param zk_base_path:str                 # zk_base_path, 默认为 "PyPart"
        :param zk_auth_data:str                 # zk ACL 例：[("digest", "username:password")]
        :param server_name:str                  # 服务名,默认："PyPart"
        ...
        zk_base_path = kwargs.get("zk_base_path", "PyPart")
    ...
    def run(self):
        ...
        # 设置路径
        logging.debug("-----------PyPart-------------")
        logging.info(f"{self.server_type} PyPart Start By {self.server_ip}:{self.service_port}")
        try:
