...
def send_mail_async_gmail(email_id, email_passwd, to, subject=None, message=None, html=None, files=[]):
    p = Process(target=send_mail, args=(email_id, email_passwd, to, subject, message, html, files), daemon=True)
    p.start()

def send_mail_gmail(email_id, email_passwd, to, subject=None, message=None, html=None, files=[]):
    ...
    for file in files:
        ...
        arg = 'attachment; filename="' + os.path.basename(file) + '"'
