...
# SendGrid
def send_mail(api_key, to, subject=None, message=None, html=None, files=[]):
    message = Mail(from_email='ops@dl2050.com', to_emails=to, subject=subject, html_content=html)
    try:
...
def send_mail_async(api_key, to, subject=None, message=None, html=None, files=[]):
    p = Process(target=send_mail, args=(api_key, to, subject, message, html, files), daemon=True)
    p.start()
...
class Notify():
    ...
    def __call__(self, who, how='mail', subject=None, message=None, html=None, files=[]):
        ...
        if how == 'email':
            return send_mail(self.api_key, to, subject=subject, message=message, html=html, files=files)
        if how == 'email_async':
            return send_mail_async(self.api_key, to, subject=subject, message=message, html=html, files=files)
        if how == 'sms':
    ...
    def send_mail_async(self, to, subject=None, message=None, html=None, files=[]):
        send_mail_async(self.api_key, to, subject=subject, message=message, html=html, files=files)
...
def send_ukey_by_email(notify, email, host, ukey, product):
    try:
        ...
        html = '<strong>From App server</strong>'
