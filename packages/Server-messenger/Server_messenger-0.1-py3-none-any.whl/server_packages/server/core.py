...
from server_part.server_packages.common.descriptors import Port
from server_part.server_packages.common.utils import send_message, get_message
from server_part.server_packages.common.variables import *
...
class MessageProcessor(threading.Thread):
    ...
    def init_socket(self):
        ...
        SERVER_LOGGER.info(f'Запущен сервер, порт для подключений: {self.port} , адрес с которого '
                           f'принимаются подключения: {self.address}.')
        # Готовим сокет
    ...
    def process_message(self, message):
        ...
        if message[DESTINATION] in self.names and self.names[message[DESTINATION]
            ...
            try:
                ...
                SERVER_LOGGER.info(f'Отправлено сообщение пользователю {message[DESTINATION]} от '
            ...
            SERVER_LOGGER.error(
                f'Пользователь {message[DESTINATION]} не зарегистрирован на сервере, отправка '
